...
public class SystemConfig extends HttpServlet {
	...
	public void init() {
		...
		try {
			 ...
			 */
			String databasePath = SystemConfig.class.getClassLoader().getResource("database.properties").getPath();
			Config.setConfigResource(databasePath);

			FileInputStream pInStream = new FileInputStream(new File(SystemConfig.class.getClassLoader().getResource("config.properties").getPath()));
