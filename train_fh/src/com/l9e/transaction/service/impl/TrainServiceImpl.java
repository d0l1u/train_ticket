...
public class TrainServiceImpl {
	...
	public int orderIsManual(final Order order, final Result result) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				sql_rand.append("select worker_id " + "from cp_workerinfo " + "where worker_type=? " + "and worker_status <>? " + "and worker_status <>? "
						+ "order by spare_thread " + "desc limit 1");
				...
				sql.append("select " + "worker_id, " + "worker_name, " + "worker_type, " + "worker_ext, " + "public_ip " + "from " + "cp_workerinfo " + "where "
						+ "worker_id=? " + "for " + "update");
				cs = cn.prepareStatement(sql.toString());
				...
				while (rs.next()) {
					...
					// 正常分配到的机器人参数
					worker2.setWorkerId(rs.getInt(1));
					worker2.setWorkerName(rs.getString(2));
					worker2.setWorkerType(rs.getInt(3));
					worker2.setWorkerExt(rs.getString(4));
					worker2.setPublicIp(rs.getString(5));
					list.add(worker2);
				}
	...
	public int orderIsStop(final Order order, final Result result) throws Exception {
		...
		logger.info(uuid + "【orderIsStop 账号临时停用】");
		...
		try {
			return dbbean.executeMethod(new ICallBack() {
				...
				public int execute(Connection cn, PreparedStatement cs) throws SQLException {
					...
					int accId = queryAccountIdByOrderNo(cn, cs, orderId);
					...
					if (StrUtil.containRegex(errorInfo, "手机.*核验") || StrUtil.containRegex(errorInfo, "联系人.*上限") || errorInfo.contains("账号状态为：未通过")
							|| StrUtil.containRegex(errorInfo, "重新.*注册") || errorInfo.contains("未通过核验，不能添加常用联系人") || errorInfo.contains("身份证件号码填写是否正确")) {
						cs.setString(1, Account.STATUS_STOP);
					} else {
					...
					if (StrUtil.containRegex(errorInfo, "手机.*核验")) {
						...
						cs.setString(2, "3");
					} else if (StrUtil.containRegex(errorInfo, "重新.*注册") || errorInfo.contains("账号状态为：未通过") || errorInfo.contains("未通过核验，不能添加常用联系人")
							|| errorInfo.contains("通过后即可在网上购票") || errorInfo.contains("身份证件号码填写是否正确")) {
						cs.setString(2, "1");
					} else {
						cs.setString(2, "2");
					}
					...
					// 添加操作账号日志
					logger.info(uuid + "stop 12306 account:" + accId);
					String opt_logs = "暂时停用账号";
					addAccLog(cn, cs, String.valueOf(accId), orderId, opt_logs);
	...
	public int orderIsEnd(final Order order, final Result result) throws Exception {
		final String uuid = order.getUuid();
		final String orderId = order.getOrderId();
		logger.info(uuid + "【orderIsEnd 账号封停】");
		try {
			return dbbean.executeMethod(new ICallBack() {
				// ResultSet rs = null;
				@Override
				public int execute(Connection cn, PreparedStatement cs) throws SQLException {
					...
					logger.info(uuid + "channelGroup:" + channelGroup + ", arrivetime:" + arrivetime);
					...
					int index = 1;
					cs.setString(index++, Order.STATUS_ORDER_RESEND);
					params.add(Order.STATUS_ORDER_RESEND);
					...
					cs.setString(index++, result.getFailReason());
					params.add(result.getFailReason());
					cs.setInt(index++, 0);
					params.add("0");
					cs.setString(index++, "robot");
					params.add("robot");
					...
					logger.info(uuid + "UPDATE ORDER SQL:" + sql.toString() + "; PARAMS:" + params.toString());
					cs.executeUpdate();

					// 账号封停用
					String errorInfo = result.getErrorInfo();
					logger.info(uuid + "账号封停用:ACCOUNT_ID" + accId + ":" + errorInfo);
					sql = new StringBuffer();
					sql.append("UPDATE cp_accountinfo SET ").append("acc_status = ?, ").append("order_id = ?, ").append("option_time = NOW(), ");
					if ("WAITCHECK".equals(errorInfo)) {
						sql.append("opt_person = 'check_app', ");
					} else if ("STOP".equals(errorInfo)) {
					...
					cs.setInt(index, accId);

					cs.executeUpdate();

					// 添加操作账号日志
					logger.info(uuid + "账号后续处理结束:" + accId);
					String opt_logs = "封停账号:" + accId;
					addAccLog(cn, cs, String.valueOf(accId), orderId, opt_logs);
	...
	public int orderIsSuccess(final Order order, final Result result) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				sql = new StringBuffer();
				// TODO TAOKAI
				// sql.append("update cp_orderinfo_cp set buy_money=?,
				// train_box=?, seat_no=?,seat_type=?, modify_time=now() where
				// cp_id=? and order_id=?");
				// sql.append("update cp_orderinfo_cp set buy_money=?,
				// train_box=?, seat_no=?, modify_time=now() where cp_id=? and
				// order_id=?");
				if ("tongcheng".equals(order.getChannel())) {
					sql.append(
		...
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection conn, PreparedStatement ps) throws SQLException {
				...
				sql.append("insert into cp_orderinfo_queue(order_id, resend_num, resend_time, create_time)")
						.append(" values (?, 1, date_add(now(), interval ? second), now())").append(" on duplicate key update ")
	...
	public int reGetAccountAndWorkerBy(final Map<String, String> map) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				log_work.append(" worker.getWorkerName()").append(worker.getWorkerName()).append(" work_url").append(worker.getWorkerExt());
	...
	public int selectAccountAndWorker(Order order, String workerWeight) {
		...
		logger.info(orderId + "-accountId:" + accountId);
		logger.info(orderId + "-accountFromWay:" + accountFromWay);
		if ((null != accountId && 0 != accountId) && (null != accountFromWay && 1 == accountFromWay)) {
			logger.info(orderId + "-根据账号ID查询账号...");
			account = Consts.accountService.getOrderAccount(accountId); // 查询账号
		} else {
			/* 按乘客证件匹配账号 */
			logger.info(orderId + "-获取乘客信息...");
			...
			// }
			// 多人匹配模式
			logger.info(orderId + "-根据乘客信息获取账号...");
			if (passengers != null && passengers.size() > 0) {
				...
				logger.info(order + "-乘客信息passportNos:" + passportNos.toString());
				...
				if (account != null) {
					...
					logger.info(orderId + "-白名单匹配成功了,更新白名单表的匹配次数和时间...");
					try {
						updateMatchingNumAndTime(order, account, passengers);
					} catch (Exception e) {
						logger.info(orderId + "-【系统异常】:更新白名单表的匹配次数和时间发生异常", e);
					}
				}
			}
			...
			/* 按渠道获取账号 */
			if (account == null) {
				logger.info(orderId + "-白名单匹配失败,获取队列[" + passengers.size() + "]中的账号...");
				account = Consts.accountService.getChannelAccount(Order.CHANNEL_19E, passengers.size());
				if (account == null) {
					logger.info(orderId + "-队列账号获取失败,获取队列[10]中的账号...");
					// 取账号联系人个数小于10的账号
					account = Consts.accountService.getChannelAccount(Order.CHANNEL_19E, 0);
				}
			}
		...
		logger.info(orderId + "-账号获取结果:" + account);
		if (account == null) {
			return 1;
		}
		...
		try {
			...
			logger.info(orderId + "-【系统异常】:getCacheVal发生异常", e);
		}
		logger.info(orderId + "-cacheValue:" + cacheValue);

		logger.info(orderId + "-开始获取机器人信息########");
		/* 获取机器人 */
		if (cacheValue != null) {
			try {
				Integer workerId = (Integer) cacheValue;
				logger.info(orderId + "-根据cacheID获取机器人信息...");
				worker = Consts.workerService.getWorkerById(workerId);
			} catch (Exception e) {
				logger.info(orderId + "-【系统异常】:getWorkerById发生异常", e);
			}
		}
		/**
		 ...
		 */
		if (worker == null) {
			logger.info(orderId + "-cache机器人信息为空,查询数据库,获取机器人...");
			if (WorkerWeight.WEIGHT_LUA.equals(workerWeight)) {
				worker = Consts.workerService.getWorkerByType(Worker.TYPE_BOOK);
				logger.info(orderId + "-lua脚本的机器人:" + worker);
			} else if (WorkerWeight.WEIGHT_JAVA.equals(workerWeight)) {
				worker = Consts.workerService.getOneJavaWorker();
				logger.info(orderId + "-java脚本的机器人:" + worker);
			}
		}

		// 如果上面还是没有获取到机器，则释放账号
		if (worker == null) {
			logger.info(orderId + "-最终获取机器人为空,释放账号...");
		...
		/* 订单关联关系建立 */
		logger.info(orderId + "-订单、账号、机器人关联关系建立...");
	...
	public int selectOrderAccountAndWorkerBy(final Map<String, String> map) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				// cs.setString(2, worker.getWorkerId());
				// cs.executeUpdate();
				// logger.info("end update workerinfo
				// workerid:"+worker.getWorkerId()+" orderid:"+orderId);
				// // end
				// worker.setWorkerStatus(Worker.STATUS_WORKING);
	...
	public int getBookWorkInfo() throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				sql.setLength(0);
				sql.append("select " + "worker_id, " + "worker_name, " + "worker_type, " + "worker_ext, " + "worker_status, " + "publicIp ")
	...
	public int getBookWorkInfo(final String worker_id) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				// WHERE setting_name = 'book_interval_time'");
				// cs = cn.prepareStatement(sql.toString());
				// rs = cs.executeQuery();
				// if(rs.next()) {
				// String settingValue = rs.getString("setting_value");
				...
				//
				// sql.setLength(0);
				sql.append("select " + "worker_id, " + "worker_name, " + "worker_type, " + "worker_ext, " + "worker_status, " + "public_ip ")
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				if (filterList != null && filterList.size() > 0) {
					...
					for (AccountFilter af : filterList) {
						...
						cpBuilder.append(af.getIds_card()).append("|").append(af.getReal_name()).append("|").append("1|100");
	...
	public int queryAccountIdByCardsId(Connection cn, PreparedStatement cs, List<String> list) throws SQLException {
		...
		// if(list.size()==1){
		// StringBuffer sql = new StringBuffer();
		// sql.append("SELECT account_id FROM cp_accountinfo_filter WHERE
		// ids_card=?");
		// cs = cn.prepareStatement(sql.toString());
	 ...
	 * @throws SQLException
	 */
	public int getAccountForNewOrder(Connection cn, PreparedStatement cs, String orderId, String channel, String acc_id) throws SQLException {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				if (rs.next()) {
					...
					if (num >= 1) {
						...
						sql = new StringBuffer();
						sql.append("select count(1) from t_zjpj_a WHERE fz=? AND dz=? AND cc LIKE '").append(result.getTrainNo()).append("/%'");
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				if (queue.size() >= 15) {
					...
					synchronized (Consts.ACCLOCK) {
						...
						cs.setString(index, channel);
						logger.info(sql.toString() + "_" + channel + "_" + WorkIdNum.contact_num + "_" + WorkIdNum.limit_day + "_" + WorkIdNum.book_num + "_"
								+ Account.STATUS_FREE);
						rs = cs.executeQuery();
	...
	public int queryStudentInfo(final String order_id, final String cp_id, final String paramCp) throws RepeatException, DatabaseException {
		...
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				if (rs.next()) {
					paramSb.append("|").append(rs.getString(1)).append("|").append(rs.getString(2)).append("|").append(rs.getString(3)).append("|")
							.append(rs.getString(4)).append("|").append(rs.getString(5)).append("|").append(rs.getString(6)).append("|").append(rs.getString(7))
							.append("|").append(rs.getString(8) == null ? "" : rs.getString(8)).append("|")
							.append(rs.getString(9) == null ? "" : rs.getString(9)).append("|").append(rs.getString(10) == null ? "" : rs.getString(10))
	...
	public int initPayAccountAndWorkerBy(final String orderId) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				sql_rand = new StringBuffer();
				sql_rand.append("select " + "worker_id, " + "worker_name, " + "worker_type, " + "worker_ext, " + "public_ip ")
	 ...
	 * @throws DatabaseException
	 */
	public String updatePassAcc(final String acc_id, final String minute, final String acc_username, final String acc_password, final String worker_ext)
	...
	public int insertTuniuOrderQueue(final Map<String, Object> map) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement ps) throws SQLException {
				...
				sql.append("insert into tuniu_order_queue (order_id,queue_number,wait_time,msg,create_time,notify_status) values(?,?,?,?,now(),?)");
	 ...
	 * @throws DatabaseException
	 */
	public void updateOrderChooseSeatType(final String orderId, final String chooseSeatType) throws RepeatException, DatabaseException {
	 ...
	 */
	public void updateMatchingNumAndTime(final Order order, final Account account, final List<Passenger> passengers) throws RepeatException, DatabaseException {
		dbbean.executeMethod(new ICallBack() {
				...
				if (passengers != null && passengers.size() > 0) {
					for (int i = 0; i < passengers.size(); i++) {
						...
						if (null != certNo) {
							...
							// 更新白名单表的匹配次数和时间
							sql = new StringBuffer();
							sql.append("update cp_pass_whitelist set count_num=count_num+1,last_match_time=now(),update_time=now() ");
							sql.append(" where cert_no=? and acc_id=?");
	 ...
	 * 
	 * @author: taoka
	 * @date: 2018年1月31日 下午5:47:29
	 * @param workId
	 * @param ip
	 * @throws Exception
	 *             void
	 */
	public int updateWorkIp(final int workId, final String ip) throws Exception {
		return dbbean.executeMethod(new ICallBack() {
			@Override
			public int execute(Connection conn, PreparedStatement ps) throws SQLException {
				StringBuilder sql = new StringBuilder();
				sql.append("update cp_workerinfo set public_ip = ? where worker_id = ?");
				ps = conn.prepareStatement(sql.toString());
				ps.setString(1, ip);
				ps.setInt(2, workId);
				int num = ps.executeUpdate();
	...
	public int updateIpStatus(final int ipId) throws Exception {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection conn, PreparedStatement ps) throws SQLException {
				StringBuilder sql = new StringBuilder();
				sql.append("UPDATE cp_ipinfo SET ").append("option_time = now(), ").append("ip_status = '00', request_num = 1 ").append("WHERE ")
						.append("ip_id = ?");
				ps = conn.prepareStatement(sql.toString());
				ps.setInt(1, ipId);
				int num = ps.executeUpdate();
		...
		return this.dbbean.executeMethod(new ICallBack() {
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				sql.append("SELECT ")//
						...
						.append("TIMEDIFF(now() , ho.out_ticket_time)<'01:00:00' ").append("AND ho.out_ticket_billno=? ");
