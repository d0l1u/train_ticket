...
public class OrderServiceImpl implements OrderService {
	...
	public String addOrder(OrderInfo orderInfo,List<OrderInfoCp> orderInfoCpList, List<OrderInfoBx> orderInfoBxList, 
		...
		for (BookStuInfo bookStuInfo : studentsList) {//学生票
			orderDao.addBookStuInfo(bookStuInfo);
		}
...
//				deleteAllOrderInfo(orderInfo.getOrder_id());
	...
	@Override
	public void addAlterOrderinfo(List<Map<String,String>> param_list ,Map<String, String> param) {
		for(Map<String,String> orderCp :param_list){
			orderDao.addAlterOrderinfo(orderCp);
		}
		//添加到改签结果通知表中
		orderDao.addAlterOrderNotify(param);
	}
	@Override
	public int queryAlterOrderifoNum(Map<String, String> param) {
		return orderDao.queryAlterOrderifoNum(param);
	}
	@Override
	...
	public void createEopOrder(OrderInfo orderInfo,int size,String goods_price){
			...
			try{
				...
				logger.info("eop下单结果："+"eop_order_id:"+orderInfo.getOrder_id()+",order_id:"+orderInfo.getOrder_id()+",Result_code:"+bean.getResult_code()
						+",Result_desc:"+bean.getResult_desc());
				...
				if("SUCCESS".equals(bean.getResult_code())){
					...
					orderDao.insertOrderLogs(logs);
					throw new TrainException("eop重复下单失败！");
				}else if("CREATE_ORDER_ERROR".equals(bean.getResult_code())){
					logger.error(bean.getEop_order_id()+"eop下单失败！失败原因："+bean.getResult_desc());
					logs.setOrder_optlog("eop下单失败："+bean.getResult_desc());
					orderDao.insertOrderLogs(logs);
					throw new TrainException("eop下单失败！");
				}else{
					logger.error(bean.getEop_order_id()+"eop下单失败！失败原因："+bean.getResult_desc());
					logs.setOrder_optlog("eop下单失败："+bean.getResult_desc());
					orderDao.insertOrderLogs(logs);
	...
	public String selectOrderLog(Map<String, Object> paramMap) {
		return orderDao.selectOrderLog(paramMap);
	}
