...
public class BaseController {
	...
	public String getParam(HttpServletRequest request, String param) {
		return request.getParameter(param) == null ? "" : request
				.getParameter(param).toString().trim();
	}
	 ...
	 * @param str
	 * @param key_check
	 *            防重提交,两次请求，都未处理成功，又来了同样的一单,处理成功后，将其移出
	 */
	public static void printJson(HttpServletResponse response, String str,
			String key_check) {
		response.setCharacterEncoding("utf-8");
		...
		try {
			...
			out.flush();
			out.close();
			MemcachedUtil.getInstance().removeAttribute(key_check);
		}
	}
