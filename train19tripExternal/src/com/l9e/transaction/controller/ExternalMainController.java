...
public class ExternalMainController extends ExternalBase {
    private static final Logger logger = Logger.getLogger(ExternalMainController.class);
	@Resource
...
//					}
	...
	public void querySubwayStation(HttpServletRequest request,
		...
		paramMap.put("train_code",train_code);
		paramMap.put("merchant_id",merchant_id);
		logger.info("查询途径站线路选择：1.本地库；2.携程数据接口");
		//String channel="2";
		String channel = getSysInterfaceChannel("stopStion_channel");//查询途径站线路
		if("2".equals(channel)){
			logger.info("查询携程的途径站数据！！！");
			queryStopStaionFromCtripData(response,paramMap);
		}else{
			logger.info("查询本地数据库的途径站数据！！！");
			queryStopStaionFromLocalData(response,paramMap);
		}


	}

	public void queryStopStaionFromCtripData(HttpServletResponse response, Map<String, String> paramMap) {

		JSONObject json = new JSONObject();
		String train_code=paramMap.get("train_code");
		String merchant_id=paramMap.get("merchant_id");

		StringBuffer param = new StringBuffer();
		param.append("train_code=").append(train_code).append("&channel=").append("ext_").append(merchant_id).append("&type=").append("stopStation");
		String ctrip_data_url=getSysInterfaceChannel("ctrip_data_interface_url");
		logger.info(ctrip_data_url+",参数："+param.toString());
		String jsonStr=HttpUtil.sendByPost(ctrip_data_url, param.toString(), "utf-8");//调用接口
		logger.info("查询途径结果:["+ctrip_data_url+"]:"+jsonStr);
		...
		JSONObject ctripJson=JSONObject.fromObject(jsonStr);
        JSONArray jsonArray=ctripJson.getJSONArray("train_stationinfo");
		List<TrainStationVo>  stationList=new ArrayList<TrainStationVo>();

        for (int i=0;i<jsonArray.size();i++){
            TrainStationVo tvo=new TrainStationVo();
            JSONObject obj=jsonArray.getJSONObject(i);
            tvo.setStationno(obj.getString("stationno"));
            tvo.setName(obj.getString("name"));
            tvo.setStarttime(obj.getString("starttime"));
            tvo.setArrtime(obj.getString("arrtime"));
            tvo.setDistance(obj.getString("distance"));
            tvo.setInterval(obj.getString("interval"));
            tvo.setCosttime(obj.getString("costtime"));
            stationList.add(tvo);
        }
        ...
        json.put("message", "");
        JSONArray arrayStation = JSONArray.fromObject(stationList);
        json.put("train_stationinfo", arrayStation);
        printJson(response, json.toString());
	...
	public void queryStopStaionFromLocalData(HttpServletResponse response,Map<String,String> paramMap) {

		JSONObject json = new JSONObject();
		String train_code=paramMap.get("train_code");
		try {
            //String trainCode = ticketService.queryTheCheciForStation(train_code);
	...
	public void checkTicketNum(HttpServletRequest request,
		...
		if (!hmac_19.equals(hmac)) {
			...
			logger.info("实时余票查询接口-调用实时余票查询接口开始");
			//3.自有12306余票查询  2.携程查询
			String channel = getSysInterfaceChannel("INTERFACE_CHANNEL");//该字段只能为3或空
			try{
				if (StringUtils.isEmpty(channel) || "3".equals(channel)) {
					newBuyTicketController.newQueryData(paramMap, request, response);
				} /*else if ("2".equals(channel)) {
