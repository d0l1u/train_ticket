...
//					}
				...
				if (count == 1){
					...
					if(!"88".equals(channel)&&"1".equals(orderInfo.getSms_notify())){//是否发短信
						//发送出票成功短信
	...
	public void refundSendNotify(HttpServletRequest request,
		...
		try{
			...
			if("0".equals(status) && TrainConsts.SUCCESS.equalsIgnoreCase(result)){
				...
				logger.info("更改ext_orderinfo_cp表的alter_money="+alterdiffmoney);
				write2Response(response, "success");
...
//				orderService.updateRefundInfo(map);
				...
				logger.info("更改ext_orderinfo_refundstream表的refund_status=11同意退款");
				write2Response(response, "success");
...
//				orderService.updateRefundInfo(map);
				logger.info("更改ext_orderinfo_refundstream表的refund_status=22拒绝退款");
				paramMap.put("notify_status","22");
				refundService.updateRefundNotifyRestart(paramMap);
				write2Response(response, "success");
				...
				//map.put("refund_type", "1");
				orderService.updateRefundInfo(map);
				logger.info("更改ext_orderinfo_refundstream表的refund_status=99搁置订单");
				write2Response(response, "success");
			}
		}catch(Exception e){
