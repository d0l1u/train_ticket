...
public class RefundStreamJob {
	...
	private void sendRefundNotfiyRequest(Map<String, String> notifyMap) throws Exception{
		String order_id = notifyMap.get("order_id");
		logger.info("发起退款通知，订单号："+order_id);
		String refund_seq = notifyMap.get("refund_seq");
		...
		for(Map<String, String> refundMap: rsList){
			...
			refund_type = refundMap.get("refund_type");
			//根据cp_id查询乘客信息
			...
			if(StringUtils.isEmpty(status)){
				...
				logger.info("已经退票成功一笔车票号："+refundMap.get("cp_id"));
			}else{
				logger.info("退票状态status为错误！status="+status);
				return;
			}
		...
		}else if(agreeNum == 0 && refuseNum == cpList.size() || refuseNum>0&&refuseNum != cpList.size()&&agreeNum==0){	
			all_status = "FAILURE";
		...
		logger.info(order_id+"-->"+"agreeNum:"+agreeNum+",refuseNum:"+refuseNum+",cpList.size():"+cpList.size()+"all_status:"+all_status);
		...
		if(TrainConsts.ORDER_PAY_TYPE_11.equals(merchantSetting.get("pay_type"))){
			if(!"FAILURE".equals(all_status) && agreeNum >0){
				if(!StringUtil.isEmpty(order.getEop_refund_url())){
					...
					if(refund_type.equals("3")){
						//改签退票
						logger.info("改签退票不添加eop退款请求。order_id:"+order_id+"refund_seq:"+refund_seq);
					}else if(receiveService.queryEopRefundNotifyAlterNum(refund_seq)==0){
...
//		  .append("&json_param=").append(json_param.toString())
		...
		if(!StringUtils.isEmpty(res) && ("SUCCESS".equals(res.toUpperCase().trim())|| res.contains("SUCCESS"))){
			...
			logger.info("退票结果通知商户成功" + "订单号：" + order_id
					+ "，退款流水号：" + refund_seq);
			logs.setOrder_optlog("退票结果通知商户成功");
			if(TrainConsts.ORDER_PAY_TYPE_22.equals(merchantSetting.get("pay_type"))){//查询如果是商户自行扣款则 同意退款的通知成功则 改为退款完成
				...
				try{
					...
					logs.setOrder_optlog("退票结果通知商户成功（属于商户自行扣费直接改为退款完成）");
				}catch (Exception e) {
			...
			logs.setOrder_optlog("退票结果通知商户失败");
		}
