...
public class SendJob {
	...
	private void notifyCpSys(String old_status,String order_id,String book_flow){
		try{
			...
			paramMap.put("channel", orderMap.get("merchant_id"));//合作商编号
			paramMap.put("ispay", book_flow);	//是否支付；00：是；11：否
			paramMap.put("username", orderMap.get("account_name"));//12306账号
			paramMap.put("password", orderMap.get("account_pwd"));//12306账号密码
			...
			if(!StringUtils.isEmpty(orderMap.get("ext_seat"))){
				...
				extSb.append(orderMap.get("seat_type")).append("#").append(orderMap.get("ext_seat"));
			...
			if(!StringUtils.isEmpty(result)){
				...
				if("success".equalsIgnoreCase(results[0]) && results.length == 2
				    ...
				    map.put("old_status", old_status);//支付完成或eop发货
