                	...
                	<table class="pub_table">
                    	<tr>
                        	<td class="pub_yuliu" rowspan="5">
                            	<strong>${train_no}</strong><br />
                            	<span class="checi">${trainTypeCn}</span>
                            </td>
                            <c:if test="${isRePay eq 'yes'}">
                                <td class="addr" width="500"><strong>${order.fromCity}<span>（${fromTime}）</span></trong>—<strong>${order.toCity}<span>（${toTime}）</span></strong></td>
                            </c:if>
                            <c:if test="${empty isRePay}">
                            	<td class="addr" width="500"><strong>${from_city}<span>（${fn:split(from_time,' ')[1]}）</span></trong>—<strong>${to_city}<span>（${fn:split(to_time,' ')[1]}）</span></strong></td>
                        	</c:if>
                        </tr>
                        <tr>

                        	<td width="234">日 期：<span>${travelTime}</span></td>
                        </tr>
                        <tr>
                        	<c:if test="${isRePay eq 'yes'}">
                        		<td width="234">
                        			坐 席：<span>${seatType}</span>
                        		</td>
                        	</c:if>
                        	<c:if test="${empty isRePay}">
                        		<td width="234">
                        		坐 席：<span>${fn:split(defaultSeat,'_')[2]}</span></td>
                        	</c:if>
                        ...
                        <TR>
									...
									<c:if test="${empty isRePay}">
										<TD id="bkSelected" width=234>
											备选坐席：
											<c:set var="Seats" value="${fn:split(selectSeats,',')}"></c:set>
											<c:forEach var="seat" items="${Seats}">
												<SPAN>${fn:split(seat,'_')[2]}:${fn:split(seat,'_')[1]}</SPAN>&nbsp;&nbsp;
											</c:forEach>
										</TD>
									</c:if>
								</TR>
	                ...
	                <div class="new_pub_con">
	                	<table class="pub_table">
	                    	<tr>
	                        	...
	                        	<td width="234">联 系 人：<span>${link_name}</span></td>
	                        	<td>手 机：<span>${link_phone}</span></td>
	                        </tr>
	                    </table>
	                </div>
	        	</div>
                ...
                <div class="new_pub_con">
                	<table class="pub_table">
                    	<tr>
                        	...
                        	<td>票 价：<span style="font-weight: bold;color: red;"><fmt:formatNumber value="${max12306Price}" type="currency" pattern="#0.00元"/></span></td>
