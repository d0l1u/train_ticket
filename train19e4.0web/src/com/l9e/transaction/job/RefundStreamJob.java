...
public class RefundStreamJob {
	...
	private void sendRefundStreamRequest(Map<String, String> refundMap){
		...
		if(count == 1){
			...
			}else if(TrainConsts.REFUND_TYPE_6.equals(refundMap.get("refund_type"))){
				bean.setRefund_reason("电话退款");
				bean.setRemark("电话退款");
				logger.info("【退款流水接口】电话退款，ASP订单号：" + refundMap.get("order_id"));
			}else if(TrainConsts.REFUND_TYPE_7.equals(refundMap.get("refund_type"))){
				bean.setRefund_reason("取消预约退款");
				bean.setRemark("取消预约发起退款");
				logger.info("【退款流水接口】取消预约退款，ASP订单号：" + refundMap.get("order_id"));
			}else if(TrainConsts.REFUND_TYPE_8.equals(refundMap.get("refund_type"))){
				bean.setRefund_reason("线下退款");
				bean.setRemark("线下退款");
				logger.info("【退款流水接口】线下退款，ASP订单号：" + refundMap.get("order_id"));
			}
			...
			logMap.put("order_id", refundMap.get("order_id"));
			logMap.put("cp_id", refundMap.get("cp_id"));
			//将原有逻辑申请成功转为eop退款中修改为退款完成
...
//			    map.put("old_refund_status", TrainConsts.REFUND_STREAM_BEGIN_REFUND);//开始退款
				...
				logMap.put("order_optlog", "eop申请退款成功，接受退款申请");
				...
				logger.info("【退款流水接口】重复退款申请-退款中" + "ASP订单号：" + bean.getAsp_order_id()
						+ "，ASP退款流水号：" + bean.getAsp_refund_number());
				logMap.put("order_optlog", "eop重复退款申请-退款中");
				logMap.put("opter", "19e");
				...
				map.put("refund_seq", bean.getAsp_refund_number());//ASP退款请求流水号
			    map.put("eop_refund_seq", bean.getEop_refund_seq());//EOP退款流水号
			    map.put("refund_status", TrainConsts.REFUND_STREAM_EOP_REFUNDING);//EOP退款
//			    map.put("old_refund_status", TrainConsts.REFUND_STREAM_BEGIN_REFUND);//开始退款
			    orderService.updateOrderStreamStatus(map);
			    
				logger.info("【退款流水接口】重复退款申请-已完成" + "ASP订单号：" + bean.getAsp_order_id()
						+ "，ASP退款流水号：" + bean.getAsp_refund_number());
				logMap.put("order_optlog", "eop重复退款申请-已完成");
				logMap.put("opter", "19e");
			}else{//申请失败
				...
				logMap.put("order_optlog", "请联系平台：result_code="+bean.getResult_code()+",eop退款申请失败,失败原因："+bean.getResult_desc());
				logMap.put("opter", "19e");
			}
