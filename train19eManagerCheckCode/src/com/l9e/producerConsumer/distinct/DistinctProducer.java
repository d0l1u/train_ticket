...
public abstract class DistinctProducer<T> implements Runnable {
	...
	public final void run() {
		try{
			while(!Thread.interrupted()){
				//获取队列
				List<T> list = getProducts(this.robotService);
				...
				if(list!=null&&list.size()>0){
					...
					for(T t:list){	
						...
						if(puttime!=null){
							...
							logger.debug("资源"+objectKeyId+"正在处理中,放弃该资源并继续循环 puttime="+puttime+" currentTime="+currentTime);
							continue;
						}
						...
						if(succtime!=null){
							...
							if(currentTime<=succtime){
								logger.debug("资源"+objectKeyId+"已成功,放弃该资源并继续循环 succtime="+succtime+" currentTime="+currentTime);
								//删除dealingmap中的缓存
								dealingmap.remove(objectKeyId);
								continue;
							}else{
								succmap.remove(objectKeyId);
								logger.debug("资源"+objectKeyId+"已成功,但获取资源的时间大于成功的时间,需要再次发送 succtime="+succtime+" currentTime="+currentTime);
							}
						}
						queue.put(t);
	...
	//获取资源list抽象方法
	public abstract List<T> getProducts(RobotCodeService robotService);
