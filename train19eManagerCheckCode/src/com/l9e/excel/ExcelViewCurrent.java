...
import com.l9e.transaction.vo.ExcelCurrentVo;
import com.l9e.util.StringUtil;

public class ExcelViewCurrent extends AbstractExcelView {

	/**
	 * 超级管理员--打码用户--导出Excel
	 */
	private void excelAdminCurrent(Map<String, Object> map,
			HttpServletRequest request, HttpServletResponse response)throws ParseException {
		List<ExcelCurrentVo> orderlist = (List<ExcelCurrentVo>) map.get("list");
//		String begin_time = (String) map.get("begin_time");
//		String end_time = (String) map.get("end_time");
		HSSFWorkbook workbook = new HSSFWorkbook();
		HSSFSheet sheet = workbook.createSheet("打码用户");
		...
		// 查询需要导出的信息
		Iterator<ExcelCurrentVo> it = orderlist.iterator();
		HSSFRow row = null;
		int index = 0;
		...
		StringBuffer header = new StringBuffer("用户打码上月收入统计");
//        if (orderlist.size() > 0) {
//        	header.append(orderlist.get(0).getProvince_name()); 
//        } header.append("出票明细");
		 
		String[] topHeaders = { header.toString(), null, null, null, null,null, null,null,null,null};

		for (int i = 0, n = topHeaders.length; i < n; i++) {
			this.createCellAndSetStrVal(row, i, firStyle, topHeaders[i]);
		}
		sheet.addMergedRegion(new Region(index, (short) 0, index, (short) 10));
		index++;
		// 写入第二行
		row = sheet.createRow(index);
		row.setHeight((short) 600);
...
//		}
		...
		String[] secHeaders = { xsDate.toString(), null, null, null, null,null, null,null,null,null};
		...
		String[] headers = {"序号","用户名","真实姓名","所在部门","打码总数","打码成功","打码失败","打码超时","截止时间","上月收入"};
		...
		while (it.hasNext()) {
			// 每条订单数据在excel中产生1行
			ExcelCurrentVo orderExcel = (ExcelCurrentVo) it.next();
			// 写入第五行
			row = sheet.createRow(index);
			...
			if(orderExcel.getOpt_ren()==null || StringUtils.isEmpty(orderExcel.getOpt_ren())){//用户名
				...
				this.createCellAndSetStrVal(row, 1, contentStyle,orderExcel.getOpt_ren());
			}
			if(orderExcel.getOpt_name()==null || StringUtils.isEmpty(orderExcel.getOpt_name())){//真实姓名
				...
				this.createCellAndSetStrVal(row, 2, contentStyle,orderExcel.getOpt_name());
			}
			...
			if(orderExcel.getDepartment()==null || StringUtils.isEmpty(orderExcel.getDepartment())){//所在部门
				...
				this.createCellAndSetStrVal(row, 3, contentStyle,department);
			}
			if(orderExcel.getPic_count()==null || StringUtils.isEmpty(orderExcel.getPic_count().toString())){//打码总数
				this.createCellAndSetNumberVal(row, 4, contentStyle,"");
			}else{
				this.createCellAndSetNumberVal(row, 4, contentStyle,orderExcel.getPic_count().toString());
			}
			if(orderExcel.getPic_success()==null || StringUtils.isEmpty(orderExcel.getPic_success().toString())){//打码成功
				...
				this.createCellAndSetNumberVal(row, 5, contentStyle,orderExcel.getPic_success().toString());
			}
			if(orderExcel.getPic_fail()==null || StringUtils.isEmpty(orderExcel.getPic_fail().toString())){//打码失败
				...
				this.createCellAndSetNumberVal(row, 6, contentStyle,orderExcel.getPic_fail().toString());
			}
			if(orderExcel.getPic_unkonwn()==null || StringUtils.isEmpty(orderExcel.getPic_unkonwn().toString())){//打码超时
				...
				this.createCellAndSetNumberVal(row, 7, contentStyle,orderExcel.getPic_unkonwn().toString());
			}
			
			if(orderExcel.getTime()==null || StringUtils.isEmpty(orderExcel.getTime())){//截止时间
				this.createCellAndSetStrVal(row, 8, contentStyle,"");
			}else{
				this.createCellAndSetStrVal(row, 8, contentStyle,orderExcel.getTime());
			}
			
			if(orderExcel.getSuccessPreCount()==null || StringUtils.isEmpty(orderExcel.getSuccessPreCount())){//上月收入
				this.createCellAndSetMoneyVal(row, 9, moneyStyle,"");
			}else{
				this.createCellAndSetMoneyVal(row, 9, moneyStyle,orderExcel.getSuccessPreCount());
			}
		...
		try {
			// 输出流导出
			OutputStream os;
			String title = "用户打码上月收入统计"+str1+".xls";
	...
	protected void buildExcelDocument(Map<String, Object> map,
		...
		this.excelAdminCurrent(map, request, response);
	}
