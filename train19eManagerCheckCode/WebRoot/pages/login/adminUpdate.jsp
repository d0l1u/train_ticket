...
<script type="text/javascript">
	...
	function onblurGet_code_time_limit(){
		$("#get_code_time_limit_info").text("");
		var get_code_time_limit=$.trim( document.getElementById("get_code_time_limit").value );
		var reg = /^\d+$/;
		//var reg = /^[1]\d+$/;
		if(get_code_time_limit==""){
			//$("#telephone_info").text("手机不能为空!").css("color","#f00");
		}else if((!get_code_time_limit=="") && (reg.test(get_code_time_limit))==false){
			$("#get_code_time_limit_info").text("拉码间隔格式不正确!").css("color","#f00");
		}
	}
</script>
</head>
...
<body>
	...
	<form action="/loginManager/updateAdminUser.do?user_id=${userMap.user_id }" method="post" name="updateForm" id="updateForm" onsubmit="return checkSubmit();">
        <ul class="order_num oz">
        	...
        	<li><span style="color:red">*</span>所在部门：
        		<select name="department" style="width: 145px;" onclick="selectReadOnly('id_select')"> 
		        	<c:forEach items="${userDepartment }" var="t">
   						<option value="${t.key}" <c:if test="${!empty userMap.department && userMap.department eq '${t.key}'}">selected</c:if>>${t.value}</option>
   					</c:forEach>
		       	</select>
        	</li>
        	...
        	<li><span style="color:red">*</span>拉码间隔：&nbsp;<input type="text" name="get_code_time_limit" id="get_code_time_limit" value="${userMap.get_code_time_limit }" onblur="onblurGet_code_time_limit()" style="width: 140px;"/>
        			<span id="get_code_time_limit_info"></span></li>
