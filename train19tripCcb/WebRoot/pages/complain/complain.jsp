    		...
    		<div class="tip_term oz" style="margin:10px auto 0;">
         	...
         	<div class="advice oz">
            	<h2>
                	...
                	<span class="nav_mid_bg">问题描述</span>
                ...
                <div class="con1 oz">
                	<dl class="oz">
                        ...
                        <c:if test="${isNULL eq 0}">
                        <dt>手机号：</dt>
                        <dd><input type="text" id=user_phone name="user_phone"/></dd>
                        </c:if>
                    </dl>
                    <dl class="oz">
                ...
                <div class="con2 oz">
                	<h3>投诉与建议</h3> 
                	   <c:if test="${isNULL eq 0}">
						<p style="margin-left:100px;">请输入您的手机号查询您以前的投诉信息 :
							<input type="text" name="user_queryPhone" id="user_queryPhone"/>
							<input type="button" id="noVipBtn" value="查询" />
						</p>
						</c:if>
                    <div class="ques oz">
...
<script type="text/javascript" language="JavaScript">
	$().ready(function(){
		//游客查询以前的投诉信息
		$("#noVipBtn").click(function(){
			var user_queryPhone=$.trim($("#user_queryPhone").val());
			window.location="/complain/complainIndex.jhtml?user_queryPhone="+user_queryPhone;	
		});
		...
		$("#btnOK").click(function(){
			...
			if(userId=="1" ){
				...
				if(!checkQuestionType() ||!checkUserIdPhone() ||  !checkQuestion() || !checkDailyOnlyOneNoVIP()){
					return;
				}else{
					$("form:first").submit();
				}
		...
		//检查手机号，因为是游客，所以在这里的手机号将用来做为user_id使用
		function checkUserIdPhone(){
			if($.trim($("#user_phone").val())==""){
				$("#user_phone").focus();
				showErrMsg("user_phone", "100px", "请填写手机号！");
				setTimeout(function(){
					hideErrMsg("user_phone")},
					3000
				); 
				return false;
			}else if(!/^1(3[\d]|4[57]|5[\d]|8[\d]|70)\d{8}$/g.test($.trim($("#user_phone").val()))){
				$("#user_phone").focus();
				showErrMsg("user_phone", "150px", "请填写正确的手机号！");
				setTimeout(function(){
					hideErrMsg("user_phone")},
					3000
				); 
	    ...
	    function checkDailyOnlyOneNoVIP(){
	    	var isOK = false;
