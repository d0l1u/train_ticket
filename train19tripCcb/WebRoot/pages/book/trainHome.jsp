	...
	<div class="content shouy oz">
    	...
    	<div class="left_con oz">
            <div class="tip_term oz" style="margin:10px auto 0;">
            	<p class="price_tip">
            		<span >温馨提醒：火车票订票时间为早7:00-晚23:00，23:00之后的订单将在次日早7:00之后处理。</span>
            ...
            <div class="shouy_ser mb10">
                ...
                <dl class="oz">
                	<dt>出发城市</dt>
                    <dd><input class="text" type="text" id="from_station" name="from_station" 
                    		onfocus="showCity('from_station')" onblur="hideCity()" value="出发城市" /></dd>
                </dl>
                <div class="swap" onclick="swapCity();"></div>
                <dl class="oz">
                	<dt>到达城市</dt>
                    <dd><input class="text" type="text" id="arrive_station" name="arrive_station" 
                    		onfocus="showCity('arrive_station')" onblur="hideCity()" value="到达城市" /></dd>
                </dl>
                 <dl class="oz">
                	<dt>出发日期</dt>
                ...
                <p><input type="button" class="btn" id="query" onclick="javascript:queryTrainInfo('','');" value="搜 索" /></p>
            </div>
            </form>
...
<script type="text/javascript" language="JavaScript">
	...
	function queryTrainInfo(from_station,arrive_station){
			...
			if(from_station!=null && from_station!=""){
				...
				if($.trim($("#from_station").val())=="" || $.trim($("#from_station").val())=="出发城市"){
					showErrMsg("from_station", "110px", "请输入出发城市！");
					isValid=false;
				}else{
					hideErrMsg("from_station");
				}
			}

			if(arrive_station!=null&&arrive_station!=""){
				$("#arrive_station").val(arrive_station);
			}else{
				if($.trim($("#arrive_station").val())=="" || $.trim($("#arrive_station").val())=="到达城市"){
					showErrMsg("arrive_station", "110px", "请输入到达城市！");
					isValid=false;
				}else{
					hideErrMsg("arrive_station");
				}
			}
	...
	function swapCity(){
		var fromCity = $("#from_station").val().replace("出发城市", "");
		var toCity = $("#arrive_station").val().replace("到达城市", "");
		$("#from_station").val(toCity);
		$("#arrive_station").val(fromCity);
	}
</script>
</body>
</html>
