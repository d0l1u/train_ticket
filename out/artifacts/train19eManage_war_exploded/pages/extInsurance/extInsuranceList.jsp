...
<html>
	<head>
		...
		function exportExcelInsurance() {
			$("form:first").attr("action","/orderForExcel/excelexportExtInsurance.do");
			$("form:first").submit();
			$("form:first").attr("action","/extInsurance/queryInsuranceList.do");
		}
		function selectAll(){
	...
	<body>
		<div class="book_manage oz">
			<form action="/extInsurance/queryInsuranceList.do" method="post" name="myform" id="myform">
					...
					<table>
						...
						<c:forEach var="list" items="${insuranceList}" varStatus="idx">
							...
							>
								...
								<td>
									<a href="/extInsurance/queryInsuranceInfo.do?bx_id=${list.bx_id }&order_id=${list.order_id }">明细</a>
									<c:if test="${list.bx_status != null && list.bx_status == 5}">
										<a href="/extInsurance/updateInsuranceStatusSendAgain.do?bx_id=${list.bx_id }&order_id=${list.order_id }"
											onclick="return confirm('确认重新投保么？')">重新投保</a>
									 </c:if>
									 <c:if test="${list.bx_status != null && list.bx_status eq '2'}">
										<a href="/extInsurance/updateInsuranceStatusNeedCancel.do?bx_id=${list.bx_id }&order_id=${list.order_id }"
