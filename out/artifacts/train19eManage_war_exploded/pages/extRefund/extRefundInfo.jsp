...
<html>
	...
	function sumbitRefund(){
		...
		var url = "/extRefund/queryRefundMoney.do?orderId="+orderId+"&refundMoney="+refundMoney+"&streamId="+streamId+"&version="+new Date();
	...
	function refuseRefund(){
		...
		var pageIndex = $("#pageIndex").val();
			$("#refundForm").attr("action","/extRefund/updateRefuseRefund.do?statusList=" + statusList+ "&pageIndex=" + pageIndex );
			$("#refundForm").submit();
	...
	function sumbitGezhi(refund_status){
		...
		var statusList = $("#statusList").val();
		var pageIndex = $("#pageIndex").val();
			$("#refundForm").attr("action","/extRefund/updateGezhiRefund.do?refund_status="+refund_status+ "&statusList=" + statusList+ "&pageIndex=" + pageIndex );
			$("#refundForm").submit();
		}
	}
</script>
		...
		<div class="content1 oz">
			...
			<div class="left_con oz">
				...
				<c:if test="${refundTicketInfo.refund_type eq '1' || refundTicketInfo.refund_type eq '2' || refundTicketInfo.refund_type eq '3'}">
					<form action="/extRefund/updateRefundTicket.do?
        	...
        	&& refundTicketInfo.refund_status eq '00' }">
        		<form action="/extRefund/updateDifferRefund.do?statusList=${statusList }&pageIndex=${pageIndex }" method="post" name="differForm" id="differForm">
       				<div class="pub_self_take_mes oz mb10_all">
           				<h4>差额退款</h4>
           				<input type="hidden" name="stream_id" value="${stream_id }"/>
						<input type="hidden" name="order_id" value="${order_id }"/>
        			...
        			&& refundTicketInfo.refund_status eq '00' }">
        		<form action="/extRefund/updateOut_Ticket_Refund.do?statusList=${statusList }&pageIndex=${pageIndex }" method="post" name="out_ticketForm" id="out_ticketForm">
       				<div class="pub_self_take_mes oz mb10_all">
           				<h4>出票失败退款</h4>
           				<input type="hidden" name="stream_id" value="${stream_id }"/>
						<input type="hidden" name="order_id" value="${order_id }"/>
