...
<html>
		...
		%>
		    function selectAllrefund_type(){
				...
				if(document.getElementById("controlAllrefund_type").checked){
					for(var i=0; i<checklist.lenXch; i++){
	    	...
	    	function gotoRefundTicketInfo(order_id,cp_id,stream_id){
		    	var url="/xcRefund/queryRfundTicketIsLock.do?stream_id="+stream_id;
					...
					if(data != null && data != ""){
						...
						$("form:first").attr("action","/xcRefund/queryRefundTicketInfo.do?order_id="+order_id + "&cp_id=" + cp_id +"&stream_id="+stream_id+ "&isActive=1&statusList="+channelStr+"&pageIndex="+<%=pageIndex%>);
						$("form:first").submit();
				     }
				    	//window.location="/xcRefund/queryRefundTicketInfo.do?order_id="+order_id+"&cp_id="+cp_id+"&stream_id="+stream_id;
				});
			    
	    	}    
	    	function exportExcelRefund() {
				$("form:first").attr("action","/orderForExcel/excelexportForXcRefund.do");
				$("form:first").submit();
				$("form:first").attr("action","/xcRefund/queryRefundTicketList.do");
			}
	...
	function showdiv(order_id){  
	     ...
	     with (oSon){   
	 		 $.ajax({
				url:"/xcBooking/queryOrderOperHistory.do?order_id="+order_id,
				type: "POST",
				cache: false,
				dataType: "json",
				async: true,
	...
	//机器改签
		function gotoRobotGai(order_id, refund_seq, order_type){
			//ajax验证是否锁
			var url = "/xcRefund/queryPayIsLock.do?order_id="+order_id+ "&refund_seq=" + refund_seq+"&version="+new Date();
			$.get(url,function(data){
				if(data != null && data != ""){
					var temp = data ;
					var str1 = temp.split("&") ;
				 ...
				 var channelStr = "";
					...
					var uri = "/xcRefund/updateOrderstatusToRobotGai.do?order_id="+order_id+"&refund_seq="+refund_seq+"&refund_status=01&statusList="+channelStr+"&version="+new Date();
					$.post(uri,function(data){
						if(data=="yes"){
							$("form:first").attr("action","/xcRefund/queryRefundTicketList.do?pageIndex="+<%=pageIndex%>);
							$("form:first").submit();
						}else{
							alert("机器改签失败");
						}
			...
			$.get(url,function(data){
				if(data != null && data != ""){
				 ...
				 var channelStr = "";
					...
					var uri = "/xcRefund/updateOrderstatusToRobotGai.do?order_id="+order_id+"&refund_seq="+refund_seq+"&refund_status=05&statusList="+channelStr+"&version="+new Date();
					$.post(uri,function(data){
						if(data=="yes"){
							$("form:first").attr("action","/xcRefund/queryRefundTicketList.do?pageIndex="+<%=pageIndex%>);
							$("form:first").submit();
		...
		//ajax验证是否锁
		var url = "/xcRefund/queryPayIsLock.do?order_id="+order_id+"&cp_id="+cp_id+"&version="+new Date();
			$.get(url,function(data){
			if(data != null && data != ""){
				var temp = data ;
				var str1 = temp.split("&") ;
	...
	function updateXcRefundNotifyNum (order_id,notify_id){
				var channelStr = "";
						$("input[name='refund_status']:checkbox:checked").each(function(){ 
							channelStr += $(this).val()+",";//遍历被选中CheckBox元素的集合 得到Value值
						}); 
				channelStr = channelStr.substring(0, channelStr.length-1);
				$("form:first").attr("action","/xcRefund/updateXcRefundNotifyNum.do?order_id="+order_id +"&notify_id="+notify_id +"&statusList="+channelStr+"&pageIndex="+<%=pageIndex%>);
				$("form:first").submit();
			//	$("form:first").attr("action","/xcRefund/queryRefundTicketList.do");
	...
	function uploadAddRefund(){
		$("form:first").attr("action","/xcRefund/uploadAddRefund.do");
		$("form:first").submit();
	}
	    </script>
	    <style>
	...
	<body><div></div>
		<div class="book_manage oz">
			<form action="/xcRefund/queryRefundTicketList.do" method="post" name="myform" id="myform">
				...
				<p>
					...
					<input type="button" value="生成线下退款" class="btn" onclick="location.href = '/xcRefund/toAddRefundPage.do'"/> 
					<input type="button" value="批量车站退票" class="btn" onclick="uploadAddRefund()" />
						<%} %>
		    		<a href="/xcRefund/queryRefundTicketList.do?refund_status=012&refund_status=456">正在处理</a>
				</p>
							...
							<td>
								<span>
									...
									<c:if test="${!empty list.notify_num && list.notify_num eq '10' && list.notify_status!=33  }">
										<a href="javascript:updateXcRefundNotifyNum('${list.order_id}','${list.notify_id  }');">重新通知</a>
									<!--	<a href="/xcRefund/updateXcRefundNotifyNum.do?notify_id=${list.notify_id  }&order_id=${list.order_id }">重新通知</a>  -->
									</c:if>
									...
									<%} %>
									<br/>	<a href="/xcRefund/queryRefundTicketInfo.do?
