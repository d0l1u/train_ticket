...
<html>
	...
	%>
		function exportExcel() {
			...
			$("form:first").attr("action","/elongBook/exportexcel.do");
			$("form:first").submit();
			$("form:first").attr("action","/elongBook/queryBookList.do");
		}
		
		
		
		function queryFailure() {
			$("form:first").attr("action","/elongBook/queryNotifyList.do?notify_status=33");
			$("form:first").submit();
			$("form:first").attr("action","/elongBook/queryBookList.do");
		}
		//通知成功锁 
				...
				if(data != null && data != ""){
					...
					return;
					 ...
					 if(confirm("你确认修改为通知成功吗？")){
					  var url ="/elongBook/notify.do?order_id=" + order_id;
					 	  $.get(url,function(data){
								if(data == "true"){
									$("form:first").submit();
									}
		...
		function gotoOrderLock(order_id){
			...
			var url = "/elongBook/queryOrderIsLock.do?order_id="+order_id+"&version="+new Date();
				...
				if(data != null && data != ""){
					...
					$("form:first").attr("action","/elongBook/bookInfo.do?order_id="+order_id+"&IsLock=1");
					$("form:first").submit();
				 }
			});
		}
	...
	function notifyAgain(order_id){
	 var url ="/elongBook/notifyAgain.do?order_id=" +order_id;
		 	  $.get(url,function(data){
					if(data == "true"){
						$("form:first").submit();
						}
...
margin-left:50px; 
	...
	<div id="center"> 
		...
		<div class="book_manage oz">
			<form action="/elongBook/queryBookList.do" method="post">
