...
%>
<table width="70%" height="25px;" border="0" align="left" cellpadding="0" cellspacing="0"
	style="margin-top: 1px; margin-left: 20px; margin-right: auto">
	<tr>
		<td align="center"><font color="#ff0000"><strong>没有可显示的项目</strong>
		</font></td>
	</tr>
...
<div class="turn2pageWrap">
	<div class="turn2page">
		<ul>
			<li>共计<%=totalPageCount%>页
			</li>
			<li><a href="javascript:gotoPageFirst();">首页</a></li>
			<li><a href="javascript:gotoPagePrev();">上一页</a></li>
			<%
				int start = ((pageIndex - 1) / 4) * 4 + 1;
					...
					if (start >= 5) {
			%>
			<li><a href="javascript:gotoPageIndex(<%=start - 1%>);">...</a>
			</li>
			...
			%>
			<li><a href="javascript:gotoPageIndex(<%=i%>);"
				<%if (i == pageIndex) {%> class="currentPage" <%}%>><%=i%></a></li>
			<%
				}

					if (end < totalPageCount) {
			%>
			<li><a href="javascript:gotoPageIndex(<%=end + 1%>);">...</a></li>
			<%
				}
			%>
			<li><a href="javascript:gotoPageNext();">下一页</a></li>
			<li><a href="javascript:gotoPageLast();">末页</a></li>
		</ul>
	</div>
	<div class="turn2pageInput">
		<span class="pr5">快速跳转</span> <input id="gotoLabel" name="gotoLabel"
			type="text" class="" /> <a href="javascript:gotoButtonClicked();"><img
