...
<script type="text/javascript">
	...
	function onblurOrderId(){
		...
		if(order_id==""){
			...
			var url = "/xcRefund/queryRefundTicketAdd.do?order_id="+order_id+"&version="+new Date();
	...
	function showCpId(order_id){
			$("#cpidAndName").show();	
			$("form:first").attr("action","/xcRefund/toAddRefundPage.do?order_id="+order_id);
			$("form:first").submit();
	}
	...
	function onblurRefundMoney(){
		...
		if(refund_money==""){
			...
			//alert(refundMoney);
			var url = "/xcRefund/checkRefundMoney.do?orderId="+orderId+"&cpId="+cpId+"&refundMoney="+refundMoney+"&version="+new Date();
			$.get(url,function(data){
				if(data == "no" ){
...
<body>
	...
	<form action="/xcRefund/refundTicketAdd.do" method="post" name="updateForm" id="updateForm" onsubmit="return checkSubmit();">
        ...
        <p><input type="button" value="提 交" class="btn" id="btnSubmit" onclick="submitForm()"/>
           <input type="button" value="返 回" class="btn" onclick="javascript:location='/xcRefund/queryRefundPage.do'"/>
           </p>
    </form>
	</div>
</body>
