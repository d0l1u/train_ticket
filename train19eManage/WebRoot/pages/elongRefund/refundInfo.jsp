	...
	<head>
		...
		%>
			...
			function refund(orderid, refundseq, refund_status) {
				...
				//	alert("streamId="+streamId);
					var url = "/elongRefund/queryRefundMoney.do?orderId="+orderId+"&cpId="+cpId+"&refundMoney="+refundMoney+"&streamId="+streamId+"&version="+new Date();
				...
				$("form:first").attr("action","/elongRefund/refund.do?orderid=" + orderid + "&refundseq=" + refundseq + "&refund_status=" + refund_status
			...
			function refuseRefund(orderid, refundseq) {
				...
				var notifyList = $("#notifyList").val();
				var pageIndex = $("#pageIndex").val();
				$("form:first").attr("action","/elongRefund/refuse.do?orderid=" + orderid + "&refundseq=" + refundseq+ 
		...
		function offlineRefund(orderid, refundseq, refund_status,type) {
				...
				//	alert(refundMoney);
					var streamId=$("#streamId").val();
				//	alert("streamId="+streamId);
					var url = "/elongRefund/queryRefundMoney.do?orderId="+orderId+"&cpId="+cpId+"&refundMoney="+refundMoney+"&streamId="+
					...
					return;
						...
						var pageIndex = $("#pageIndex").val();
						var channel = $("#channel").val();
						$("form:first").attr("action","/elongRefund/refund.do?orderid=" + orderid + "&refundseq=" + refundseq + "&refund_status=" 
			...
			function offlineRefuseRefund(orderid, refundseq) {
				...
				var notifyList = $("#notifyList").val();
				var pageIndex = $("#pageIndex").val();
				$("form:first").attr("action","/elongRefund/refuse.do?orderid=" + orderid + "&refundseq=" + refundseq+ "&statusList=" + statusList+ "&notifyList=" + notifyList+ "&pageIndex=" + pageIndex );
				$("form:first").submit();
			}
			//搁置订单
			function gotoGezhi(order_id,refund_seq){
				...
				var uri = "/elongRefund/updateOrderstatusToRobotGai.do?order_id="+order_id+"&refund_seq="+refund_seq+"&refund_status=44&version="+new Date();
				$.post(uri,function(data){
					if(data=="yes"){
						$("form:first").attr("action","/elongRefund/queryRefundTicketList.do?refund_status=03&refund_status=07&refund_status=33&refund_status=72&refund_status=73");
						$("form:first").submit();
					//	window.location="/elongRefund/queryRefundTicketList.do?refund_status=03&refund_status=07&refund_status=33";
		...
		function notifyAgain(stream_id,order_id,cp_id,refund_type){
				...
				var url = "/elongRefund/queryRefundMoney.do?orderId="+order_id+"&cpId="+cp_id+"&refundMoney="+refundMoney+"&streamId="+stream_id+"&version="+new Date();
						$.get(url,function(data){
							if(data == "no" ){
					alert("退款金额不能大于票面金额！");
					return;
					...
					if(confirm("确认重新通知？")){
					$("form:first").attr("action","/elongRefund/notifyAgain.do?stream_id=" + stream_id + "&order_id=" + order_id+"&cp_id="+cp_id
		...
		function deleteOrder(stream_id,order_id,cp_id,refund_seq){
				...
				var pageIndex = $("#pageIndex").val();
				if(confirm("确认删除订单？")){
				$("form:first").attr("action","/elongRefund/deleteOrder.do?stream_id=" + stream_id + "&order_id="+ order_id+"&cp_id="+cp_id 
		...
		<form id="myform" name="myform" method="post">
			<div class="content1 oz">
				...
				<div class="left_con oz">
						...
						<div class="pub_con">
							<table class="pub_table">
								...
								<tr>
									<td>
										渠&nbsp;&nbsp;&nbsp;&nbsp;道：
										<span>${Channel[orderInfo.channel] }</span>
									</td>
									<td>
										出票时间：
					...
					<c:if test="${refundInfo.refund_type eq '11'}">
										...
										<tr>
											...
											<td>
												...
												<select name="refuse_reason" id="refuse_reason">
														...
														<c:choose>
															<c:when test="${reason.key eq refundInfo.channel}">
																	<c:forEach var="rs" items="${reason.value }">
																	<option value="${rs.key}"
																		<c:if test="${rs.key eq refundInfo.refuse_reason }">selected="selected"</c:if>>
																		${rs.value }
																	</option>
																	</c:forEach>
															</c:when>
															<c:when test="${!fn:contains('tongcheng,meituan,301030',refundInfo.channel)}">
