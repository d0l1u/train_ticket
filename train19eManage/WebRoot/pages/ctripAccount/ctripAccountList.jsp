...
<html>
	...
	function updateInfo(ctrip_id){
		var url="/ctripAccount/queryCtripAccountInfo.do?ctrip_id="+ctrip_id;
	...
	function changeStatus(ctrip_id,ctrip_name,ctrip_status){
		var url = "/ctripAccount/updateCtripAccountStatus.do?version="+new Date();
		$.post(url,{ctrip_id:ctrip_id,ctrip_name:ctrip_name,ctrip_status:ctrip_status},function(data){
			if(data=="yes"){
				$("form:first").attr("action","/ctripAccount/queryCtripAccountList.do?pageIndex="+<%=pageIndex%>);
				$("form:first").submit();
			}else{
				alert("修改账号状态失败！");
			}
	...
	<body><div></div>
		<div class="book_manage oz">
			<form  name="queryFrm" action="/ctripAccount/queryCtripAccountList.do" method="post">
				<ul class="ser oz">
					<li>
						登陆名称：
				...
				<p>
					...
					<input type="button" value="添 加"
						onclick="location.href = '/ctripAccount/addCtripAccountPage.do'" class="btn" />
					<input type="button" value="查看日志" onclick="javascript:opt_history();" class="btn" />
					<input type="button" value="出票金额设置" onclick="javascript:amountAreaSet();" class="btn" />
					<input type="button" value="异常账号"
						onclick="location.href = '/ctripAccount/queryOddAccountCtrip.do'" class="btn" />
				</p>
				...
				<br/>
					...
					<table>
						...
						<c:forEach var="list" items="${ctripAccountList}" varStatus="idx">
							...
							<td>
								<span> 
									<a href="javaScript:void(0)" onclick="updateInfo('${list.ctrip_id }');" >修改</a> 
									<c:if test="${list.ctrip_status eq '11' }">
									<a href="javascript:void(0)" onclick="changeStatus('${list.ctrip_id}','${list.ctrip_name}','00')">启用</a>
									</c:if> 
									<c:if test="${list.ctrip_status eq '00' }">
									<a href="javascript:void(0)" onclick="changeStatus('${list.ctrip_id}','${list.ctrip_name}','11')">停用</a>
									</c:if> 
								</span>
							</td>
							</tr>
