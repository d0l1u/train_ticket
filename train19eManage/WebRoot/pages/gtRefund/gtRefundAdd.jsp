...
<script type="text/javascript">
	...
	function onblurOrderId(){
		...
		if(order_id==""){
			...
			var url = "/gtRefund/queryRefundTicketAdd.do?order_id="+order_id+"&version="+new Date();
	...
	function showCpId(order_id){
			...
			$("form:first").attr("action","/gtRefund/toAddRefundPage.do?order_id="+order_id+"&refund_type="+refund_type);
			$("form:first").submit();
	}
	...
	function onblurRefundMoney(){
		...
		if(refund_money==""){
			...
			//alert(refundMoney);
			var url = "/gtRefund/checkRefundMoney.do?orderId="+orderId+"&cpId="+cpId+"&refundMoney="+refundMoney+"&version="+new Date();
			$.get(url,function(data){
				if(data == "no" ){
...
<body>
	...
	<form action="/gtRefund/refundTicketAdd.do" method="post" name="updateForm" id="updateForm" onsubmit="return checkSubmit();">
        <ul class="order_num oz">
            ...
            <li id="cpidAndName" style="display: none;">
            	...
            	<c:if test="${list.cp_id eq cpid }">checked="checked"</c:if> />
			            	...
			            	票价：<input value="${list.buy_money }" type="text" id="AddbuyMoney_${idx.count }" 
            ...
            <li>退款类型&nbsp;&nbsp;：&nbsp;线下退款
            <input type="hidden" value="${refund_type}" id="refund_type" name="refund_type"/>
            </li>
        ...
        <p><input type="button" value="提 交" class="btn" id="btnSubmit" onclick="submitForm()"/>
           <input type="button" value="返 回" class="btn" onclick="javascript:location='/gtRefund/queryRefundPage.do'"/>
           </p>
    </form>
	</div>
</body>
