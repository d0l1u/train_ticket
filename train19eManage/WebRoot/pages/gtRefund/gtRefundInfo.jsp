...
<html>
	...
	function sumbitRefund(){
		...
		var url = "/gtRefund/queryRefundMoney.do?orderId="+orderId+"&refundMoney="+refundMoney+"&streamId="+streamId+"&version="+new Date();
	...
	function refuseRefund(){
		...
		var pageIndex = $("#pageIndex").val();
			$("#refundForm").attr("action","/gtRefund/updateRefuseRefund.do?statusList=" + statusList+ "&pageIndex=" + pageIndex );
			$("#refundForm").submit();
	...
	function sumbitGezhi(refund_status){
		...
		var statusList = $("#statusList").val();
		var pageIndex = $("#pageIndex").val();
			$("#refundForm").attr("action","/gtRefund/updateGezhiRefund.do?refund_status="+refund_status+ "&statusList=" + statusList+ "&pageIndex=" + pageIndex );
			$("#refundForm").submit();
		}
	}
</script>
		...
		<div class="content1 oz">
			...
			<div class="left_con oz">
					...
					<div class="pub_con">
						<table class="pub_table">
							...
							<tr>
								<td width="234">
									代理商姓名：
									<span>${orderInfo.merchant_id}</span>
								</td>
								<td>
									代理商账号：
									<span>${orderInfo.merchant_name}</span>
								</td>
							</tr>
							<tr>
								<td>
				...
				<c:if test="${refundTicketInfo.refund_type eq '1' || refundTicketInfo.refund_type eq '2' || refundTicketInfo.refund_type eq '3'}">
					<form action="/gtRefund/updateRefundTicket.do?
						cp_id=${cp_id }&order_id=${order_id }&stream_id=${stream_id }&refund_seq=${refundTicketInfo.refund_seq }&statusList=${statusList }&pageIndex=${pageIndex }" 
		                	...
		                	<table class="pub_table">
		                        ...
		                        <tr>
		                         ...
		                         <tr>
		                        	...
		                        	<td colspan="2">退订金额：
			                        	<span>
				                        	<c:if test="${refundTicketInfo.refund_status !=null && 
				                        	(refundTicketInfo.refund_status eq '00' || refundTicketInfo.refund_status eq '99' 
				                        	|| refundTicketInfo.refund_status eq '03'|| refundTicketInfo.refund_status eq '07'
				                        	|| refundTicketInfo.refund_status eq '55') }">
			                        	...
			                        	<span>
												<select name="our_remark" style="width: 380px;" id="our_remark">
												    <option value="0" > </option>
														<option value="2001"
															<c:if test="${'2001' eq refundTicketInfo.our_remark }">selected="selected"</c:if>>
																车票过期 或者12306限制的热门车次不能网上退票。
														</option>
														<option value="2002"
															<c:if test="${'2002' eq refundTicketInfo.our_remark }">selected="selected"</c:if>>
																抱歉，退票失败，请前往就近火车站售票窗口办理。
														</option>
														<option value="2005"
															<c:if test="${'2005' eq refundTicketInfo.our_remark }">selected="selected"</c:if>>
																已取票
														</option>
												</select>
										</span>
        	...
        	&& refundTicketInfo.refund_status eq '00' }">
        		<form action="/gtRefund/updateDifferRefund.do?statusList=${statusList }&pageIndex=${pageIndex }" method="post" name="differForm" id="differForm">
       				<div class="pub_self_take_mes oz mb10_all">
           				<h4>差额退款</h4>
           				<input type="hidden" name="stream_id" value="${stream_id }"/>
						<input type="hidden" name="order_id" value="${order_id }"/>
        			...
        			&& refundTicketInfo.refund_status eq '00' }">
        		<form action="/gtRefund/updateOut_Ticket_Refund.do?statusList=${statusList }&pageIndex=${pageIndex }" method="post" name="out_ticketForm" id="out_ticketForm">
       				<div class="pub_self_take_mes oz mb10_all">
           				<h4>出票失败退款</h4>
           				<input type="hidden" name="stream_id" value="${stream_id }"/>
						<input type="hidden" name="order_id" value="${order_id }"/>
						...
						<div class="pub_debook_mes  oz mb10_all">
							<p>
								<c:if test="${empty query_type && refundTicketInfo != null}">
									...
									<c:if test="${refundTicketInfo.refund_status eq '55'}">
