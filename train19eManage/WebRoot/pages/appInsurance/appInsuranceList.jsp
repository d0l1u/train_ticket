...
<html>
	...
	function exportExcelInsurance() {
		$("form:first").attr("action","/orderForExcel/excelexportAppInsurance.do");
		$("form:first").submit();
		$("form:first").attr("action","/appInsurance/queryInsuranceList.do");
	}
	function selectAll(){
	...
	<body>
		<div class="book_manage oz">
			<form action="/appInsurance/queryInsuranceList.do" method="post" name="myform" id="myform">
				<div style="border: 0px solid #00CC00; margin: 10px;">
					...
					<ul class="ser oz" style="margin-top: 10px;">
						<li>
							渠&nbsp;&nbsp;&nbsp;&nbsp;道：
						</li>
					...
					<table>
						...
						<c:forEach var="list" items="${insuranceList}" varStatus="idx">
							<c:choose>
								...
								<td>
									<a href="/appInsurance/queryInsuranceInfo.do?bx_id=${list.bx_id }&order_id=${list.order_id }">明细</a>
									<c:if test="${list.bx_status != null && list.bx_status == 5}">
										<a href="/appInsurance/updateInsuranceStatusSendAgain.do?bx_id=${list.bx_id }&order_id=${list.order_id }"
											onclick="return confirm('确认重新投保么？')">重新投保</a>
									 </c:if>
									 <c:if test="${list.bx_status != null && list.bx_status eq '2'}">
										<a href="/appInsurance/updateInsuranceStatusNeedCancel.do?bx_id=${list.bx_id }&order_id=${list.order_id }"
