...
<html>
	<head>
		...
		function exportExcel() {
			$("form:first").attr("action","/meituanBook/exportrefundexcel.do");
			$("form:first").submit();
			$("form:first").attr("action","/meituanRefund/queryRefundTicketList.do");
		}
		function gotoOrderRefund(order_id,cp_id, refund_seq, order_type){
		//ajax验证是否锁
		var url = "/meituanRefund/queryPayIsLock.do?order_id="+order_id+"&cp_id="+cp_id+ "&refund_seq=" + refund_seq+"&version="+new Date();
			...
			if(data != null && data != ""){
				...
				$("form:first").attr("action","/meituanRefund/ticketInfo.do?order_id="+order_id +"&cp_id="+cp_id+ "&refund_seq=" + refund_seq + "&isActive=1&statusList="+channelStr+ "&notifyList="+ notifyStr+"&pageIndex="+<%=pageIndex%>);
				$("form:first").submit();
			//	$("form:first").attr("action","/meituanRefund/queryRefundTicketList.do");
			 }
	...
	//重新通知
		...
		function showdiv(order_id){  
		     ...
		     with (oSon){   
		 		 $.ajax({
					url:"/meituanRefund/queryOrderOperHistory.do?order_id="+order_id,
					type: "POST",
					cache: false,
					dataType: "json",
					async: true,
		...
		function gotoRobotGai(order_id, cp_id,refund_seq, order_type){
			//ajax验证是否锁
			var url = "/meituanRefund/queryPayIsLock.do?order_id="+order_id+ "&refund_seq=" + refund_seq+"&version="+new Date();
			$.get(url,function(data){
				if(data != null && data != ""){
					var temp = data ;
					var str1 = temp.split("&") ;
					...
					var uri = "/meituanRefund/updateOrderstatusToRobotGai.do?order_id="+order_id+"&cp_id="+cp_id+"&refund_seq="+refund_seq+"&refund_status=01&version="+new Date();
					$.post(uri,function(data){
						if(data=="yes"){
							$("form:first").attr("action","/meituanRefund/queryRefundTicketList.do?pageIndex="+<%=pageIndex%>);
							$("form:first").submit();
						}else{
							alert("机器改签失败");
						}
		...
		function gotoRobotRefund(order_id, cp_id, refund_seq, order_type){
			...
			$.get(url,function(data){
				if(data != null && data != ""){
					...
					var uri = "/meituanRefund/updateOrderstatusToRobotGai.do?order_id="+order_id+"&cp_id="+cp_id+"&refund_seq="+refund_seq+"&refund_status=05&version="+new Date();
					$.post(uri,function(data){
						if(data=="yes"){
							$("form:first").attr("action","/meituanRefund/queryRefundTicketList.do?pageIndex="+<%=pageIndex%>);
							$("form:first").submit();
...
var url = "/remind/queryCount.do?&version ="+new Date();
		...
		$("form:first").attr("action","/meituanRefund/uploadAddRefund.do");
		$("form:first").submit();
		}
...
#hint{width:700px;border:1px solid #C1C1C1;background:#F0FAC1;position:fixed;z-index:9;padding:6px;line-height:17px;text-align:left;overflow:auto;} 
	...
	<body><div></div>
		<div class="book_manage oz">
			<form action="/meituanRefund/queryRefundTicketList.do" method="post">
				...
				<p>
					...
					<%
           			...
           			%>
						...
						<input type="button" value="生成线下退款" class="btn" onclick="location.href = '/meituanRefund/toAddRefundPage.do?refund_type=22'"/>
						<input type="button" value="生成改签退款" class="btn" onclick="location.href = '/meituanRefund/toAddRefundPage.do?refund_type=55'"/>
						<input type="button" value="批量车站退票" class="btn" onclick="uploadAddRefund()" />
					<%} %>
					&nbsp;&nbsp; 	 
					<a href="/meituanRefund/queryRefundTicketList.do?refund_status=81">车站退票</a>&nbsp;&nbsp;	
					...
					<table>
						...
						<c:forEach var="list" items="${refundTicketList}" varStatus="idx">
							...
							<td>
								...
								<br/>
								<span>
								<a href="/meituanRefund/ticketInfo.do?order_id=${list.order_id}&cp_id=${list.cp_id }&refund_seq=${list.refund_seq}&isActive=0" onmouseover="showdiv('${list.order_id}')" onmouseout="hidediv()">明细</a>
								</span>
							</td>
							</tr>
						</c:forEach>
