...
<script type="text/javascript">
	...
	function onblurOrderId(){
		...
		if(order_id==""){
			...
			var url = "/tuniuRefund/queryRefundTicketAdd.do?order_id="+order_id+"&version="+new Date();
	...
	function showCpId(order_id){
			$("#cpidAndName").show();	
			$("form:first").attr("action","/tuniuRefund/toAddRefundPage.do?order_id="+order_id);
			$("form:first").submit();
	}
	...
	function onblurRefundMoney(){
		...
		if(refund_money==""){
			...
			//alert(refundMoney);
			var url = "/tuniuRefund/queryRefundMoney.do?orderId="+orderId+"&cpId="+cpId+"&refundMoney="+refundMoney+"&version="+new Date();
			$.get(url,function(data){
				if(data == "no" ){
...
<body>
	...
	<form action="/tuniuRefund/refundTicketAdd.do" method="post" name="updateForm" id="updateForm" onsubmit="return checkSubmit();">
        <ul class="order_num oz">
            ...
            <li id="cpidAndName" style="display: none;">
            	...
            	<input type="hidden" id="out_ticket_billno"name="out_ticket_billno" value="${list.out_ticket_billno }" 
            	<c:if test="${list.cp_id eq cpid }">checked="checked"</c:if> />
			            	...
			            	12306单号：<input value="${list.out_ticket_billno }" type="text" id="AddbuyMoney_${idx.count }" 
			            	style="color:#f60;width:150px;border:0;"/>
			            	<br/>
							</label>
            	</c:forEach>
        ...
        <p><input type="button" value="提 交" class="btn" id="btnSubmit" onclick="submitForm()"/>
           <input type="button" value="返 回" class="btn" onclick="javascript:location='/tuniuRefund/queryRefundPage.do'"/>
           </p>
    </form>
	</div>
</body>
