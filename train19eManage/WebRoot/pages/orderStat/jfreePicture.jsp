...
<html>
	...
	<body>
			...
			<td>
				<p align="center">
					<c:if test="${status eq 'thisDayEachProvince'}">
						<span> 
							<img src="/jchart/provinceHour/jfreeThisDayEachProvince${area_noCreate_time }.jpg" border=0 usemap="#imgMap" /> 
						</span>
					</c:if>
				</p>
			</td>
		...
		<div class="pub_debook_mes  oz mb10_all">
			...
			<c:if test="${status eq 'thisDay'}">
				<input type="button" value="详情" class="btn btn_normal" 
					 onclick="submitQueryThisDayInfo()" />
			</c:if>
		</div>
	</body>
</html>
