...
<html>
	...
	//重新通知订单
	function gotoNormal(order_id,order_status,create_time){
		//ajax验证是否锁
		var url = "/elongBook/queryOrderIsLock.do?order_id="+order_id+"&version="+new Date();
			...
			if(data != null && data != ""){
				...
				var  uri = "/elongBook/updateGotoNormal.do?order_id="+order_id+"&order_status="+order_status+"&version="+new Date();
				$.post(uri,function(data){
					if(data == "yes"){
						$("form:first").attr("action","/elongBook/queryBookList.do?abnormal=1&pageIndex="+<%=pageIndex%>);
						$("form:first").submit();
					}else{
						alert("重新通知【失败】");
					}
	...
	//预订失败订单
	function gotoFailure(order_id,order_status,create_time){
		//ajax验证是否锁
		var url = "/elongBook/queryOrderIsLock.do?order_id="+order_id+"&version="+new Date();
			$.get(url,function(data){
			if(data != null && data != ""){
				...
				var  uri = "/elongBook/updateGotoFailure.do?order_id="+order_id+"&order_status="+order_status+"&version="+new Date();
				$.post(uri,function(data){
					if(data == "yes"){
						$("form:first").attr("action","/elongBook/queryBookList.do?abnormal=1&pageIndex="+<%=pageIndex%>);
						$("form:first").submit();
...
#hint{width:700px;border:1px solid #C1C1C1;background:#F0FAC1;position:fixed;z-index:9;padding:6px;line-height:17px;text-align:left;overflow:auto;} 
	...
	<body>
		<div class="book_manage oz">
			<form action="/elongBook/queryBookList.do?abnormal=1" method="post" name="queryFrm">
					...
					<table>
						...
						<c:forEach var="list" items="${bookList}" varStatus="idx">
							...
							<td>
								...
								<br/> 
								<c:if test="${ list.order_status eq '33' || list.order_status eq '22'|| list.order_status eq '44' }">
								<a style="color:#3BA0FF;" href="javascript:gotoNormal('${list.order_id}','${list.order_status}','${list.create_time}');">重新通知</a><br/>
								</c:if>
								<a style="color:#FF82AB;" href="javascript:gotoFailure('${list.order_id}','${list.order_status}','${list.create_time}');">预订失败</a>
							</td>
							</tr>
						</c:forEach>
					</table>
