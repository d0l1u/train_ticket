...
<html>
  <head>
	...
	<script type="text/javascript">
		function changeChannel(channel){
			if(confirm("确认切换吗？")){
				$("form:first").attr("action", "/innerSystemSetting/updateInterfaceChannel.do?channelId=" + channel);
				$("form:first").submit();
			}
		}
		function changeConTimeout(conTimeout) {
			...
			if(confirm("确认修改吗?")) {
				$("form:first").attr("action", "/innerSystemSetting/updateConTimeout.do?conTimeoutId=" + conTimeout);
				$("form:first").submit();
			}
		}
		function changeReadTimeout(readTimeout) {
			...
			if(confirm("确认修改吗?")) {
				$("form:first").attr("action", "/innerSystemSetting/updateReadTimeout.do?readTimeoutId=" + readTimeout);
				$("form:first").submit();
			}
		}
		//修改12306旧接口地址
		function changeURL(urlId) {
			...
			if(confirm("确认修改12306接口地址吗?")) {
				$("form:first").attr("action", "/innerSystemSetting/updateInterface12306RUL.do?urlId=" + urlId);
				$("form:first").submit();
			}
		}
		//切换12306旧接口地址
		function changeStatus(urlId) {
			if(confirm("确认切换12306接口地址吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/changeURLStatus.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//删除12306旧接口地址
		function deleteUrl(urlId) {
			if(confirm("确认删除12306接口地址吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/deleteURL.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//增加12306旧接口地址
		function addURL() {
			$("form:first").attr("action","/innerSystemSetting/turnToAddURLPage.do");
			$("form:first").submit();
		}
		//修改12306新接口地址
		function changeNewURL(urlId) {
			...
			if(confirm("确认修改12306新接口地址吗?")) {
				$("form:first").attr("action", "/innerSystemSetting/updateInterface12306NewRUL.do?urlId=" + urlId);
				$("form:first").submit();
			}
		}
		//切换12306新接口地址
		function changeNewStatus(urlId) {
			if(confirm("确认切换12306新接口地址吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/changeNewURLStatus.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//删除12306新接口地址
		function deleteNewUrl(urlId) {
			if(confirm("确认删除12306新接口地址吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/deleteNewURL.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//增加12306新接口地址
		function addNewURL() {
			$("form:first").attr("action","/innerSystemSetting/turnToAddNewURLPage.do");
			$("form:first").submit();
		}
		function changePayCtrl(id){
			if(confirm("确认切换吗？")){
				$("form:first").attr("action", "/innerSystemSetting/updatePayCtrl.do?setting_id=" + id);
				$("form:first").submit();
			}
		}
		function changeBxChannel(id) {
			if(confirm("确认切换保险渠道吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateBxChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeMsgChannel(id) {
			if(confirm("确认切换短信渠道吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateMsgChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeTtlChannel(id){
			if(confirm("确认切换默认乘车日期吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateTtlChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeStaChannel(id){
			...
			if(confirm("确认切换余票阀值吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateStaChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeQtaChannel(id){
			if(confirm("确认修改默认排队等候买票人数吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateQtaChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeQttChannel(id){
			if(confirm("确认修改排队等候时间吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateQttChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeBookRobot(id){
			if(confirm("确认切换预定机器人版本吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateBookRobotChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		...
		function changeSysBook(id) {
			if(confirm("确认切换前台终止预订功能吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/changeSysBook.do?setting_id=" +id);
				$("form:first").submit();
			}
		}

		function opt_rizhi(){
			var url="/innerSystemSetting/querySystemSetList.do";
			showlayer('操作日志',url,'950px','600px')
		}
		//切换打码方式：0人工打码  1机器识别
		function changeCodeType(id){
			if(confirm("确认切换打码方式吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/changeCodeType.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		//切换改退方式：0人工改退  1机器改退
		function changeRefundAndAlert(id){
			if(confirm("确认切换打码方式吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/changeRefundAndAlert.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		...
		function changeBuyTime(id){
			if(confirm("确认切换开车前购票时间吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateBuytime.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeRefundTime(id){
			if(confirm("确认切换开车后退票时间吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateRefundtime.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
			function changeBeforeRefundTime(id){
			if(confirm("确认切换开车后退票时间吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateBeforeRefundtime.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		...
		function changeRapnChannel(id){
			if(confirm("确认修改预订机器人一次处理订单量吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateRapnChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		...
		function changeBdnChannel(id){
			if(confirm("确认修改预约购票天数吗？")) {
				$("form:first").attr("action", "/innerSystemSetting/updateBdnChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}	
	</script>
  ...
  <body>
  	...
  	<form action="/innerSystemSetting/getSystemSetting.do" method="post">
	  	...
	  	<table>
		    ...
		    <c:set value="ss" scope="request" var="ss"/>
		    	...
		    	<c:if test="${ss.setting_name eq 'inner_book_day_num'}">
		    		<tr>
		    			<td>预购天数：</td>
		    			<td>
