...
<html>
	...
	function sumbitRefund(){
		...
		var url = "/innerRefund/queryRefundMoney.do?orderId="+orderId+"&refundMoney="+refundMoney+"&streamId="+streamId+"&version="+new Date();
	...
	function refuseRefund(){
		...
		//if(confirm("确认拒绝退款？并且在备注中说明拒绝原因。")){
			...
			$("#refundForm").attr("action","/innerRefund/updateRefuseRefund.do?statusList=" + statusList+ "&pageIndex=" + pageIndex );
			$("#refundForm").submit();
	...
	function offlineRefuseRefund() {
		var refuse = confirm("确认拒绝退款?");
		if(!refuse)return;
		var statusList = $("#statusList").val();
		var pageIndex = $("#pageIndex").val();
		$("#offlineForm").attr("action","/innerRefund/updateRefuseRefund.do?statusList=" + statusList+ "&pageIndex=" + pageIndex );
		$("#offlineForm").submit();
	}
	...
	<body>
		<div class="content1 oz">
			...
			<div class="left_con oz">
					...
					<div class="pub_con">
						<table class="pub_table">
							...
							<!-- 	<td>
								...
								<td>
									渠&nbsp;&nbsp;&nbsp;&nbsp;道：
									<span>${innerChannels[orderInfo.inner_channel]}</span>
								</td>
							</tr>
					...
					<div class="pub_con">
						<table class="pub_table">
							...
							<tr>
								...
								<td>
									配 送：
									<span style="font-weight: bold; color: red;">${orderInfo.ps_pay_money}
									</span> 元
								</td>
							</tr>
							<tr>
				...
				<!-- 退款流程开始 -->
				<c:if test="${refundTicketInfo.refund_type eq '1'}">
					<form action="/innerRefund/updateRefundTicket.do?
						cp_id=${cp_id }&order_id=${order_id }&stream_id=${stream_id }&statusList=${statusList }&pageIndex=${pageIndex }" 
        	...
        	&& refundTicketInfo.refund_status eq '00' }">
        		<form action="/innerRefund/updateDifferRefund.do?statusList=${statusList }&pageIndex=${pageIndex }" method="post" name="differForm" id="differForm">
       				<div class="pub_self_take_mes oz mb10_all">
           				<h4>差额退款</h4>
           				<input type="hidden" name="stream_id" value="${stream_id }"/>
						<input type="hidden" name="order_id" value="${order_id }"/>
        			...
        			&& refundTicketInfo.refund_status eq '00' }">
        		<form action="/innerRefund/updateOut_Ticket_Refund.do?statusList=${statusList }&pageIndex=${pageIndex }"  method="post" name="out_ticketForm" id="out_ticketForm">
       				<div class="pub_self_take_mes oz mb10_all">
           				<h4>出票失败退款</h4>
           				<input type="hidden" name="stream_id" value="${stream_id }"/>
						<input type="hidden" name="order_id" value="${order_id }"/>
        	...
        	<c:if test="${refundTicketInfo.refund_type != null && refundTicketInfo.refund_type eq '3'
        		...
        		<!-- 线下退款 -->
        		<c:if test="${refundTicketInfo.refund_type != null && refundTicketInfo.refund_type eq '6' 
        		&& fn:contains('77,55', refundTicketInfo.refund_status)}">
	        	<div class="pub_self_take_mes oz mb10_all">
	           				<h4>线下退款</h4>
	               			<div class="pub_con">
	               				<table class="pub_table">
        		...
        		<c:if test="${refundTicketInfo.refund_type != null && refundTicketInfo.refund_type eq '6'
        	&& refundTicketInfo.refund_status eq '66'}">
        		<form action="/innerRefund/updateOfflineRefund.do?statusList=${statusList }&pageIndex=${pageIndex }" method="post" name="offlineForm" id="offlineForm">
       				<div class="pub_self_take_mes oz mb10_all">
           				<h4>线下退款</h4>
           				<input type="hidden" name="stream_id" value="${stream_id }"/>
						<input type="hidden" name="order_id" value="${order_id }"/>
               			...
               			<div class="pub_con">
               				<table class="pub_table">
               					...
               					<tr>
               						...
               						<td>
               							<span>12306退款：
               								<input type="text" name="actual_refund_money" id="actual_refund_money_id" value="${refundTicketInfo.actual_refund_money}" />
               							</span>
               						</td>
               						<td>备注：
			                        	<span>
				                        	<c:if test="${refundTicketInfo.refund_status !=null && refundTicketInfo.refund_status eq '66'}">
				                        		<input type="text" name="our_remark" id="our_remark" value="${refundTicketInfo.our_remark }"/>
				                        	</c:if>
				                        	<c:if test="${fn:contains('77', refundTicketInfo.refund_status)  }">
				                        			${refundTicketInfo.our_remark }
				                        	</c:if>
			                        	</span>
		                        	</td>
               					</tr>
	               				<%--<c:if test="${refundInfo.refund_status eq '66' ||refundInfo.refund_status eq '77'}">
	               					<tr>
	               						<td class="pub_yuliu"><br /></td>
	               						<td>
						...
						<div class="pub_debook_mes  oz mb10_all">
							<p>
								...
								<c:if test="${empty query_type && refundTicketInfo != null}">
									<c:if test="${refundTicketInfo.refund_status eq '00' && refundTicketInfo.refund_type eq '1'}">
									...
									<c:if test="${refundTicketInfo.refund_status eq '00' && refundTicketInfo.refund_type eq '3' }">
				   					...
				   					<c:if test="${refundTicketInfo.refund_status eq '66' && refundTicketInfo.refund_type eq '6'}">
				   						<input type="button" value="拒绝退款" class="btn"
											onclick="offlineRefuseRefund();" />
										<input type="button" value="线下退款" class="btn" id="offlineSubmit"
											onclick="offlineRefund();" />
