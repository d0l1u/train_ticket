...
<head>
		...
		function importExcel(){
			...
			if(confirm("确定上传？")){
			$("form:first").attr("action","/xcStation/xcAddRefunds.do?file="+file);
			$("form:first").submit();
			}
		}

		function goback(){
			$("form:first").attr("action","/xcStation/queryXcStationPage.do");
			$("form:first").submit();
		}
...
<body>
	<form action="/xcStation/xcAddRefunds.do" method="post" enctype="multipart/form-data" id="addForm" onsubmit="return checkSubmit();">
