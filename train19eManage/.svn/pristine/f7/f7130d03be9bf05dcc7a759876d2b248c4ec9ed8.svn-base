...
<html>
		...
		function notify(order_id) {
		 if(confirm("你确认修改为通知成功吗？")){
			  ...
			  $.get(url,function(data){
					if(data == "true"){
						$("form:first").attr("action","/elongBook/queryBookList.do?notify_status=33");
						$("form:first").submit();
						}
					});
			 }
	...
	<body>
		<div class="content1 oz">
			<!--左边内容 start-->
			<form action="/elongBook/queryBookList.do" method="post" id="updateForm">
					...
					<div class="pub_con">
						<table class="pub_table">
							...
							<tr>
								<td>
									渠道:
									<span>${Channel[bookVo.channel] }</span>
								</td>
								<td>
									12306单号：
