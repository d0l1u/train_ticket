...
<html>
	...
	function submitForm(){
		...
		$("form:first").attr("action","/xcStation/queryXcStationInfo.do?refund_seq="+refund_seq+"&begin_time="+begin_time);
		$("form:first").submit();
		}
	...
	function exportExcel() {
		var refund_seq = $("#refund_seq").val();
		var begin_time = $("#begin_time").val();
			$("form:first").attr("action","/xcStation/exportXcStationexcel.do?refund_seq="+refund_seq+"&begin_time="+begin_time);
			$("form:first").submit();
			$("form:first").attr("action","/xcStation/queryXcStationInfo.do");
		}
</script>
</head>
	<body onload="div();"><div></div>
		<div class="book_manage oz">
			<form action="/xcStation/queryXcStationInfo.do" method="post">
		...
		<%} %>
		<input type="button" value="返 回" class="btn" onclick="javascript:window.location.href='/xcStation/queryXcStationPage.do';"/>
        </p>
				...
				<c:if test="${!empty isShowList}">
					<table>
						...
						<c:forEach var="list" items="${xcStationList}" varStatus="idx">
