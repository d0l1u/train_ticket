...
<script type="text/javascript">
	...
	function onblurOrderId(){
		...
		if(order_id==""){
			...
			var url = "/elongRefund/queryRefundTicketAdd.do?order_id="+order_id+"&version="+new Date();
	...
	function showCpId(order_id){
			$("#cpidAndName").show();	
			$("form:first").attr("action","/elongRefund/toAddRefundPage.do?order_id="+order_id);
			$("form:first").submit();
	}
	...
	function onblurRefundMoney(){
		...
		if(refund_money==""){
			...
			//alert(refundMoney);
			var url = "/elongRefund/queryRefundMoney.do?orderId="+orderId+"&cpId="+cpId+"&refundMoney="+refundMoney+"&refundType="+refundType+"&version="+new Date();
			$.get(url,function(data){
				if(data == "no" ){
				...
				$("#refund_money_info").text("改签单不能线下退票!").css("color","#f00");
				RefundMoney = false;
				$("#btnSubmit").attr("disabled", true);
				// document.getElementById('alterSubmit').style.display = "block";
				}else{
...
<body>
	...
	<form action="/elongRefund/refundTicketAdd.do" method="post" name="updateForm" id="updateForm" onsubmit="return checkSubmit();">
        <ul class="order_num oz">
            ...
            <li id="cpidAndName" style="display: none;">
            	...
            	<c:if test="${list.cp_id eq cpid }">checked="checked"</c:if> />
			            	...
			            	渠道：
			            	<c:if test="${list.channel eq 'elong' }">
			            	<input value="艺龙" type="text" id="AddChannel_${idx.count }" 
			            	style="color:#f60;width:50px;border:0;"/></c:if> 
			            	<c:if test="${list.channel eq 'tongcheng' }">
			            	<input value="同程" type="text" id="AddChannel_${idx.count }" 
			            	style="color:#f60;width:50px;border:0;"/></c:if> 
			            	<br/>
							</label>
            	</c:forEach>
        ...
        <p><input type="button" value="提 交" class="btn" id="btnSubmit" onclick="submitForm()"/>
           <input type="button" value="返 回" class="btn" onclick="javascript:location='/elongRefund/queryRefundTicketPage.do'"/>
           <!-- 
