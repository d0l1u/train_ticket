...
<head>
		...
		function importExcel(){
			...
			if(confirm("确认批量导入？")){
			$("form:first").attr("action","/tuniuRefund/addAllRefundStation.do?file="+file);
			$("form:first").submit();
			}
		...
		function goback(){
			$("form:first").attr("action","/tuniuRefund/queryRefundPage.do");
			$("form:first").submit();
		}
...
<body>
	<form action="/tuniuRefund/queryRefundPage.do" method="post" enctype="multipart/form-data" id="addForm" onsubmit="return checkSubmit();">
