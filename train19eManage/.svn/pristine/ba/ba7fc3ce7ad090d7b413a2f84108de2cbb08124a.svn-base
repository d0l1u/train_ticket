...
<html>
	...
	function exportExcel() {
		$("form:first").attr("action","/orderForExcel/excelExportExtBooking.do");
		$("form:first").submit();
		$("form:first").attr("action","/extBooking/queryExtBookList.do");
	}
	$().ready(function(){
...
//鼠标悬浮于“明细”，显示该订单的操作日志
	...
	function showdiv(order_id){  
	     ...
	     with (oSon){   
	 		 $.ajax({
				url:"/extBooking/queryOrderOperHistory.do?order_id="+order_id,
				type: "POST",
				cache: false,
				dataType: "json",
				async: true,
...
#hint{width:700px;border:1px solid #C1C1C1;background:#F0FAC1;position:fixed;z-index:9;padding:6px;line-height:17px;text-align:left;overflow:auto;} 
	...
	<body><div></div>
		<div class="book_manage oz">
			<form action="/extBooking/queryExtBookList.do" method="post">
				...
				<ul class="order_state oz">
					...
					<ul class="ser order_num oz">
						<li>
						合作商户：&nbsp;&nbsp;
						<input type="checkbox" onclick="selectAllMerchant()" name="controlAllMerchant"
						 style="controlAllMerchant" id="controlAllMerchant"/><label for="controlAllMerchant">
						 &nbsp;全部&nbsp;&nbsp;</label>
				...
				<c:if test="${!empty isShowList}">
					<table>
						...
						<c:forEach var="list" items="${bookList}" varStatus="idx">
							...
							<td>
								<span><a href="/extBooking/extQueryBookInfo.do?order_id=${list.order_id}"  onmouseover="showdiv('${list.order_id}')" onmouseout="hidediv()" >明细</a>
								</span>
							</td>
							</tr>
						</c:forEach>
