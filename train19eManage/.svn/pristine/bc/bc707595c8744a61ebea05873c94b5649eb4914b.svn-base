...
<html>
	<head>
		...
		%>
			...
			function addtrain_return_optlog() {
				...
				var url = "/trainSystemSetting/queryreturn_optlog_id.do?return_id="+return_id+"&version="+new Date();
				$.get(url,function(data){
					if(data == "yes"){
						$("form:first").attr("action","/trainSystemSetting/addtrain_return_optlog.do?pageIndex="+<%=pageIndex%>);
						$("form:first").submit();
					}else{
						alert("添加编号已存在，请重新输入！");
					}
			...
			function updatetrain_return_optlog(return_id) {
				$("form:first").attr("action","/trainSystemSetting/updatetrain_return_optlog.do?return_id="+return_id+"&pageIndex="+<%=pageIndex%>);
				$("form:first").submit();
			}
			 //删除返回日志
			function deletetrain_return_optlog(return_id) {
			if(confirm("确认删除返回日志吗?")) {
				$("form:first").attr("action","/trainSystemSetting/deletetrain_return_optlog.do?return_id="+return_id+"&pageIndex="+<%=pageIndex%>);
				$("form:first").submit();
				}
			}
			//查看操作日志
			...
			//返回
			function goback(){
				$("form:first").attr("action","/trainSystemSetting/getSystemSetting.do");
				$("form:first").submit();
			}
	    </script>
	    <style>
	...
	<body>
		<div>
			<form action="/trainSystemSetting/gototrain_return_optlog.do" method="post" name="myform" id="myform">
			<div class="train_returnlog">
				<c:if test="${!empty isShowList}">
					...
					<div id="divcon" style="display:none;margin-left: 50px;margin-top: 20px;">
					<table>
						<tr>
							<td>编号：</td>
