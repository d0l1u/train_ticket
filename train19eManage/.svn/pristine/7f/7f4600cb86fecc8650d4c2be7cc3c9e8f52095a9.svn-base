...
<html>
  <head>
	...
	<script type="text/javascript">
		function changeChannel(channel){
			if(confirm("确认切换吗？")){
				$("form:first").attr("action", "/extSystemSetting/updateInterfaceChannel.do?channelId=" + channel);
				$("form:first").submit();
			}
		}
		function changeConTimeout(conTimeout) {
			...
			if(confirm("确认修改吗?")) {
				$("form:first").attr("action", "/extSystemSetting/updateConTimeout.do?conTimeoutId=" + conTimeout);
				$("form:first").submit();
			}
		}
		function changeReadTimeout(readTimeout) {
			...
			if(confirm("确认修改吗?")) {
				$("form:first").attr("action", "/extSystemSetting/updateReadTimeout.do?readTimeoutId=" + readTimeout);
				$("form:first").submit();
			}
		}
		//修改12306旧接口地址
		function changeURL(urlId) {
			...
			if(confirm("确认修改12306接口地址吗?")) {
				$("form:first").attr("action", "/extSystemSetting/updateInterface12306RUL.do?urlId=" + urlId);
				$("form:first").submit();
			}
		}
		//切换12306旧接口地址
		function changeStatus(urlId) {
			if(confirm("确认切换12306接口地址吗？")) {
				$("form:first").attr("action", "/extSystemSetting/changeURLStatus.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//删除12306旧接口地址
		function deleteUrl(urlId) {
			if(confirm("确认删除12306接口地址吗？")) {
				$("form:first").attr("action", "/extSystemSetting/deleteURL.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//增加12306旧接口地址
		function addURL() {
			$("form:first").attr("action","/extSystemSetting/turnToAddURLPage.do");
			$("form:first").submit();
		}
		//修改12306新接口地址
		function changeNewURL(urlId) {
			...
			if(confirm("确认修改12306新接口地址吗?")) {
				$("form:first").attr("action", "/extSystemSetting/updateInterface12306NewRUL.do?urlId=" + urlId);
				$("form:first").submit();
			}
		}
		//切换12306新接口地址
		function changeNewStatus(urlId) {
			if(confirm("确认切换12306新接口地址吗？")) {
				$("form:first").attr("action", "/extSystemSetting/changeNewURLStatus.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//删除12306新接口地址
		function deleteNewUrl(urlId) {
			if(confirm("确认删除12306新接口地址吗？")) {
				$("form:first").attr("action", "/extSystemSetting/deleteNewURL.do?urlId=" +urlId);
				$("form:first").submit();
			}
		}
		//增加12306新接口地址
		function addNewURL() {
			$("form:first").attr("action","/extSystemSetting/turnToAddNewURLPage.do");
			$("form:first").submit();
		}
		function changePayCtrl(id){
			if(confirm("确认切换吗？")){
				$("form:first").attr("action", "/extSystemSetting/updatePayCtrl.do?setting_id=" + id);
				$("form:first").submit();
			}
		}
		function changeBxChannel(id) {
			if(confirm("确认切换保险渠道吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateBxChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeMsgChannel(id) {
			if(confirm("确认切换短信渠道吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateMsgChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeTtlChannel(id){
			if(confirm("确认切换默认乘车日期吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateTtlChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeStaChannel(id){
			...
			if(confirm("确认切换余票阀值吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateStaChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeQtaChannel(id){
			if(confirm("确认修改默认排队等候买票人数吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateQtaChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeQttChannel(id){
			if(confirm("确认修改排队等候时间吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateQttChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeBookRobot(id){
			if(confirm("确认切换预定机器人版本吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateBookRobotChannel.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		...
		function changeSysBook(id) {
			if(confirm("确认切换前台终止预订功能吗？")) {
				$("form:first").attr("action", "/extSystemSetting/changeSysBook.do?setting_id=" +id);
				$("form:first").submit();
			}
		}

		function opt_rizhi(){
			var url="/extSystemSetting/querySystemSetList.do";
			showlayer('操作日志',url,'950px','600px')
		}
		//切换打码方式：0人工打码  1机器识别
		function changeCodeType(id){
			if(confirm("确认切换打码方式吗？")) {
				$("form:first").attr("action", "/extSystemSetting/changeCodeType.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		...
		function changeBuyTime(id){
			if(confirm("确认切换开车前购票时间吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateBuytime.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeRefundTime(id){
			if(confirm("确认切换开车后退票时间吗？")) {
				$("form:first").attr("action", "/extSystemSetting/updateRefundtime.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
		function changeNoticeById(id){
			if(confirm("确认切换通知吗？")){
				$("form:first").attr("action", "/extSystemSetting/updateNoticeSetting.do?setting_id=" +id);
				$("form:first").submit();
			}
		}
	</script>
  ...
  <body>
  	...
  	<form action="/extSystemSetting/getSystemSetting.do" method="post">
	  	...
	  	<table>
		    ...
		    <c:set value="ss" scope="request" var="ss"/>
		    	...
		    	<c:if test="${ss.setting_name eq 'out_notify'}">
		    		<tr>
							<td>
								通知：
							</td>
							<td>
								<input type="radio" name="notice_value"
									<c:if test="${ss.setting_value eq '1'}">
			    					checked
			    				</c:if>
									value="1" />
							...
							<td>
		    				<input type="button" class="btn" value="切换" onclick="changeNoticeById('${ss.setting_id}')"/>
		    			</td>	
					</tr>
				</c:if>
		    </c:forEach>
