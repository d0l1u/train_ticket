...
<head>
		...
		function importExcel(){
			...
			if(confirm("确定上传？")){
			$("form:first").attr("action","/elongStation/elongAddRefunds.do?file="+file);
			$("form:first").submit();
			}
		}

		function goback(){
			$("form:first").attr("action","/elongStation/queryElongStationPage.do");
			$("form:first").submit();
		}
...
<body>
	<form action="/elongStation/elongAddRefunds.do" method="post" enctype="multipart/form-data" id="addForm" onsubmit="return checkSubmit();">
