...
public class MobileMsgUtilNew {
	
	private static final Logger logger = Logger.getLogger(MobileMsgUtilNew.class);
	...
	public void send(String phoneNo, String content,String channel){
		//短信渠道：00、19e；11、鼎鑫移动；22、企信通; 88、商户自发短信
//		String channel = commonService.querySysSettingByKey("mobile_msg_channel");
		/*
		if(StringUtils.isEmpty(channel)||"00".equals(channel)){
			sendBy19e(phoneNo, content);
			
		}else if("11".equals(channel)){
			sendByDxyd(phoneNo, content);
		}else if("22".equals(channel)){
			sendByQxt(phoneNo, content);
		}else if("88".equals(channel)){
	...
	public boolean sendBy19e(String phoneNo, String content){
		...
		String result = HttpUtilNew.sendByGet(sendData.toString(), strEncoding, "30000", "30000");
	...
	public boolean sendByDxyd(String phoneNo, String content){
		...
		String result = HttpUtilNew.sendByGet(sendData.toString(), "GBK", "30000", "30000");
		if(!StringUtils.isEmpty(result)){
