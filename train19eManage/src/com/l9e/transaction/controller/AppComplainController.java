...
@Controller
@RequestMapping("/appComplain")
public class AppComplainController extends BaseController {

	private static final Logger logger = Logger.getLogger(AppComplainController.class);
...
//					area_Map=appComplainService.querySupervise_nameToArea_no(Supervise_name_List.get(i).toString());
...
//			}
			request.setAttribute("questionType", ComplainVo.getAppQuestionType());
			return "appComplain/appComplainList";
		}
...
//			paramMap.put("borough_id", this.getParam(request, "borough_id"));
			...
			int totalCount = appComplainService.queryComplainListCount(paramMap);//总条数	
			//分页
			PageVo page = PageUtil.getInstance().paging(request, 20, totalCount);
			paramMap.put("everyPagefrom", page.getFirstResultIndex());//每页开始的序号
			paramMap.put("pageSize", page.getEveryPageRecordCount());//每页显示的条数
			
			List<Map<String, String>> complainList = appComplainService.queryComplainList(paramMap);
...
//			request.setAttribute("district", complainService.getArea(city_id)); 
			...
			// 统计
			List<Map<String,String>> complainStatCount = appComplainService.queryComplainStatCount();
			...
			for(int i = 0; i < complainStatCount.size(); i++){
				...
				if(complainStatCount.get(i).get("question_type")!= null &&
						complainStatCount.get(i).get("question_type").equals("00")){
					order_question++;
				}
//				if(complainStatCount.get(i).get("question_type")!= null &&
...
//				}
				if(complainStatCount.get(i).get("question_type")!= null && 
						complainStatCount.get(i).get("question_type").equals("11")){
					acquire_question++;
				}
				if(complainStatCount.get(i).get("question_type")!= null && 
						complainStatCount.get(i).get("question_type").equals("22")){
					operation_advice++;
				}
				if(complainStatCount.get(i).get("question_type")!= null && 
						complainStatCount.get(i).get("question_type").equals("55")){
					other_advice++;
				}
		...
		public String queryComplainInfo(HttpServletRequest request,
				HttpServletResponse response){
			String complain_id = this.getParam(request, "complain_id");
			
			Map<String, String> complainInfo = appComplainService.queryComplainParticularInfo(complain_id) ;
			request.setAttribute("complainInfo", complainInfo);
			request.setAttribute("questionType", ComplainVo.getAppQuestionType());
			request.setAttribute("purview", ComplainVo.getPurview()) ;
			
			return "appComplain/appComplainInfo";
		}
		/**
		 * 修改详细信息
		 * @param request
		...
		public String updateComplainInfo(ComplainVo complain,HttpServletRequest request,
			...
			appComplainService.updateComplainInfo(complain) ;
		
			return "redirect:/appComplain/queryComplainList.do";
		}
		/**
		 *删除投诉信息
		 *@param request
		...
		public String deleteComplain(HttpServletRequest request,
				HttpServletResponse response ){
			String complain_id = this.getParam(request, "complain_id") ;
			appComplainService.deleteComplain(complain_id) ;
