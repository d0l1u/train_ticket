...
public class ElongAlterController extends BaseController {
	...
	public String getAlterTicketPage(HttpServletRequest request,
...
//		String change_diff_money = this.getParam(request, "change_diff_money");
//		String change_refund_money = this.getParam(request, "change_refund_money");
//		String change_receive_money = this.getParam(request, "change_receive_money");
		...
		for(int i=0; i<cpInfo.size(); i++){
				String cp_id = cpInfo.get(i).get("cp_id");
					...
					if(change_train_box.trim().length()>0 || change_seat_no.trim().length()>0 
							|| change_seat_type.trim().length()>0 || change_buy_money.trim().length()>0 ){
...
//		paramMap.put("change_receive_money", change_receive_money);
	...
	public void updateRobotAlter(HttpServletResponse response,HttpServletRequest request) {
		...
		if (StringUtil.isNotEmpty(order_id)||StringUtil.isNotEmpty(change_id)||StringUtil.isNotEmpty(change_status)) {
			...
			if(Double.parseDouble(String.valueOf(cpInfo.get(0).get("buy_money")))<Double.parseDouble(String.valueOf(cpInfo.get(0).get("change_buy_money")))){
				//moneyType="22";
