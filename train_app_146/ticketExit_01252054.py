...
g_url="http://101.200.80.146:17080/so/soCall.htm?"
g_dll_url = "http://101.200.80.146:7777"
...
def unwrap_resp_data(_data, is_need_decrypt):
    if not is_need_decrypt:
        ...
        return requests.session().post(url=url, data=json.dumps(data), headers=headers,verify=False).content
        # return paraCryptexit.decode(_data)
#
def wrap_req_data(_data):
    data = {"data": _data}
    ...
    print url
    return bytearray(requests.session().post(url=url, data=json.dumps(data), headers=headers, verify=False).content)
    # return paraCryptexit.encode(_data)
    ...
    for ind in range(0,len(name_array)):
        ...
        if(not checkRet):
            ...
            if not addRet:
                ...
                result['status'] = '2'
                result['message'] = "添加联系人失败:%s"%(name_array[ind])
                return result
