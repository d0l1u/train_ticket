...
//		orderInfo.setUser_name(user_name);
		...
		return "redirect:/chunqiu/order/orderComfirm.jhtml?order_id=" + order_id
	...
	private void groupData(BookInfo bookInfo, OrderInfo orderInfo, OrderInfoPs orderInfoPs,
		...
		String order_id = CreateIDUtil.createID("CHQ");
		//订单
	...
	public String orderCmpay(HttpServletRequest request, HttpServletResponse response) {
		...
		param.append("&page_type=").append("OUTER");
		...
		payInfoMap.put("pay_channel", "ccb");
		orderService.updateOrderPayNo(payInfoMap);
		logger.info("pay url:"+req_url+param.toString());
	...
	public String orderResult(HttpServletRequest request, HttpServletResponse response) {
		...
		if(hmac_param.equals(hmac)){
			return "redirect:/chunqiu/query/queryOrderDetail.jhtml?fromFunc=payNotify&order_id=" + order_id;
		}else{
...
//		.append("&arrive_station=").append(paramMap.get("to_city")).append("&channel=").append("chq");
		try {
			param.append("travel_time=").append(orderInfo.getTravel_time()).append("&from_station=").append(URLEncoder.encode(paramMap.get("from_city"),"UTF-8"))
			.append("&arrive_station=").append(URLEncoder.encode(paramMap.get("to_city"),"UTF-8")).append("&channel=").append("inner");
		} catch (UnsupportedEncodingException e1) {
			e1.printStackTrace();
		}
		try{
			
			String jsonStr = HttpUtil.sendByPost(getSysSettingValue("query_left_ticket_url","query_left_ticket_url"),param.toString(), "utf-8");//调用接口
