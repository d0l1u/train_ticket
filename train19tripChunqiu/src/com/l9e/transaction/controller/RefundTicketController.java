...
public class RefundTicketController extends BaseController {
	...
	public String refunding(RefundVo refund, MultipartHttpServletRequest request,
		...
		logger.info("[用户主动发起退款]order_id="+refund.getOrder_id());
		//9点-21点可以
		...
		if(currentDate.before(begin) || currentDate.after(end)){
			...
			return "redirect:/chunqiucommon/goToErrPage.jhtml?errMsg="+errMsg;
		}
	...
	public String refund(MultipartHttpServletRequest request,
		...
		if(null==request.getSession().getAttribute("sessionToken") 
			...
			return "redirect:/chunqiuquery/queryOrderList.jhtml";
		}else{
		...
		if(currentDate.before(begin) || currentDate.after(end)){
			...
			errMsg = URLEncoder.encode(errMsg, "UTF-8");
			return "redirect:/chunqiu/common/goToErrPage.jhtml?errMsg="+errMsg;
		}
...
//			}
	 ...
	 * @return
	 */
	@RequestMapping("/requestRefund_no.jhtml")
	public String requestRefund(MultipartHttpServletRequest request,
			HttpServletResponse response) throws IOException {
