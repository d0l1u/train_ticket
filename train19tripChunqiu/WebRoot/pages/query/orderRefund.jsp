	...
	function sumbitRefund(){
		...
		if(confirm("您是否确认对勾选的"+count+"张车票进行退票？")){
			...
			var dialog = art.dialog({
			    ...
			    content: '您的退票申请已提交，我们会在30分钟内处理，可在我的订单退款状态页面查看退票处理结果。'
			});
			setTimeout(function(){
				dialog.close();
				$(".aui_titleBar").hide();
				$("#refundForm").attr("action", "/chunqiu/refund/refund.jhtml?cp_id_str="+cp_id_str);
				$("#refundForm").submit();
				},
				5000
			); 
...
<body>
	<div class="content oz">
    	...
    	<div class="left_con oz">
            ...
            <form action="/chunqiu/refund/refund.jhtml" method="post" name="refundForm" id="refundForm"
            ...
            <div class="pub_passager_mes oz mb10_all">
            	<div class="oz">
                    <h4 class="fl">车票信息</h4>
                    <p class="tit_tip">（每张票收取5%-20%的退票费、最低2元、保险不退）<input type="checkbox" id="all_chk"/><label for="all_chk" style="color:black;">全选</label></p>
                </div>
                <div class="pub_con">
            	<c:forEach var="detailInfo" items="${detailList}" varStatus="idx">
                	...
                	<table class="pub_table add_bg1">
                    	<tr>
                        	...
                        	<td width="300"><span class="chk_area"><input type="checkbox" class="chk_refund" id="chk_${detailInfo.cp_id}" value="${detailInfo.cp_id}" /></span>
	                        ...
	                        <c:choose>
	                        	<c:when test="${detailInfo.refund_status eq '55'}">
		                        	...
		                        	<tr>
			                    		...
			                    		<td><div id="reason_${detailInfo.cp_id}" style="display:none;">退票金额：<span style="font-weight: bold;color: red;font-size: 20px;">￥${detailInfo.cp_refund_money}</span></div></td>
			                    	</tr>
		                        	<!-- 
			                        <tr>
            ...
            <div class="pub_order_mes oz mb10_all">
          	...
          	<div class="vertified_btn oz">
            	 <p class="tijiao">
            	 	...
            	 	<input type="button" value="返 回" class="btn btn2" onclick="window.location='/chunqiu/query/queryOrderList.jhtml'" />
            	 </p>
            </div>
            <!--业务所有标注 start-->
