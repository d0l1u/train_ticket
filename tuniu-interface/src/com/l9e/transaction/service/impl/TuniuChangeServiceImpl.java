...
public class TuniuChangeServiceImpl extends TuniuCommonServiceImpl implements TuniuChangeService {
	...
	public Result addChange(Parameter parameter) throws TuniuCommonException {
		...
		Boolean hasSeat = parameter.getBoolean("hasSeat");// 是否支持无座 true:不接受
															// false：接受
		...
		Boolean isChooseSeats = parameter.getBoolean("isChooseSeats"); // 是否选座,
																		// true:选,
																		// false:非选
		...
		try {
			/* 业务参数检查 */
			if ("".equals(vendorOrderId) || "".equals(orderId) || "".equals(orderNumber) || "".equals(changeId) || "".equals(changeCheCi)
					|| "".equals(changeDateTime) || "".equals(changeZwCode) || "".equals(hasSeat) || "".equals(oldZwCode) || "".equals(tickets)
					|| "".equals(callBackUrl)) {
			 ...
			 * if(orderInfo.getFromCity().equals("")||orderInfo.getToCity().
			 * equals("")){
			 * logger.info("途牛改签，接口暂时不支持变更到站，orderId:"+vendorOrderId); throw new
			 * TuniuCommonException(TuniuCommonService.RETURN_CODE_PARAM_ERROR);
			 * }
			 */
			if (userName != "") {
				// 传账号信息
			...
			if (tickets.size() > 1) {
				/* 批量改签 */
				if (oldZwCode.equals("3") || oldZwCode.equals("4") || oldZwCode.equals("6") || oldZwCode.equals("F") || changeZwCode.equals("3")
						|| changeZwCode.equals("4") || changeZwCode.equals("6") || changeZwCode.equals("F")) {
					/* 批量改签原票坐席不能为卧铺 */
			...
			changeInfo.setIsChooseSeats(isChooseSeats != null && isChooseSeats ? 1 : 0); // 1：
																							// 选
																							// ,
																							// 0：
																							// 非选
	...
	public void callbackChangeNotice(List<TuniuChangeInfo> notifyList) {
		...
		try {
			for (TuniuChangeInfo changeInfo : notifyList) {
				...
				logger.info("改签订单号：" + changeInfo.getOrder_id() + ",回调地址：" + changeInfo.getCallbackurl() + ",count:" + count + ",通知状态："
						+ changeInfo.getChange_notify_status());
				...
				/* 请求线程 */
				SimpleRequest request = new SimpleRequest(SimpleRequest.METHOD_POST, changeInfo.getCallbackurl(), callback);
	...
	public Result addChangeCaptcha(Parameter parameter, String captchaResultUrl, String robotUrl) throws TuniuCommonException {
		...
		// newParameter.getString("vendorOrderId");//我库订单号
		...
		Boolean hasSeat = newParameter.getBoolean("hasSeat");// 是否支持无座 true:不接受
																// false：接受
		...
		try {
			/* 业务参数检查 */
			if ("".equals(orderId) || "".equals(orderNumber) || "".equals(changeCheCi) || "".equals(changeDateTime) || "".equals(changeZwCode)
			...
			// throw new TuniuCommonException(CHANGE_NOT_EXIST_ERROR);
			// }else
			// if(!orderInfo.getOrderStatus().equals(TuniuOrderService.STATUS_OUT_SUCCESS)){
			// logger.info("途牛改签，订单状态不是出票成功，orderId:"+vendorOrderId);
			// throw new TuniuCommonException(CHANGE_STATUS_ERROR);
			// }
			...
			// Map<String,Object> changeParam = new HashMap<String,Object>();
			// changeParam.put("reqtoken", changeId);
			// TuniuChangeInfo changeInfo =
			// tuniuChangeDao.selectChangeInfo(changeParam);
			// if(changeInfo!=null){
			// logger.info("途牛请求改签，该请求已存在，reqtoken为"+changeId);
			// throw new TuniuCommonException(APPLY_CHANGE_REPEAT_ERROR);
			// }
			...
			/* 改签时间验证 */
			// if(DateUtil.minuteDiff(DateUtil.stringToDate(changeDateTime,
			// DateUtil.DATE_FMT3), new Date()) < 30) {
			// /*距离发车时间小于30分*/
			// logger.info("途牛-请求改签ERROR,距离开车时间太近无法改签");
			// throw new TuniuCommonException(APPLY_CHANGE_TIME_ERROR);
			// }
			...
			param1.append(orderId).append("|").append(orderNumber).append("|").append(fromStationName).append("|").append(toStationName).append("|")
					.append(changeDateTime).append("|").append(changeDateTime).append("|").append(changeCheCi).append("|").append(changeCheCi).append("|")
					.append("0");
			// 总参数拼接
			luaParams.append("ScriptPath=").append("changeTicket.lua").append("&commond=").append("changeTicketCaptcha").append("&SessionID=")
					.append(String.valueOf(System.currentTimeMillis())).append("&Timeout=").append("1000000").append("&ParamCount=").append(2)
					.append("&DeviceNo=").append("s9eyi3p5evmc9g83").append("&FromCity3c=").append(fromCity3c).append("&ToCity3c=").append(toCity3c)
					.append("&HasSeat=").append(hasSeat ? 1 : 0).append("&Param1=").append(URLEncoder.encode(param1.toString(), "utf-8"));
			// params2
			for (int i = 0; i < tickets.size(); i++) {
				luaParams.append("&Param").append(i + 2).append("=");
				JSONObject jsonObject = tickets.getJSONObject(i);
				StringBuffer param = new StringBuffer();
				param.append(jsonObject.get("passengerId")).append("|").append(jsonObject.get("passengerName")).append("|").append(jsonObject.get("piaoType"))
						.append("|").append(jsonObject.get("passportTypeId")).append("|").append(jsonObject.get("passportNo")).append("|")
						.append(sysSeatType(changeZwCode)).append("|").append(sysSeatType(oldZwCode)).append("|").append(jsonObject.get("zwName"));
			...
			captchaBuffer.append("commond=").append("getCaptcha").append("&requestId=").append(orderId).append("&captchaType=").append("change");
			...
			// if(tickets.size() > 1) {
			// /*批量改签*/
			// if(oldZwCode.equals("3") ||oldZwCode.equals("4")
			// ||oldZwCode.equals("6")
			// ||changeZwCode.equals("3") ||changeZwCode.equals("4")
			// ||changeZwCode.equals("6")) {
			// /*批量改签原票坐席不能为卧铺*/
			// logger.info("途牛-请求改签ERROR,批量改签原票或新票坐席不能为卧铺");
			// throw new TuniuCommonException(APPLY_CHANGE_SEAT_TYPE_ERROR);
			// }
			...
			// /*组装改签车票信息*/
			// changeInfo = new TuniuChangeInfo();
			// List<TuniuChangePassengerInfo> changePassengers = new
			// ArrayList<TuniuChangePassengerInfo>();
			// for(int i = 0; i < tickets.size(); i++) {
			// /*传入的参数数据*/
			// JSONObject ticket = tickets.getJSONObject(i);
			// //改签车票信息
			// TuniuChangePassengerInfo cp = new TuniuChangePassengerInfo();
			// Map<String, Object> changeCpParam = new HashMap<String,
			// Object>();
			// changeCpParam.put("cp_id", ticket.getString("oldTicketNo"));
			// List<TuniuChangePassengerInfo> cps =
			// tuniuChangeDao.selectChangeCp(changeCpParam);
			// if(cps != null) {
			// for(TuniuChangePassengerInfo changeCp:cps){
			// /*每张车票只能改签一次*/
			// if(changeCp.getIs_changed().equals("Y")){
			...
			// changeInfo.setTrain_no(orderInfo.getTrainNo());//车次
			// changeInfo.setChange_train_no(changeCheCi);//改签后车次
			// changeInfo.setFrom_time(DateUtil.dateToString(orderInfo.getFromTime(),
			// DateUtil.DATE_FMT3));//发车时间
			// changeInfo.setChange_from_time(changeDateTime);//改签后发车时间
			// changeInfo.setTravel_time(DateUtil.dateToString(orderInfo.getTravelDate(),
			// DateUtil.DATE_FMT3));
			// changeInfo.setFrom_city(orderInfo.getFromCity());//出发车站
			// changeInfo.setTo_city(orderInfo.getToCity());//到达车站
			// changeInfo.setFrom_station_code(orderInfo.getFromCityCode());
			...
			// }
			// changeInfo.setcPassengers(changePassengers);//改签、车票关系
			// changeInfo.setBook_ticket_time(DateUtil.dateToString(new Date(),
			// DateUtil.DATE_FMT3));
			// String acc_id =
			// tuniuOrderDao.selectAccountByOrder(vendorOrderId);
			// changeInfo.setAccount_id(acc_id);//出票账号id
			// changeInfo.setChange_notify_count(0);
			// changeInfo.setChange_notify_status(CHANGE_NOTIFY_PRE);
			// changeInfo.setMerchant_id(TuniuConstant.merchantId);
			...
			// tuniuChangeDao.insertChangeInfo(changeInfo);
			// int change_id = changeInfo.getChange_id();
			// List<TuniuChangePassengerInfo> cPassengers =
			// changeInfo.getcPassengers();
			// if (cPassengers != null && cPassengers.size() != 0) {
			// for (TuniuChangePassengerInfo cPassenger : cPassengers) {
			// cPassenger.setChange_id(change_id);
			// tuniuChangeDao.insertChangeCp(cPassenger);
	...
	static {
		...
		changeErrorInfo.put("325", "确认改签失败");
		changeErrorInfo.put("326", "确认改签时间已超时，确认改签失败");
	}
	...
	static {
		...
		changeErrorCode.put("325", "1600201");
		changeErrorCode.put("326", "1600202");
	}
