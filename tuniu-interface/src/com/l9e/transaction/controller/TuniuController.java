...
public class TuniuController extends BaseController {
	...
	private static final Logger logger = Logger.getLogger(TuniuController.class);
	...
	public void tuniuTrain_Account(HttpServletRequest request, HttpServletResponse response, @PathVariable String method) throws IOException {
		...
		if ("validate".equals(method)) {
			if (ut.getHour() > 23 || ut.getHour() < 6) {
				...
				try {
					...
					String data1 = EncryptUtil.decode(dataBase64, TuniuConstant.signKey, "UTF-8");
	...
	public void tuniuTrain_contact(HttpServletRequest request, HttpServletResponse response, @PathVariable String method) throws IOException {
		SynchronousOutput synOutput = new SynchronousOutput();
		try {
			if ("query".equals(method)) {
				...
				if ("error".equals(flag)) {
					...
					for (int j = 0; j < arrayflag.size(); j++) {
						...
						Integer checkStatus = (Integer) maps.get("checkStatus");// 身份是否校验通过
																				// 0通过1不通过
				...
				TuniuDeleteContact tuniuDeleteContact = JacksonUtil.readJson(str, TuniuDeleteContact.class);
	...
	public void tuniuTrain(HttpServletRequest request, HttpServletResponse response, @PathVariable String method) {
		/* 同步输出对象 */
		SynchronousOutput synOutput = new SynchronousOutput();
	...
	public void tuniuTrain_PushOrderStatus(HttpServletRequest request, HttpServletResponse response) {
	 ...
	 * @throws TuniuCommonException
	 */
	private void tuniuVerify(HttpServletResponse response, SynchronousOutput synOutput, String paramStr) throws IOException, TuniuCommonException {
		TuniuInput input = JacksonUtil.readJson(paramStr, TuniuInput.class);
		if (input == null)
			throw new TuniuCommonException(TuniuCommonService.RETURN_CODE_PARAM_ERROR);
		/* 通用参数检查 */
	 ...
	 * @throws TuniuCommonException
	 */
	public void tuniuPushOrderStatus(Parameter parameter, SynchronousOutput synOutput) throws TuniuCommonException {
	...
	private void tuniuTrainSearch(Parameter parameter, SynchronousOutput synOutput) {
		...
		if (StringUtils.isEmpty(trainDate) || StringUtils.isEmpty(fromStation) || StringUtils.isEmpty(toStation)) {
		...
		if (StringUtils.isEmpty(result) || result.equalsIgnoreCase("STATION_ERROR") || result.equalsIgnoreCase("ERROR")) {
	...
	private void validateCommonParameter(TuniuInput input, String signkey) throws TuniuCommonException {
		...
		// return;
		/* end */
		if (isEmpty(input.getAccount()) || isEmpty(input.getTimestamp()) || isEmpty(input.getSign()))
			throw new TuniuCommonException(TuniuCommonService.RETURN_CODE_PARAM_ERROR);

		if (!input.getAccount().equals(TuniuConstant.account))
			throw new TuniuCommonException(TuniuCommonService.RETURN_CODE_USER_NOT_EXISTS);

		if (!tuniuCommonService.validateTimestamp(input.getTimestamp()))
			throw new TuniuCommonException(TuniuCommonService.RETURN_CODE_TIMESTAMP_ERROR);
		...
		if (!input.getSign().equals(validateSign))
			throw new TuniuCommonException(TuniuCommonService.RETURN_CODE_SIGNATURE_ERROR);
	}
	...
	private void wrapSynOutput(Result result, SynchronousOutput synOutput) {
		...
		String errorMsg = tuniuCommonService.getErrorMessage(code);
		synOutput.setReturnCode(code);
		if (code != null && !code.equals(TuniuCommonService.RETURN_CODE_SUCCESS) && !code.equals(TuniuOrderService.RETURN_CODE_CONFIRM_ORDER_SUCCESS)
				&& !code.equals(TuniuOrderService.RETURN_CODE_CANCEL_SUCCESS) && !code.equals(TuniuRefundService.RETURN_CODE_REFUND_SUCCESS)) {
			synOutput.setSuccess(false);
		}
		synOutput.setErrorMsg(errorMsg);
	}
	...
	public String saveUpdateContact(Account account, TuniuContact tuniuContact) {
		...
		if (id > 0) {
			...
			String result = HttpUtil.sendByPost(url, uri, "UTF-8");
			// String result =
			// "[{\"cert_no\":\"510623199407301613\",\"cert_type\":\"2\",\"check_status\":\"12\",\"user_name\":\"郭鸿\",\"user_type\":\"0\"}]";
	...
	public void submitCaptcha(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			captchaBuffer.append("commond=").append("updateCaptcha").append("&requestId=").append(parameter.getString("requestId")).append("&randCode=")
					.append(parameter.getString("randCode")).append("&captchaType=").append(parameter.getString("captchaType"));
			String captchaResultUrl = (String) config.get("captchaResultUrl");
			...
			if ("success".equals(updateCaptcha)) {
				...
				captchaResultBuffer.append("commond=").append("resultCaptcha").append("&requestId=").append(parameter.getString("requestId"))
	...
	public void checkLogin(HttpServletRequest request, HttpServletResponse response) {
		...
		try {
			...
			luaParams.append("ScriptPath=").append("appTuniuCheck.lua").append("&commond=").append("trainOrder").append("&SessionID=")
					.append(String.valueOf(System.currentTimeMillis())).append("&Timeout=").append("100000").append("&ParamCount=").append(2).append("&Param1=")
					.append(URLEncoder.encode(parameter.getString("userName") + "|" + parameter.getString("userPassword"), "UTF-8"));
			...
			// HttpUtil.sendByGet(reqUrl, inputCharset, connectiontimeout,
			// readtimeout)(checkUrl, luaParams.toString(), "UTF-8");
