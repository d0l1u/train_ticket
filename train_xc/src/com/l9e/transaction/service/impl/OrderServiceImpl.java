...
public class OrderServiceImpl implements OrderService {
	...
	public String addOrder(OrderInfo orderInfo,
			List<OrderInfoCp> orderInfoCpList, List<OrderInfoBx> orderInfoBxList, Map<String, String> bxfpMap,String book_flow) {
...
//				deleteAllOrderInfo(orderInfo.getOrder_id());
	...
	public void updateRefundAgree(Map<String, Object> map, Map<String,String> paramMap) {
		orderDao.updateRefundInfo(map);//更新xc_orderinfo_refundstream表的信息（同意退款）
		orderDao.updateCPOrderInfo(map);//更新xc_orderinfo_cp表的信息
		String refund_seq = orderDao.queryRefundStreamSeq(paramMap);
		int count = orderDao.queryRefundCount(refund_seq);//根据refund_seq查询xc_orderinfo_refundstream有几条退款记录
		if(count <= 0){
			map.put("refund_seq", refund_seq);
			try {
				orderDao.updateExtRefundNotifyStatus(map);//更新xc_orderinfo_refundnotify表的信息（同意退款）
	...
	@Override
	public String queryOrderIdById(String merchantOrderId) {
		return orderDao.queryOrderIdById(merchantOrderId);
	}
