...
public class OutTicketController extends BaseController {
	...
	public void orderNotice(HttpServletRequest request,
		...
		try {
			...
			logger.info("[接收出票结果通知接口]参数orderId=" + orderId + ",result="
					+ result + ",billNo=" + billNo + ",buyMoney=" + buyMoney
					+ ",seatTrains=" + seatTrains + ",status=" + status
					+ ",level=" + level + ",from_time=" + from_time
					+ ",to_time=" + to_time + ",pay_limit_time"
			...
			if (StringUtils.isEmpty(result) || StringUtils.isEmpty(orderId)) {
				...
				if ("11".equals(status)) {// 预订成功
					if (Consts.ELONG_ORDER_MAKED.equals(orderInfo
							.getOrder_status())
							|| Consts.ELONG_ORDER_WAITPAY.equals(orderInfo
									.getOrder_status())) {
						logger.info("[接收出票结果通知接口]本次出票成功通知为重复通知,orderId="
								+ orderId);
						write2Response(response, "success");
						return;
					}
					if (Consts.ELONG_OUT_TIME.equals(orderInfo
							.getOrder_status())) {
					...
					if (Consts.ELONG_ORDER_CANCELED.equals(orderInfo
							.getOrder_status())) {
					...
					orderInfo.setOrder_type("");
					orderInfo.setExt_field2("");
					if ("".equals(pay_limit_time) || "".equals(zz_time)) {
						...
						log.setContent("出票系统返回pay_limit_time:"
								+ pay_limit_time + ",out_ticket_time:"
								+ zz_time);
					...
					//改为线程处理
					BookResultThread book = new BookResultThread(orderInfo, noticeService, orderService);
					...
					/*if ("meituan".equals(orderInfo.getChannel())) {
						...
						if (Consts.NOTICE_OVER.equals(book_notify)) {
							...
							logger.info("send to [美团] the booked resuld,"
				...
				if ("00".equals(status)) {// 出票成功 //
					/** 重复通知处理 */
					if (Consts.ELONG_ORDER_SUCCESS.equals(orderInfo
							.getOrder_status())) {
						logger.info("[接收出票结果通知接口]本次出票成功通知为重复通知,orderId="
								+ orderId);
						write2Response(response, "success");
					...
					orderInfo.setOut_ticket_time(zz_time);
					orderInfo.setOut_ticket_billno(billNo);
					orderInfo.setOrder_status(Consts.ELONG_ORDER_SUCCESS);//出票成功
					orderInfo.setFrom_time(from_time);//
					orderInfo.setTo_time(to_time);
					orderInfo.setOrder_type("");
					orderInfo.setExt_field2("");
					...
					if ("meituan".equals(orderInfo.getChannel())) {
						...
						for (Map<String, String> cpInfo : cpMapList) {
							...
							if (!cpInfo.get("seat_no").equals(firstSeatNo)) {
								//
								reOrder = true;
								log.setContent("检测到预订坐席和出票坐席内容不一致:开启补单通知"
								...
								logger.info("检测坐席类型" + firstSeatNo + "|"
										+ cpInfo.get("seat_no") + "，一致"
										+ orderInfo.getOrder_id() + ","
										+ cpInfo.get("cp_id"));
							}
						}
					...
					//改为线程处理
					TicketResultThread ticketResult = new TicketResultThread(orderInfo, orderService);
					Thread thread = new Thread(ticketResult);
					thread.start();
				...
				if (!StringUtils.isEmpty(orderInfo.getOrder_status())
						&& Consts.ELONG_ORDER_FAIL.equals(orderInfo
								.getOrder_status())) {
				...
				//错误信息：1所购买的车次坐席已无票 2身份证件已经实名制购票 3票价和12306不符 4乘车时间异常 5证件错误 6用户要求取消订单 
				//7未通过12306实名认证 8乘客身份信息待核验 9、系统异常 10、超时未支付 
				write2Response(response, "success");
				orderInfo.setBuy_money(buyMoney);
				orderInfo.setOut_ticket_billno(billNo);
				orderInfo.setOrder_status(Consts.ELONG_ORDER_FAIL);//出票失败
	...
	public void refundNotify_no(HttpServletRequest request,
		...
		logger.info("【接收退票系统通知】参数orderId=" + orderId + "，cpid=" + cpid
				+ "，alterdiffmoney=" + alterdiffmoney + "，refund12306money="
				+ refund12306money + "，refundmoney=" + refundmoney
				+ "，refund12306seq=" + refund12306seq + "，status=" + status
				+ "，result=" + result + "，our_remark=" + our_remark
