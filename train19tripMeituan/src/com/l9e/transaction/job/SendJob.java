...
public class SendJob {
	...
	private void notifyRefundSys(Map<String, String> sendMap){
		...
		try{
			...
			}else if("55".equals(sendMap.get("refund_type"))){
				orderMap=refundService.queryChangeRefundCpOrderInfo(param);
				paramMap.put("refundtype","55");//改签退票
			}
			Map<String, Object> accountMap = refundService.queryAccountOrderInfo(param);
			...
			paramMap.put("accountname",accountMap.get("acc_username").toString());
			paramMap.put("accountpwd", accountMap.get("acc_password").toString());
			...
			logger.info("美团通知出票系统参数为"+paramMap.toString());
