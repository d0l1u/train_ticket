...
public class HthyRequestService {
	...
	// 下单接口
	private static final String orderUrl = PropertyUtil.getValue("interface.order");
	...
	public String trainOrder(JSONObject paramJson) {
		...
		String chooseSeat = trainJson.getString("chooseSeat");
		...
		logger.info("{}航天华有占位下单接口参数:{}", logid, requestStr);

		String httpResult = httpUtil.doHttpPost(orderUrl, requestStr, 1000 * 30, false, "UTF-8");
		// {"orderid":"T1609086DC260C00B4FD0434D0B3AD04aaaaaaaA70","code":"802","msg":"创建订单成功","success":true}
		logger.info("{}航天华有占位下单接口结果:{}", logid, httpResult);
		return httpResult;
	}
	...
	public String cancel(String orderId, String supplierId) {
		...
		String requestStr = "jsonStr=" + requestJson.toJSONString();
		logger.info("{}航天华有取消下单接口参数:{}", logid, requestStr);
		String httpResult = httpUtil.doHttpPost(orderUrl, requestStr, 1000 * 10, false, "UTF-8");
		logger.info("{}航天华有取消下单接口结果:{}", logid, httpResult);
		return httpResult;
	}
	...
	public String confirm(String orderId, String supplierId) {
		...
		String requestStr = "jsonStr=" + requestJson.toJSONString();
		logger.info("{}航天华有确认下单接口参数:{}", logid, requestStr);
		String httpResult = httpUtil.doHttpPost(orderUrl, requestStr, 1000 * 10, false, "UTF-8");
		logger.info("{}航天华有确认下单接口结果:{}", logid, httpResult);
		return httpResult;
	}
	...
	public String refund(JSONObject paramJson) {
		...
		JSONObject accountJson = paramJson.getJSONObject("account");// 12306帐号相关数据
		JSONObject trainJson = paramJson.getJSONObject("data");// 预定车次相关数据
		JSONArray passengerArray = trainJson.getJSONArray("passengers");// 预定人相关数据
		...
		requestJson.put("ordernumber", trainJson.getString("sequence"));
		...
		String requestStr = "jsonStr=" + requestJson.toJSONString();
		logger.info("{}航天华有退票接口参数:{}", logid, requestStr);
		String httpResult = httpUtil.doHttpPost(orderUrl, requestStr, 1000 * 10, false, "UTF-8");
		logger.info("{}航天华有退票接口结果:{}", logid, httpResult);
		return httpResult;
	}

	public String change(JSONObject paramJson) {
		JSONObject accountJson = paramJson.getJSONObject("account");// 12306帐号相关数据
		JSONObject trainJson = paramJson.getJSONObject("data");// 预定车次相关数据
		JSONArray passengerArray = trainJson.getJSONArray("passengers");// 预定人相关数据
		...
		String trainCode = trainJson.getString("newTrainCode");
		requestJson.put("change_checi", trainCode);// 改签车次
		// 改签发车时间，例：yyyy-MM-dd HH:mm:ss
		requestJson.put("change_datetime", trainJson.getString("newDepartureDate") + " 00:00:00");
		requestJson.put("from_station_code", trainJson.getString("newFromStationCode")); // 出发站简码(非必选)
		requestJson.put("from_station_name", trainJson.getString("newFromStationName")); // 出发站名称
		requestJson.put("to_station_code", trainJson.getString("newToStationCode")); // 到达站简码(非必选)
		requestJson.put("to_station_name", trainJson.getString("newToStationName")); // 到达站名称
		...
		String requestStr = "jsonStr=" + requestJson.toJSONString();
		logger.info("{}航天华有改签占位接口参数:{}", logid, requestStr);
		String httpResult = httpUtil.doHttpPost(orderUrl, requestStr, 1000 * 10, false, "UTF-8");
		logger.info("{}航天华有改签占位接口结果:{}", logid, httpResult);
		return httpResult;
	}
