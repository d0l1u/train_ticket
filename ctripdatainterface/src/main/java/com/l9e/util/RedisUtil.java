...
/**
 * Redis工具类 Created by caicf on 2017/1/1.
 */
public class RedisUtil {
	...
	private RedisUtil() {
		...
		try {
			InputStream ins = RedisUtil.class.getClassLoader().getResourceAsStream("redis.properties");
			properties.load(ins);
		} catch (Exception e) {
			log.info("读取properties文件异常", e);
		}
	}
	...
	private void initJedisPool() {
		...
		config.setMaxWaitMillis(Integer.valueOf(properties.getProperty("redis.pool.maxWaitTime")));
		...
		int dbIndex=Integer.valueOf(properties.getProperty("redis.pool.dbIndex"));
		try {
			if (null != password && !"".equals(password)) {
				jedisPool = new JedisPool(config, properties.getProperty("redis.ip"),
						Integer.valueOf(properties.getProperty("redis.port")),
						Integer.valueOf(properties.getProperty("redis.pool.timeOut")), password,dbIndex);
			} else {
				jedisPool = new JedisPool(config, properties.getProperty("redis.ip"),
						Integer.valueOf(properties.getProperty("redis.port")),
						Integer.valueOf(properties.getProperty("redis.pool.timeOut")));
			}
		} catch (Exception e) {
			log.info("初始化JedisPool失败", e);
		}
	}
