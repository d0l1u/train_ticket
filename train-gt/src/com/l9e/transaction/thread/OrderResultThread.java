...
/**
 * @author meizs
 * @create 2018-03-20 13:54
 *  出票回调异步处理
 **/

@Component("orderResultThread")
@Scope("prototype")
public class OrderResultThread implements Runnable{

    private static final Logger logger = Logger.getLogger(OrderResultThread.class);
    ...
    private Map<String, String> waitNotify;

    public Map<String, String> getParamMap() {
        return waitNotify;
    }
    public void setParamMap(Map<String, String> paramMap) {
        this.waitNotify = paramMap;
    }
    ...
    public void run() {
        ...
        try {
            ...
            String orderId = waitNotify.get("order_id");
            ...
            if ("11".equals(order_type)) {
                confirmOrderResult(timestamp, waitNotify); // 先预定后出票,确认出票异步通知
            } else if ("22".equals(order_type)) {// 先支付后预定
                orderResult(timestamp, waitNotify); // 原有的出票结果通知,不做修改
            }

        } catch (Exception e) {
            logger.info("OrderResultThread执行出现异常：", e);
        }
    ...
    public void confirmOrderResult(String timestamp, Map<String, String> map) {
        ...
        String ticketEntrancesStr="";//检票口字段
        if ("SUCCESS".equals(status)) {
            //查询订单的检票口信息,cp_orderinfo_ct
            List<TicketEntrance> ticketEntrancesList = orderService.selectOrderTicketEntrance(order_id);
            logger.info(order_id + ",[list]检票口：" + ticketEntrancesList);
            if (null != ticketEntrancesList && ticketEntrancesList.size() > 0) {
                TicketEntrance ticketEntrance = ticketEntrancesList.get(0);//获取一个实体
                ticketEntrancesStr = ticketEntrance.getEntrance();
            } else {
                ticketEntrancesStr = "";
            }
            logger.info(order_id + ",[str]出票成功检票口：" + ticketEntrancesStr);
            json_param.put("ticket_entrance", ticketEntrancesStr);//检票口
        }else{
            logger.info(order_id + ",[str]出票失败检票口：" + ticketEntrancesStr);
            json_param.put("ticket_entrance", ticketEntrancesStr);//检票口
        }
     ...
     */
    public void orderResult(String timestamp, Map<String, String> map) throws NumberFormatException {
        JSONArray orderUsers = null;
        String status = "";
        String fail_reason = "";
        String refund_amount = "";
        ...
        for (Map<String, String> cp : cpInfoList) {
            ...
            // Map<String,String> merchantInfoMap2 =
            // orderService.queryMerchantInfoByOrderId(order_id);
            // if("301008".equals(merchantInfoMap2.get("merchant_id"))&&
            // "9".equals(String.valueOf(cp.get("seat_type"))) &&
            // cp.get("seat_no")!= null && "无座".equals( cp.get("seat_no"))){
            // OrderInfo order2 = orderService.queryOrderInfo(order_id);
            // if(order2.getTrain_no().indexOf("C")!=-1 ||
            // order2.getTrain_no().indexOf("D")!=-1 ||
            // order2.getTrain_no().indexOf("G")!=-1){
