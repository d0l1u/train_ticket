...
public class CreateOrderController extends ExternalBase {
	 ...
	 * @return
	 */
	public void createTrainOrder(Map<String, String> merchantInfo, BookInfo bookInfo, HttpServletRequest request, HttpServletResponse response)
			throws Exception {
		...
		try {
			...
			logger.info(bookInfo.getMerchant_order_id() + ",method:" + method + ",do_type:" + do_type + ",book_flow:" + book_flow + ",order_type:"
	 ...
	 */
	private void groupData(Map<String, String> merchantInfo, BookInfo bookInfo, OrderInfo orderInfo, List<OrderInfoCp> orderInfoCpList,
		...
		orderInfo.setIsChooseSeats(bookInfo.isIsChooseSeats() ? 1 : 0); // 是否选座,
																		// 1:选,
																		// 0:非选
		orderInfo.setChooseSeats(bookInfo.getChooseSeats()); // 选座信息
																// ,如：1A1D2A2B2F

		if (!StringUtils.isEmpty(bookInfo.getSeat_type()) && TrainConsts.SEAT_8.equals(bookInfo.getSeat_type()) && "1".equals(bookInfo.getWz_ext())) {// 硬座选择无座备选
		...
		for (BookDetailInfo bookDetailInfo : bookInfo.getBook_detail_list()) {
			...
			if ("1".equals(buy_bx)) {
				...
				// String bx_channel =
				// commonService.querySysSettingByKey("bx_channel");
				// 保险
	...
	private String getTotalPay(BookInfo bookInfo, Map<String, String> map) {
		...
		// Double.valueOf(map.get("merchant_fee")) *
		// bookInfo.getBook_detail_list().size();
		...
		if ("order".equals(fee_type)) {
			...
			ticket_total_pay = Double.valueOf(getTicketTotalPay(bookInfo)) * Double.valueOf(fee_map.get("percent_fee"))
					+ Double.valueOf(getTicketTotalPay(bookInfo));
			...
			if (map.get("ticket_num").compareTo(echelon_fir[0]) > 0) {
				...
				ticket_total_pay = Double.valueOf(fee_map.get("echelon_zeo")) * bookInfo.getBook_detail_list().size()
						+ Double.valueOf(getTicketTotalPay(bookInfo));
			}
