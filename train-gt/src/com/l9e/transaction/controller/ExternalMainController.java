...
/**
 * 
 * @ClassName: ExternalMainController
 * @Description: 对外接口
 * @author: taoka
 * @date: 2018年1月10日 上午10:33:48
...
public class ExternalMainController extends ExternalBase {
	...
	// 实名认证
	@Value("${config.real_name_verify_url}")
	protected String real_name_verify_url;

	@Value("${config.notify_cp_interface_url}")
	private String notify_cp_interface_url;

	/** 为回调订单请求key */
	@Value("${config.GT_GETNOCALLORDER_KEY}")
	private String GT_GETNOCALLORDER_KEY;

	/** 为回调订单请求URL */
	@Value("${config.get_nocallorder_url}")
	...
	public void ExternalInterface(HttpServletRequest request, HttpServletResponse response) {
		// 日志随机编号，不确保唯一
		String uuid = createLogUUID();

		logger.info(uuid + "请求参数列表:" + getFullURL(request));
		request.setAttribute("uuid", uuid);

		String type = this.getParam(request, "type");
		String method = this.getParam(request, "method");
		logger.info(uuid + "【TYPE】:" + type + ", 【METHOD】:" + method);
		...
		if ("createOrder".equals(type) || "createOrder".equals(type) && "book".equals(method)) {
			...
			logger.info(uuid + "hcp对外接口-非法的type接口参数 :type=" + type);
	...
	public void createOrder(HttpServletRequest request, HttpServletResponse response) {
		...
		String jsonParam = this.getParam(request, "json_param");
		String hmac = this.getParam(request, "hmac");
		// 和先付款后买票的区别就是新增了do_type参数和method参数，method的值是book,do_type是1时，就是先下单后付款，否则就是先付款后买票
		String method = this.getParam(request, "method");
		String doType = this.getParam(request, "do_type");
		String reqtoken = this.getParam(request, "reqtoken");

		if (StringUtils.isEmpty(method) || StringUtils.isEmpty(doType)
				|| !("book".equals(method) && "1".equals(doType))) {// 先付款后买票
			try {
				if (StringUtil.isEmpty(terminal) || StringUtil.isEmpty(merchantId) || StringUtil.isEmpty(timestamp)
						|| StringUtil.isEmpty(type) || StringUtil.isEmpty(version) || StringUtil.isEmpty(hmac)
						|| StringUtil.isEmpty(jsonParam)
						|| ParamCheckUtil.isNotCheck(timestamp, ParamCheckUtil.TIMESTAMP_REGEX)) {
					logger.info(uuid + "购票下单接口-参数校验error:必要参数为空或格式错误!");
					printJson(response, getJson("003").toString());
					return;
				}
				...
				if (merchantSetting == null) {
					logger.info(uuid + "购票下单接口-用户身份校验error:不存在的用户!");
				...
				merchantSetting.put("do_type", doType);

				logger.info(uuid + "购票下单接口-用户:【" + merchantSetting.get("merchant_name") + "】登入成功!");
				logger.info(uuid + "购票下单接口-用户传递hmac：【" + hmac + "】");
				// 加密明文
				String mdStr = terminal + merchantId + timestamp + type + version + jsonParam + method + doType;

				String hmac19 = Md5Encrypt.getKeyedDigestFor19Pay(mdStr + merchantSetting.get("sign_key"), "", "utf-8");
				logger.info(uuid + "购票下单接口-用户下单参数：" + mdStr);
				logger.info(uuid + "-系统加密hmac：【" + hmac19 + "】");
				if (!hmac19.equals(hmac)) {
					logger.info(uuid + "购票下单接口-安全校验error:不符合安全校验规则。");
				...
				if ("00".equals(merchantSetting.get("merchant_status"))) {
					logger.info(uuid + "购票下单接口-下单规则error:合作商户已禁用，请客服人员通知商户！");
					printJson(response, getJson("007").toString());
					return;
				}
				bookInfo = mapper.readValue(jsonParam, BookInfo.class);
				String key = merchantId + bookInfo.getMerchant_order_id();
			...
			try {
				if (StringUtil.isEmpty(terminal) || StringUtil.isEmpty(merchantId) || StringUtil.isEmpty(timestamp)
						...
						|| StringUtil.isEmpty(jsonParam)
						|| ParamCheckUtil.isNotCheck(timestamp, ParamCheckUtil.TIMESTAMP_REGEX)
						|| StringUtil.isEmpty(method) || StringUtil.isEmpty(doType)) {
					logger.info(uuid + "购票下单接口-参数校验error:必要参数为空或格式错误!");
					cnro.setGtgjOrderId("");
					cnro.setSupplierOrderId("");
					cnro.setReqtoken(StringUtil.isEmpty(reqtoken) ? "" : reqtoken);
					cnro.setSuccess("false");
					cnro.setMsg(TrainConsts.getReturnCode().get(TrainConsts.RETURN_CODE_003));
					cnro.setReturn_code(TrainConsts.RETURN_CODE_003);
					String result_json = gson.toJson(cnro);
					logger.info(uuid + "先预定后支付，下单接口，返回结果：" + result_json);
				...
				if (merchantSetting == null) {
					...
					cnro.setReturn_code(TrainConsts.RETURN_CODE_004);
					String result_json = gson.toJson(cnro);
					logger.info(uuid + "先预定后支付，下单接口，返回结果：" + result_json);
					printJson(response, result_json);
					return;
				...
				logger.info(uuid + "购票下单接口-用户传递hmac：【" + hmac + "】");
				// 加密明文
				String md_str = terminal + merchantId + timestamp + type + version + jsonParam + method + doType;

				String hmac_19 = Md5Encrypt.getKeyedDigestFor19Pay(md_str + merchantSetting.get("sign_key"), "",
						"utf-8");
				logger.info(uuid + "购票下单接口-用户下单参数：" + md_str);
				logger.info(uuid + "-系统加密hmac：【" + hmac_19 + "】");
				if (!hmac_19.equals(hmac)) {
					...
					cnro.setReturn_code(TrainConsts.RETURN_CODE_002);
					String result_json = gson.toJson(cnro);
					printJson(response, result_json);
					return;
				}
				...
				if ("00".equals(merchantSetting.get("merchant_status"))) {
					...
					cnro.setReturn_code(TrainConsts.RETURN_CODE_007);
					String result_json = gson.toJson(cnro);
					printJson(response, result_json);
					return;
				}
				...
				if (ParamCheckUtil.createOrderParamIsEmpty(bookInfo)) {
					...
					cnro.setReturn_code(TrainConsts.RETURN_CODE_204);
					String result_json = gson.toJson(cnro);
					printJson(response, result_json);
					return;
				}
				...
				if (ParamCheckUtil.bookDetailInfoCheck(bookInfo.getBook_detail_list(), bookInfo.getOrder_level())) {
					...
					cnro.setReturn_code(TrainConsts.RETURN_CODE_205);
					String result_json = gson.toJson(cnro);
					printJson(response, result_json);
					return;
				}
				...
				if (null == MemcachedUtil.getInstance().getAttribute(key)) {
					...
					if (str.equals(key)) {
						...
						cnro.setReturn_code(TrainConsts.RETURN_CODE_201);
						String result_json = gson.toJson(cnro);
						printJson(response, result_json);
						return;
					}
				...
				cnro.setReturn_code(TrainConsts.RETURN_CODE_001);
				String result_json = gson.toJson(cnro);
				printJson(response, result_json);
			}
			// --------------------------------//
		...
		try {
			...
			for (int i = 0; i < index; i++) {
				if (!"0".equals((jsonArr.getJSONObject(i).get("check_status")))) {
					 ...
					 * else if
					 * ("0".equals((jsonArr.getJSONObject(i).get("check_status")
					 * )) &&
					 * "登录异常!".equals((jsonArr.getJSONObject(i).get("message")))
					 * ) { fail_pas += jsonArr.getJSONObject(i).get("user_name")
					 * + ","; fail_idcard +=
					 * jsonArr.getJSONObject(i).get("cert_no") + ","; fail_info
					 * += (jsonArr.getJSONObject(i).get("user_name") + "/" +
		...
		if (checkUniversalParam(request, response)) {
			try {
				...
				if (isChooseSeats) { // 如果选座，做检验
					if (StringUtil.isEmpty(chooseSeats)
							|| ParamCheckUtil.isNotCheck(chooseSeats, ParamCheckUtil.chooseSeat)
							|| chooseSeats.length() != tickets.size() * 2) {// 校验不通过，例如：1A2B,
