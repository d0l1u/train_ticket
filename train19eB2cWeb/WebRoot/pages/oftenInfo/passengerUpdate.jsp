...
function savePassenger(){
	...
	if(linkName == true && idsCard == true && linkPhone == true){
		...
		var url = "/login/updatePassenger.jhtml?link_id="+link_id+"&link_name="+link_name+"&ids_card="+ids_card+"&link_phone="+link_phone+"&ids_type="+ids_type+"&passenger_type="+passenger_type+"&version="+new Date();
...
<div class="travel_con">
    ...
    <!--右边内容 start-->  
	<div class="right_con">
	<form id="trainForm" action="/login/updatePassenger.jhtml" method="post">
		<input type="hidden" id="link_id" name="link_id" value="${linkerMap.link_id }" />
		<ul class="MyOrder">
    		<li>编辑乘客</li>
    	</ul>
        ...
        <tr>
	        ...
	        <td><input type="text" class="text" id="link_name" name="link_name" value="${linkerMap.link_name }" onblur="checkName();" /></td>
        ...
        <tr>
	        ...
	        <td>
	        	<select id="passenger_type" name="passenger_type">
		        	<option value="0" <c:if test="${linkerMap.passenger_type eq '0' }">selected="true"</c:if> >成人</option>
                   	<option value="1" <c:if test="${linkerMap.passenger_type eq '1' }">selected="true"</c:if> >儿童</option>
	        	</select>
	        </td>
	        <td></td>
	    </tr>
        <tr>
	        ...
	        <td><select id="ids_type" name="ids_type">
	        		 <option value="2" <c:if test="${linkerMap.ids_type eq '2' }">selected="true"</c:if> >二代身份证</option>
                     <option value="3" <c:if test="${linkerMap.ids_type eq '3' }">selected="true"</c:if> >港澳通行证</option>
                     <option value="4" <c:if test="${linkerMap.ids_type eq '4' }">selected="true"</c:if> >台湾通行证</option>
                     <option value="5" <c:if test="${linkerMap.ids_type eq '5' }">selected="true"</c:if> >护照</option>
	        	</select>
	        </td>
	        <td></td>
        </tr>
        <tr>
	        ...
	        <td><input type="text" class="text" id="ids_card" name="ids_card" value="${linkerMap.ids_card }" onblur="valiIdCard();" /></td>
	        <td id="ids_card_info"></td>
        </tr>
        <tr>
	        <td class="right">手机号码：</td>
	        <td><input type="text" class="text" id="link_phone" name="link_phone" value="${linkerMap.link_phone }" onblur="checkPhone();" /></td>
