 ...
 */
@Component("sendJob")
public class SendJob extends BaseController {
	...
	private void notifyCpSys(String order_id){
		try{
			...
			paramMap.put("outtickettype", "11");//出票方式11、电子票 22、配送票
			paramMap.put("channel", "web");
			String style = this.getSysSettingValueNow("buy_ticket_style");
			paramMap.put("ispay", style);//11：先预定后支付；00：先支付后预订
			//购买了保险
			...
			if(!StringUtils.isEmpty(result)){
				...
				if("success".equalsIgnoreCase(results[0]) && results.length == 2
				    ...
				    map.put("old_status", TrainConsts.PRE_BOOK);//准备预定
