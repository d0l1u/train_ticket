...
public class SessionValidFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		if(checkIsHomeList(req)){
			//logger.info("【sid=" + esid + "】用户从EOP左侧菜单开始访问本系统，请求为：" + requestpath);
			
		}else if(isNoLoginUrl(requestpath)){
			//logger.info("【sid=" + esid + "】免登录用户开始访问本系统，请求为：" + requestpath);
			//logger.info("【sid=" + esid + "】用户开始访问本系统，请求为：" + requestpath);
...
//			return;
		...
		chain.doFilter(new XssHttpRequest((HttpServletRequest) request), response);
