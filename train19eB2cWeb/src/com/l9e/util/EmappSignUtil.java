...
public class EmappSignUtil {
	private static final Logger logger = Logger.getLogger(EmappSignUtil.class);
	 ...
	 */
	public static boolean checkReqSign(String[][] key, String hmac, String appInitKey) {
		try {
			if (key.length<=0) {
				return false;
			}
...
//			Object[] key = hm.keySet().toArray();
			...
			for (int m = 0; m < key.length; m++) {
				sBuf.append(key[m][0] + "=" + (key[m][1]==null?"":key[m][1]) + "&");
			}
			String sString = sBuf.substring(0, sBuf.length() - 1);// .toString();
			String signString = KeyedDigestMD5.getKeyedDigest(sString, appInitKey);
			if (signString != null && signString.equals(hmac)) {
				 ...
				 * 兼容通过utf加密的签名
				 */
				signString = KeyedDigestMD5.getKeyedDigestUTF8(sString, appInitKey);
				if (signString != null && signString.equals(hmac)) {
					return true;
				}
				logger.error("验证的加密串失败|" + sString + "|key=" + appInitKey + "|19pay传的加密串=" + signString);
			}
...
//			});
			...
			String md5Str = Md5Encrypt.md5(rStr, "GBK");
			logger.info("加密串是md5Str:"+md5Str);
			return md5Str;
//			// 分类记录日志
//			if ("0".equals(errcode)) {
...
//				return rStr + "&hmac=" + KeyedDigestMD5.getKeyedDigest(rStr, "");
//			}
//
//			return rStr + "&hmac=" + KeyedDigestMD5.getKeyedDigest(rStr, appInitKey);
	...
	public static void main(String[] args) {
		...
		num[5][0]="status";
		num[5][1]="11";
		System.out.println("最后返回的信息为:"+EmappSignUtil.creatResSign(num, appInitKey));
		...
		boolean flag=false;
		flag=EmappSignUtil.checkReqSign(num, KeyedDigestMD5.getKeyedDigest(signData.toString(), "nihao"), "nihao");
