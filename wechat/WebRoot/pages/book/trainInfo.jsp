...
html {
	...
	window.onload = function(){
		var offset = $('#station_list').offset();
		//alert(offset.top);
		//alert(offset.bottom);
		var avHeight = screen.availHeight;
		//alert(avHeight);
		$("#content").attr("height", avHeight-offset.top-40);
	};
</script>
		...
		<div>
			...
			<div class="wrap" style="background: #fff;">
				...
				<h2 class="book_title">
					...
					<c:if test="${fn:indexOf(cost_time,':') < 0}">
						<fmt:formatNumber value="${(cost_time-cost_time%60)/60}"
							pattern="0" />时${cost_time%60 }分
					</c:if>
				</h2>
				...
				<table cellspacing="0" cellpadding="0" border="0" id="book_list"
					...
					<c:forEach var="seat" items="${seat_info}">
						<c:if
							test='${empty seat.seat_num || seat.seat_num eq "" || (seat.seat_num eq "0")}'>
						...
						<c:if
							test='${!empty seat.seat_num && seat.seat_num ne "" && (seat.seat_num ne "0")}'>
							<tr>
								<td class="book_seat">
									${seat.seat_type}
								</td>
								...
								<td class="book_btn_yes">
									<a href="javascript:void(0);"
										onclick="gotoBookOrder('${travel_time }','${train_code}','${from_city}','${to_city}','${startTime}','${endTime}','${cost_time}','${seat.seat_type}','${seat.seat_price }','${day }');">预
										订</a>
								</td>
							</tr>
						</c:if>
				...
				<table cellspacing="0" cellpadding="0" border="0" id="station_list"
					class="station_list">
					<thead style="background: #ebebeb; display: inherit; width: 100%;">
						<tr>
							<td>
								站次
							</td>
