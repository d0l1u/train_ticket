...
.order_infor dd span {
	...
	$().ready(function() { //页面加载时执行
		...
		if(linker==null || ""==linker) {
			...
			linkerObj =eval('('+linker+')');
		}
		
		document.getElementById("link_name").value = linkerObj.linker_name;
	 	document.getElementById("link_phone").value = linkerObj.linker_phone;
		var myobj;
		...
		for(var i=0;i<myobj.length;i++){
			var passenger_name = myobj(i).name;
        	var user_id = myobj(i).user_id;
        	var ids_type = myobj(i).ids_type;
        	var ticket_type=myobj(i).ticket_type;
        	if(j == 0){
	...
	//添加通行成人
	function adddiv(passenger_name, user_id, ids_type){		
		...
		//alert("html:"+html);
		$("<tr style='background: #F2F2F2; height:8px; width:120%;'><td style='line-height:8px;'>&nbsp;</td></tr><tr>" + html + "</tr>")
			.appendTo("#train_ticket_list").show();
		//if(count>1){
			$("#passenger_delete").attr("class","passenger_delete");
		//}
		...
		*/
		...
		var passengerMsg = '<tr class="adult blank" >'+
					 		'<td>'+(passenger_name, user_id, ids_type)
							'<table style="width:100%;margin:0 0 0 0;padding:0 0 0 0;border:0px solid red;border-top:10px solid #f5f6f6;">'+
							 	'<tr style=" border-bottom:1px solid #C0C0C0">'+
							 		...
							 		'<td class="text_item" style="text-align:right;"><input type="hidden" name="bookDetailInfoList['+count+'].ticket_type" id="ticket_type_'+(count+1)+'" value="0"/>'+
							 			'第<input id="num_'+(count+1)+'" value="'+ (count + 1)+'" style="color:#888;width:6px;border:0;"/>位&nbsp;&nbsp;成人票<a href="javascript:void(0);" id="passenger_delete" class="passenger_delete">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</a>&nbsp;&nbsp;'+
							 	'</tr>'+
							 	'<tr style=" border-bottom:1px solid #C0C0C0">'+
							 		'<td class="text_item" style="color:black;">&nbsp;&nbsp;证件类型</td>'+
							 		'<td class="text_item" style="text-align:right;"><input type="hidden" name="bookDetailInfoList['+count+'].ids_type" id="ids_type_'+(count+1)+'" value="2"/>'+
							 			'二代身份证&nbsp;&nbsp;'+baoxian_str+
							 		'</td>'+
							 	'</tr>'+
							 	'<tr style=" border-bottom:1px solid #C0C0C0">'+
							 		...
							 		'<td class="text_item" >'+
							 			'<input type="text" name="bookDetailInfoList[' + (count ) + '].user_name" id="user_name_'+(count+1)+'" class="pass_ziti pass_inp text_item user_name_text" autocomplete="off" placeholder="请填写真实姓名以免取不出票"  value="' + passenger_id + '"/>'+
									'</td>'+
							 	'</tr>'+
							 	'<tr>'+
							 		'<td class="text_item" style="color:black;">&nbsp;&nbsp;证件号码</td>'+
							 		'<td class="text_item" >'+
							 			'<input type="text" name="bookDetailInfoList[' + (count ) + '].user_ids" id="user_ids_'+(count+1)+'" class="pass_ziti pass_inp text_item" autocomplete="off" placeholder="请填写证件号码" value="' + user_id + '"/>'+
									'</td>'+
	...
	function isEmpty() {
		...
		var count = table.rows.length;
		for(var i = 1;i <= count;i++ ){
			var ticket_type = $.trim(document.getElementById("ticket_type_" + i).value);
			...
			if("" == ticket_type || ("" == name) || ("" == user_id) || ("" == ids_type)){
			return;
		}
		adddiv("", "", "");
	}
					...
					<div class="order_con">
					 ...
					 -->
						...
						<dl class="order_item order_infor">
							...
							<dt>
								乘客信息
								<a href="javascript:isEmpty();" id="add_person"
								...
								<div>
									<p
										...
										<span id="meal_insurance" class="org"
											onclick="save();window.location='/pages/book/orderMeal.jsp';">交通意外险&nbsp;￥20*1</span>
									</p>
								</div>
							</dd>
										...
										<tr style="border-bottom: 1px solid #C0C0C0">
											...
											<td class="text_item">
												<input type="text" name="link_name" id="link_name"
													class="pass_ziti pass_inp text_item" autocomplete="off"
													placeholder="请填写预订人姓名">
											</td>
										</tr>
										<tr>
											<td class="text_item" style="color: black;">
											...
											<td class="text_item">
												<input type="text" class="pass_ziti pass_inp text_item"
													autocomplete="off" placeholder="用于接收购票通知短信"
													name="link_phone" id="link_phone">
											</td>
										</tr>
	...
	function save() {
		...
		for(var i = 1;i <= count;i++ ){
	 	...
	 	var linker_name= $.trim(document.getElementById("link_name").value);
	 	var linker_phone = $.trim(document.getElementById("link_phone").value);
	 	//alert( "insert  into  linkers (name, phone) values('" + linker_name + "', '" + linker_phone + "')");
	 	var linker = {
	 		linker_name:linker_name,
	 		linker_phone:linker_phone
	 	};
	 	localStorage.setItem("linker",JSON.stringify(linker));
	}
