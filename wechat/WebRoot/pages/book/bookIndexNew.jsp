...
<html>
	...
	<script>
			...
			function queryNearDay(travel_time, from_city, to_city , day){
				...
				var url = "/buyTicket/queryByStation.jhtml?travel_time=" + travelTime + "&from_city=" + from_city + "&to_city=" + to_city;
				window.location = encodeURI(url);
			}
							 ...
							 -->
								<c:choose>
										...
										<table class="infotable"
											...
											<c:otherwise>
												<table class="none">
													...
													<tr>
														<td class="checi">
															<span>${list.station_train_code}<!-- ( <c:choose>

																	<c:when
																		test="${fn:contains(list.station_train_code, 'G')}">
												高速动车	
											</c:when>
																	<c:when
																		test="${fn:contains(list.station_train_code, 'C')}">
												城际动车	
											</c:when>
																	<c:when
																		test="${fn:contains(list.station_train_code, 'D')}">
												动车	
											</c:when>
																	<c:when
																		test="${fn:contains(list.station_train_code, 'T')}">
												特快	
											</c:when>
																	<c:when
																		test="${fn:contains(list.station_train_code, 'K')}">
												快车	
											</c:when>
																	<c:when
																		test="${fn:contains(list.station_train_code, 'Z')}">
												直达
											</c:when>
																	<c:otherwise>
												其他
														...
														<td>
															<span style="font-size: 16px;"><c:if
																	test="${fn:substring(list.lishi, 0,2) ne '00'}">${fn:substring(list.lishi, 0,2)}小时</c:if>${fn:substring(list.lishi,
																3, 5) }分</span>
														</td>
														<td class="canbuy">
															<c:choose>
																<c:when test="${list.canBook eq '1'}">
																	<span class="yes">有票</span>
																</c:when>
																<c:otherwise>
																	<span class="no">无票</span>
																</c:otherwise>
															</c:choose>
														</td>
								 ...
								 -->
															...
															<c:if
																test="${list.to_station_name eq list.end_station_name}">
																<span class="ed"
																	id="${list.train_no}_${list.to_station_name}"><em>终</em>${list.to_station_name}</span>
															</c:if>
															<c:if
																test="${list.to_station_name ne list.end_station_name}">
																<span class="ps"
																	id="${list.train_no}_${list.to_station_name}"><em>过</em>${list.to_station_name}</span>
															</c:if>
														</td>
														<!-- <td class="duration">
									<span><c:if test="${fn:substring(list.lishi, 0,2) ne '00'}">${fn:substring(list.lishi, 0,2)}时</c:if><br/>${fn:substring(list.lishi, 3, 5) }分</span>
														...
														<td class="train_price">
															<c:if test="${!empty list.yz && list.yz ne '-'}">硬座<br>
																<strong>￥${list.yz}</strong>
																<br />
															</c:if>
															<c:if test="${empty list.yz || list.yz eq '-'}">
																<c:if test="${!empty list.ze && list.ze ne '-'}">二等座<br>
																	<strong>￥${list.ze}</strong>
																	<br />
																</c:if>
																<c:if test="${empty list.ze || list.ze eq '-'}">
																	<c:if test="${!empty list.rz && list.rz ne '-'}">软座<br>
																		<strong>￥${list.rz}</strong>
																		<br />
																	</c:if>
																	<c:if test="${!empty list.zy && list.zy ne '-'}">一等座<br>
																		<strong>￥${list.zy}</strong>
																		<br />
																	</c:if>

																	<c:if test="${!empty list.yws && list.yws ne '-'}">硬卧<br>
																		<strong>￥${list.ywx}<strong><br />
																	</c:if>
																	<c:if test="${empty list.yws || list.yws eq '-'}">
																		<c:if test="${!empty list.rws && list.rws ne '-'}">软卧<br>
																			<strong>￥${list.rwx}<strong><br />
																		</c:if>
																	</c:if>
																</c:if>
															</c:if>
