...
.phone_popup,.login_popup,.loading_popup,.tip_popup {
	width: 310px;
	background: #FFF;
	position: fixed;
	z-index: 50;
...
.phone_popup dt,.explain_popup dt,.tip_popup dt {
	height: 35px;
	line-height: 35px;
	color: #fff;
	text-indent: 15px;
...
.phone_popup dt a.close,.explain_popup dt a.close,.tip_popup dt a.close
	{
	width: 25px;
	height: 25px;
	float: right;
...
.order_infor .org {
	...
	$().ready(function() { //页面加载时执行
		...
		document.getElementById('trainCode').innerHTML += ("<em>"+localStorage.getItem("trainCode")+"</em>");
		...
		if( baoxian=="0" ){
			...
			//有几个乘客，买几份保险
			$("#meal_insurance").html('交通意外险&nbsp;￥20*'+count);
		...
		$("#train_ticket_list").find("table").find("tbody").append(baoxianStr).show();
	...
	$("#meal_insurance").live("click",function(){
		...
		$('.zjlxchildren').each(function(){
			if(cardType == $(this).text()){
				this.style.color = '#ff5400';
	...
	$('.shadow').live('click',function(){
		$("#vertify_code_btn").attr("disabled", false);
	    $("#vertify_code_btn").html("获得验证码");
		$(this).parent().hide();
	});
	
	$(".zjlxchildren").live("click",function(){
		count = document.getElementById("train_ticket_list").rows.length;
		var selectzjlxHtml=$(this).html();
		if(selectzjlxHtml.indexOf("20")>=0) {
			selectzjlxHtml = "交通意外险 ￥20×" + count + "元";
			$("#product_id").val("PT000000001");
			param_product_id = "PT000000001";
			for(var i =0;i < count;i++){
				$("#product_id_" + (i + 1)).val("PT000000001");
				$("#sale_price_" + (i + 1)).val("20");
				$("#shouldPayMoney").text("￥"+((Number($("#danjia").val())+20)*count))
			}
		}else {
			selectzjlxHtml += " 0元";
			$("#product_id").val("PT000000005");
			param_product_id = "PT000000005";
			for(var i =0;i < count;i++){
				
				$("#product_id_" + (i + 1)).val("PT000000005");
				$("#sale_price_" + (i + 1)).val("0");
				$("#shouldPayMoney").text("￥"+(Number($("#danjia").val())*count))
			}
		}
		$("#"+zjlxid).html(selectzjlxHtml);
		
		/*
		if(selectzjlxHtml!='二代身份证' && $(".InsurancePriceShows").html()=='不购买保险' ){
			$("#"+zjlxid).parent().next().show();
		}else if(selectzjlxHtml!='二代身份证' && $(".InsurancePriceShows").html()!='不购买保险' ){
			$("#"+zjlxid).parent().next().show();
		}else{
			$("#"+zjlxid).parent().next().hide();
		}
		
		if($("#"+zjlxid).hasClass("firstCardType") ){
			if($(".add_child_ticket").length>0){
				$(".add_child_ticket").each(function(){
					setUserCardType($(this),selectzjlxHtml);
						$(this).children("dl").eq(4).show();
						$(this).children("dl").eq(5).show();
				})
			}
		}
		*/
	...
	function showdiv(divname) {      
             ...
             if ($("#" + divname).css("display") == "none") {       //显示div
             	if(divname=="intro"){
             		$(".slide-down").css("background-size","190px 120px");
             	}
                 $("#" + divname).slideDown(500);
             }
             else if ($("#" + divname).css("display") == "block") {       //隐藏div
             	if(divname=="intro"){
             		$(".slide-down").css("background-size","");
             	}
             	$("#" + divname).slideUp(500);
              }
            else  {
	...
	function getVertify(){
		...
		var user_phone=$.trim($("#register_user_phone").val());
		...
		$("#vertify_code_btn").attr("disabled", true);
		$.ajax({
	...
	function remainTime(i){  
	    ...
	    $("#vertify_code_btn").html("<span>请在<em style='color:red;text-align:center;'>" + i-- + "</em>后获取</span>");  
	...
	function submitVertify() {
		var user_phone=$.trim($("#register_user_phone").val());
		var openID=$.trim($("#openID").val());
		var vertify_code = $.trim($("#vertify_code").val());
		if("" == user_phone) {
			alert("请输入本机手机号码！");
			return;
		}
		if(vertify_code.length != 6) {
			alert("请输入正确的验证码！");
			return;
		}
		...
		$.ajax({
				...
				async: false,
				cache: false,
				data: {user_phone:user_phone, openID:openID, vertify_code:vertify_code},
				dataType: "text",
				success: function(res){
					if(res=="0"){
						submitForm();
	...
	function checkUser() {
		...
		$.ajax({
					url:"/register/checkUser.jhtml",
					type: "POST",
					cache: false,
					async: false,
					data: {user_phone:user_phone, openID:openID, user_id:user_id},
					dataType: "text",
					success: function(res){
						alert("返回数据: " + res);
						if(res=="0"){
	...
	function submitOrder(){
		if(checkForm()){
			...
			//selectWz();
			checkUser();
			if(isUser != null && "" != isUser && parseInt(isUser) > 0 && isUser != "0") {
				alert("checkuser after:" + isUser);
				submitForm();
			}else {
				alert("else checkuser after:" + isUser);
				return;
			}
		}
	}
	window.unonload=art.dialog({id:'check'}).close();
	function submitForm(){
				//消息框	
	...
	function checkForm(){
			//保存常用乘客超出系统限制数量
			count = $(".adult:visible,.child:visible").length;
			...
			<div>
					...
					<div class="order_con">
						...
						<dl class="order_item">
							...
							<dd style="line-height: 30px">
								...
								<table
									style="width: 100%; margin: 20px 0 0 0; padding: 0 0 0 0;">

									<tr>
										<td style="width: 10px;"></td>
										<td class="font_18 b"
											style="height: 20px; line-height: 20px; text-align: left;"
											id="startCity"></td>
										<td></td>
										<td class="icon" rowspan="3">
											<span id="trainCode"></span>
											<span class="from_to_icon"></span>
											<span id="travel_time" style="height:25px; vertical-align:top;"></span>
										</td>
										<td class="font_18 b right"
											style="height: 20px; line-height: 20px; text-align: right;"
											id="endCity"></td>
										<td style="width: 10px"></td>
									</tr>
						...
						<dl class="order_item order_infor">
							...
							<dd>
								...
								<div>
									...
									<dl class="passenger_popup">
										<dt>
											...
											<a href="javascript:void(0);" class="close"
												onclick="hideZj('zjlx')"></a>
										</dt>
										<dd>
											<a href="javascript:void(0);" class="zjlxchildren">交通意外险<span
												class="org">￥20元/份</span> </a>
										</dd>
										<dd>
											<a href="javascript:void(0);" class="zjlxchildren">不购买保险</a>
										</dd>
									...
									<!--弹层@填写手机-->
									<article class="phone" style="display:none">
									<div class="shadow"></div>
									<dl class="phone_popup">
										<dt>
											填写手机号
											<a href="javascript:void(0);" class="close"
												onclick="hideZj('phone')"></a>
										</dt>
										<dd>
											<input type="number" name="register_user_phone"
												id="register_user_phone"
												style="width: 100%; line-height: 25px;"
												placeholder="请填写您的手机号吗" />
											<br>
											<span style="font-size: 12px;">本手机号用于注册酷游旅游账号，请填写本机手机号，以免收不到验证码</span>
										</dd>
										<dd>
											<input type="number" name="vertify_code" id="vertify_code" />
											<button id="vertify_code_btn" name="vertify_code_btn"
												class="vertify_code_btn" onclick="getVertify();"
												value="获取验证码">
												获取验证码
											</button>
											<div id="mesg" style="font-size: 12px; color: red;"></div>
										</dd>
										<dd>
											<button id="submitNumber" name="submitNumber"
												class="submit_btn" value="确定" onclick="submitVertify();">
												确定
											</button>
										</dd>
									...
									<p
										...
										<span id="meal_insurance" class="org"
											onclick="javascript:void(0);">交通意外险&nbsp;￥20×1</span>
									</p>
								</div>
							</dd>
				...
				<dl class="order_item">
				 ...
				 -->
							...
							<div
								...
								<div
									...
									<input type="text" class="pass_ziti pass_inp text_item"
										...
										value="${param.link_phone}" id="link_phone">
								</div>
							</div>
					...
					<div class="oz"
						style="position: fixed; bottom: 42px; background-color: #fff; width: 100%;">
						...
						<span class="fl">阅读并同意<span
							...
							onclick="window.open('/pages/common/protocol.html');">酷游旅游服务协议</span>和<span
							style="color: #0081cc; cursor: pointer; line-height: 24px;"
					...
					<div class="order-sum oz">
						<div class="order-box">
							...
							<input class="btn submit-order" type="button" value="提交订单"
								id="btnSubmit" onclick="submitOrder();"
