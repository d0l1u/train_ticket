...
.train_status span {display:block;line-height:25px;width:100%; -webkit-box-sizing:border-box;text-align:right;}
.qtr_number span{display:block; line-height:25px; -webkit-box-sizing:border-box;margin:2px 0 2px 0;padding-left:5px;}
			...
			<table id="train_list" class="qtrain_list">
				<tr>
					<td class="qtr_number">	
						...
						<span><em>${order.train_no}</em>${order.from_station}—${order.arrive_station}</span>
					...
					<c:choose>
						<c:when test="${order.order_status eq '45'}">
							...
							<span class="book_btn_status"><a href="javascript:void(0);"
									onclick="window.location='/query/queryOrderDetail.jhtml?order_id=${order.order_id}&type=detail'">详&nbsp;情</a></span>
						</c:when>
							...
							<c:choose>
								...
								<c:otherwise>
									...
									<span class="border_style" onclick="javascript:window.location='/query/queryOrderRefund.jhtml?order_id=${order.order_id}&type=refund'">&nbsp;退&nbsp;票&nbsp;</span>
									 -->
									<span class="book_btn_status"><a href="javascript:void(0);"
									onclick="window.location='/query/queryOrderRefund.jhtml?order_id=${order.order_id}&type=refund'">退&nbsp;票</a></span>
								</c:otherwise>
							</c:choose>
	                    </c:when>
				...
				<tr><td colspan="5"><HR style="FILTER: alpha(opacity=100,finishopacity=0,style=3)" width="100%" color=#C0C0C0 SIZE=1></td></tr>
			</table>
		</c:forEach>
		</section>
	</div>
