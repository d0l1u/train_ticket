...
*/
	...
	public void addRefundStream(List<Map<String, String>> refundList, String order_id) {
		Map<String, String> map = orderDao.queryOrderInfo(order_id);
		logger.info("订单信息为：" + map);
		logger.info("退款总额为：" + map.get("refund_total"));
		String refundTotal = map.get("refund_total");
		if(refundTotal == null || refundTotal.isEmpty()) {
			refundTotal = "0";
			
		}
		try {
			Double.parseDouble(refundTotal);
		}catch(Exception e) {
			logger.error("[添加退款流水service]数据库中数据发生异常：" + e);
			return ;
		}
		double refund_total = Double.parseDouble(refundTotal); 
