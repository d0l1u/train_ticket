...
public class BuyTicketController extends BuyTicketBase {
	...
	public String gotoBookOrder(HttpServletRequest request,
		...
		// String seatMsg = URLDecoder.decode(this.getParam(request,
		// "seatMsg"),"UTF-8");
		...
		// request.setAttribute("cm_phone", loginUser.getCm_phone());
		// //"15201169346");//
		...
		request.setAttribute("seatPrizeMapper", JSONObject.fromObject(
				seatPrizeMap).toString());
		...
		request.setAttribute("openID", (String) request.getSession()
				.getAttribute("weixinOpenID"));
	...
	public String queryTrainInfo(HttpServletRequest request,
		...
		List<TrainNewStationVo> stationList = ticketService
				.queryWayStationInfo(checi);
		for (TrainNewStationVo tnv : stationList) {
			tnv.reSetInterval();
		}
		/*
		 * String trainStation = HttpUtil.sendByPost(
		 * "http://localhost:8080/station/subwayName.jhtml", param, "utf-8");
		 * System.out.println("返回数据：："+trainStation); List<StationVo>
		 * stationList = JSONArray.toList(JSONArray .fromObject(trainStation),
		 * new StationVo(), new JsonConfig());
		 */
