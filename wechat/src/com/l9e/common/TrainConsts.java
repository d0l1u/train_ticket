...
public class TrainConsts {
	...
	public static final String REFUND_STREAM_FAIL = "44";	//退款失败
	...
	// 初始化
	static {
		BOOKSTATUS.put(PRE_ORDER, "待支付");// 预下单
		...
		REFUND_STREAM_STATUS.put(REFUND_STREAM_WAIT_REFUND, "等待退款");// 等待退款
		REFUND_STREAM_STATUS.put(REFUND_STREAM_BEGIN_REFUND, "正在退款");// 开始退款
		//EFUND_STREAM_STATUS.put(REFUND_STREAM_EOP_REFUNDING, "正在退款");// EOP退款中
		REFUND_STREAM_STATUS.put(REFUND_STREAM_REFUND_FINISH, "退款完成");// 退款完成
		REFUND_STREAM_STATUS.put(REFUND_STREAM_REFUSE, "决绝退款");// 拒绝退款
		REFUND_STREAM_STATUS.put(REFUND_STREAM_FAIL, "退款失败");
