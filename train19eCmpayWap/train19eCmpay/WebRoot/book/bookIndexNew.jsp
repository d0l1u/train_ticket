			...
			<div class="wrap">
				...
				<div class="ser_data">
					<a
						...
						class="prev_data">前一天</a>
					<span href="" class="cur_data"><strong>${paramMap.travel_time}</strong>${paramMap.day}</span>
					<a
						href="javascript:queryNearDay('${paramMap.travel_time}', '${paramMap.from_city}', '${paramMap.to_city}', 1)"
						class="next_data">后一天</a>
				</div>
					...
					<table cellspacing="0" cellpadding="0" border="0" id="train_list"
						...
						<c:forEach items="${osnd.datajson}" var="list">
							<c:if test="${!empty list.station_train_code}">
								...
								<td class="duration">
									<span>${list.lishi}</span>
								</td>
								<td class="train_price">
									<c:if test="${!empty list.yz && list.yz ne '-'}">硬座<strong>￥${list.yz}</strong>
										<br />
									</c:if>
									<c:if test="${empty list.yz || list.yz eq '-'}">
										<c:if test="${!empty list.ze && list.ze ne '-'}">二等座<strong>￥${list.ze}</strong>
											<br />
										</c:if>
										<c:if test="${empty list.ze || list.ze eq '-'}">
											<c:if test="${!empty list.rz && list.rz ne '-'}">软座<strong>￥${list.rz}</strong>
												<br />
											</c:if>
											<c:if test="${!empty list.zy && list.zy ne '-'}">一等座<strong>￥${list.zy}</strong>
												<br />
											</c:if>

											<c:if test="${!empty list.yws && list.yws ne '-'}">硬卧<strong>￥${list.ywx}<strong><br />
											</c:if>
											<c:if test="${!empty list.rws && list.rws ne '-'}">软卧<strong>￥${list.rwx}<strong><br />
											</c:if>
										</c:if>
									</c:if>
								</td>
