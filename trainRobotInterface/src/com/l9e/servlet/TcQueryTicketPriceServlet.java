...
public class TcQueryTicketPriceServlet  extends HttpServlet{
			
			
		  private static final long serialVersionUID = 3264299111876887524L;
		  	
		  	RobotServiceImp robotServiceImp=new RobotServiceImp();
		  	private static Logger logger=Logger.getLogger(TcQueryTicketPriceServlet.class);
		  	...
		  	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		  		...
		  		//日志
		  		log_req.append("同程预订查询").append("param:")
		  		...
		  		if(null == memcache){
		  			...
		  			logger.info("同程预订查询,查询结果jsonStr:"+jsonStr);
		  		...
		  		try {
		  			...
		  			if(null==robot){
		  				...
		  				if(num == 0){
		  					...
		  					//再次发起查询
		  					jsonStr = qc.javaQueryDataAndPrice(param,robot);
		  				}else{
		  					robot = getRobot("java_query",param.get("channel"));
		  					jsonStr = qc.javaQueryDataAndPrice(param,robot);
		  				}
