...
//					paramMap.put("eop_refund_money", refund_money);
	...
	public void cpNotify(HttpServletRequest request,
		...
		logger.info("[接收出票结果通知接口]参数orderId=" + orderId 
				...
				+ "，buyMoney=" + buyMoney + "，seatTrains=" + seatTrains + "，status=" + status+",level="+level+",errorinfo1="+errorinfo1);
...
//					}
				...
				if (count == 1){
					...
					if(!"88".equals(channel)&&"1".equals(orderInfo.getSms_notify())){
						//发送出票成功短信
			...
			if(!StringUtils.isEmpty(orderInfo.getOrder_status())
					&& TrainConsts.ORDER_OUT_TIME.equals(orderInfo.getOrder_status())){
			...
			if(!StringUtils.isEmpty(orderInfo.getOrder_status())
					&& TrainConsts.CANCEL_SUCCESS.equals(orderInfo.getOrder_status())){
