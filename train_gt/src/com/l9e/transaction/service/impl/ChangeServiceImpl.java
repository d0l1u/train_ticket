...
public class ChangeServiceImpl implements ChangeService{
	...
	protected String query_left_ticket_url;//实名认证
	...
	public String queryTime(ChangeInfo changeInfo){
		// 针对没有to_time采用查询补全
		/*String url = TrainConsts.QUERY_TRAIN_TIME;
		String result ="";
		paramMap.put("commond", "queryStation");
		try {
			String params = UrlFormatUtil.CreateUrl(url,paramMap,"");
			result = HttpUtil.sendByGet(params, "UTF-8","30000","30000");
			logger.info("高铁管家改签回调补全时间，查询结果"+result);
		} catch (Exception e) {
			logger.info("高铁管家改签回调补全时间发生异常");
			e.printStackTrace();
		}*/
		String arrive_time="";
		...
		try{
			...
			String url = "http://10.3.12.95:18086/tcQueryTicket";
			...
			logger.info(changeInfo.getOrder_id()+" 高铁管家发起余票查询" + paramMap.toString() + "url" + url);
				...
				try{
					...
					for (int i = 0; i < index; i++) {
						if (arr.getJSONObject(i).get("train_code").equals(changeInfo.getChange_train_no())) {
							...
							arrive_days = arr.getJSONObject(i).getString("arrive_days");
							break;
						}
					...
					logger.info("高铁管家通知,针对没有to_time采用查询补全_查询异常, result="+result);
					e1.printStackTrace();
				}
			}
		} catch (Exception e) {
