		...
		for (Map<String, String> sendMap : sendList) {
			...
			if (eopOrder) {// 发货完成则通知出票系统开始支付出票
				...
				try {
					...
					return_str = HttpUtil.sendByPost(this.getTrainSysSettingValue("notify_pay_interface_url", "ext_notify_pay_interface_url"), reqParams,
							"utf-8");
					logger.info(sendMap.get("order_id") + "请求出票系统支付,返回结果：" + return_str);
				} catch (Exception e) {
					logger.info("通知平台发货失败，通知出票系统开始支付订单失败！", e);
