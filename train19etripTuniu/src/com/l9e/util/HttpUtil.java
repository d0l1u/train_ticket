...
//			System.getProperties().setProperty("http.proxyPort", "41000" );
			...
			e.printStackTrace();
			logger.error("访问远程服务器超时：<" + reqUrl + ">" + e.getMessage(),e);
		} finally {
			try {
				if (in != null) {
					in.close();
		...
		logger.info("请求响应耗时："+(endTime-startTime)/1000d+"秒");
...
//			System.getProperties().setProperty("http.proxyPort", "3128" );
			...
			try {
				...
				logger.error("读取远程服务器返回流异常" + reqUrl + ">" + e.getMessage(),e);
				e.printStackTrace();
			} finally {
				try {
					if (rd != null)
					...
					logger.error("读取远程服务器返回流后关闭读取流异常",e);
				}
			}
		} catch (IOException e) {
			e.printStackTrace();
