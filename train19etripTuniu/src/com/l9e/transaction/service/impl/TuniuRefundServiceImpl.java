...
import com.l9e.transaction.vo.TuniuRefundTicket;
...
public class TuniuRefundServiceImpl extends TuniuCommonServiceImpl implements TuniuRefundService {
	...
	private String tuniuFailMsg(String failId) {
		...
		try {
			...
			switch (codeValue) {
			case 1: return "已取纸制票，不能退票";
			case 2: return "已过发车时间";
			case 3: return "来电取消";
			case 4: return "账号被封";
			case 6: return "退款金额有损失";
			case 7: return "尊敬的旅客，为防止网上囤票倒票，给广大旅客创造一个公平的购票环境，凡通过互联网或手机购买的本次列车车票，如需办理退票、改签和变更到站等变更业务，请持乘车人身份证件原件到就近车站办理，代办时还需持代办人的身份证件原件";
			default:
				return "退票失败";
			}
	...
	private String tuniuFailReason(String failReason) {
		...
		try {
			...
			switch (codeValue) {
			case 1: return "5"; //已取票
			case 7: return "6";//不可退票
			case 2: return "6";//已过发车时间
			case 3: //来电取消
			case 4: return "6";//账号被封
			case 6: //退款金额有损失
			default:
				return "2";
			}
	...
	private String tuniuFailReasonCode(String failReason) {
		...
		try {
			Integer codeValue = Integer.valueOf(failReason);
			switch (codeValue) {
			case 1: return "140003"; //已取票
			case 7: return "140007";//不可退票
			case 2: return "140000";//已过发车时间
			case 3: //来电取消
			case 4: return "140010";//账号被封
			case 6: //退款金额有损失
			case 8: return "140009"; //12306账号密码错误
			default:
				return "140002";
			}
		} catch (NumberFormatException e) {
...
/*		for(TuniuRefundTicket ticket : tickets) {
	...
	Boolean getReturnSuccess(String status){
		if(status.equals(STATUS_URGE_ING)||status.equals(STATUS_URGE_SUCCESS)){
			return true;
		}
		return false;
	}
	String getReturnMsg(String status,String remark){
