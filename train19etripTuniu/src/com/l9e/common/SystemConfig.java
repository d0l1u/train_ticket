		...
		try {
				...
				if (cp == null) {
					...
					try {
						...
						Element cpEditAccount = (Element) cp.selectObject("cp_edit_account");
						...
						TuniuConstant.EDIT_ACCOUNT = cpEditAccount.getTextTrim();
						plog.info("系统参数-修改账号接口：" + TuniuConstant.EDIT_ACCOUNT);
					} catch (Exception e) {
				...
				if (inter == null) {
					...
					plog.info("加载出票系统相关配置信息开始");
					try {
						Element queryTicket = (Element) inter.selectObject("query_ticket");
						Element queryTicketTc = (Element) inter.selectObject("query_ticket_tc");
						Element searchStatus = (Element) inter.selectObject("search_status");
						Element queryTrainTime = (Element) inter.selectObject("query_ticket_time");
						
						TuniuConstant.QUERY_TICKET = queryTicket.getTextTrim();
						plog.info("余票查询接口：" + TuniuConstant.QUERY_TICKET);
						TuniuConstant.QUERY_TICKET_TC = queryTicketTc.getTextTrim();
						plog.info("补全查询接口：" + TuniuConstant.QUERY_TICKET_TC);
						TuniuConstant.SEARCH_STATUS = searchStatus.getTextTrim();
						plog.info("查询订单车票状态接口：" + TuniuConstant.SEARCH_STATUS);
						
						TuniuConstant.QUERY_TRAIN_TIME = queryTrainTime.getTextTrim();
						plog.info("系统参数-查询车次时间接口：" + TuniuConstant.QUERY_TRAIN_TIME);
					} catch (Exception e) {
						plog.error("加载系统接口相关配置信息时 发生异常"+e);
						return false;
					}
				}
