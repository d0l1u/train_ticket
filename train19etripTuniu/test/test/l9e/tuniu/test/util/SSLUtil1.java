...
public class SSLUtil1 {
	...
	private static Logger logger = Logger.getLogger(SSLUtil1.class);
...
//	}
	 ...
	 * @throws IOException
	 */
	public static String httpGet(String url, String charsetName, Proxy proxy, int connectionTimeout, int readTimeout) throws IOException {
		String pageHtml = null;
		
		HttpURLConnection conn = null;
		conn = httpConnection(url, proxy);
		try{
...
//	}
	 ...
	 * @return
	 */
	public static String httpsGet(String url, String charsetName, Proxy proxy, int connectionTimeout, int readTimeout) throws IOException {
		
		String pageHtml = null;
		
		HttpsURLConnection conn = null;
		conn = httpsConnection(url, proxy);
		try{
	 ...
	 * 创建https连接
	 */
	private static HttpsURLConnection httpsConnection(String url, Proxy proxy) throws IOException {
		...
		if(proxy == null) {
			...
			connection = (HttpsURLConnection) connUrl.openConnection(proxy);
		}
	 ...
	 * 创建http连接
	 */
	private static HttpURLConnection httpConnection(String url, Proxy proxy) throws IOException {
		...
		if(proxy == null) {
			...
			connection = (HttpURLConnection) connUrl.openConnection(proxy);
		}
