...
public class MemcachedUtil {
	...
	private static final Logger logger = Logger.getLogger("MemcachUtil");
	...
	public static boolean useMemcache(boolean f) {
		...
		try {
			...
			retF = false;
			logger.info("ErrorMsg=EXCEPTION_Memcached | Timestamp="
					+ DateUtil.dateToString(new Date(), "yyyyMMddHHmmss"));
			e.printStackTrace();
		}
		return retF;
	}
	...
	public static synchronized MemcachedUtil getInstance() {
		...
		if (instance == null) {
			synchronized (lock) {
				if (instance == null) instance = new MemcachedUtil();
			}
		}
		return instance;
	}
	...
	public boolean setMapping(String id, Map valueMap) {
		try {
			return setMapping(id, valueMap, new Date(new Date().getTime()
					+ sessionTimeout));
		} catch (Exception e) {
			logger.info("ErrorMsg=EXCEPTION_Memcached | Timestamp="
					+ DateUtil.dateToString(new Date(), "yyyyMMddHHmmss"));
...
//			if (obj == null) {
//				logger.error("MemcachedUtil 取值失败！");
//			}
		} catch (Exception e) {
			logger.info("ErrorMsg=EXCEPTION_Memcached | Timestamp="
					+ DateUtil.dateToString(new Date(), "yyyyMMddHHmmss"));
	...
	public boolean setAttribute(String id, Object obj, long expireTime) {
		...
		try {
			...
			long end = System.currentTimeMillis();
			logger.info("set id=" + id + "   flag=" + flag + " use time="+ (end - start));
			if (flag == false) {
				logger.info("set id=" + id + "   flag=" + flag + " use time="+ (end - start));
				logger.info("MemcachedUtil 设置值失败！");
			}
		} catch (Exception e) {
			logger.info("ErrorMsg=EXCEPTION_Memcached | Timestamp="
					+ DateUtil.dateToString(new Date(), "yyyyMMddHHmmss"));
	...
	public boolean setAttribute(String id, Object obj) {
		return this
				.setAttribute(id, obj, new Date().getTime() + sessionTimeout);
	}
	...
	protected static class InnerConvertUtil {
		...
		/**
		 * 根据key值了 值 ，如果没有属性文件中没有值，则的返回默认值
		 * @param props
		 * @param key
		 * @param defaultValue
		 * @return
		...
		/**
		 * 根据key返回long值，若属性文件中没有相应的值，则返回默认值
		 * @param props
		 * @param key
		 * @param defaultValue
		 * @return
