...
@NoArgsConstructor
@Slf4j
public class Cancel4ChangeProducerThread extends BaseThread<OrderPo> {
    ...
    public void execute() {
        ...
        log.info("改签取消订单信息:changeId:{}, orderId:{}, supplierType:{}[{}]", changeId, orderId, supplierType,
                SupplierCode.MESSAGE);
        ...
        if (payLimitTime != null) {
            if (payLimitTime.before(new Date())) {
                ...
                changeIdLog(changeId, orderId, ChangeStatus.CANCEL_SUCCESS, "超过支付截止时间，默认取消成功");
                notifyService.updateChangeOrder(changeId);
                return;
            }
        } else if (DateUtil.add(createTime, 1, TimeUnit.DAYS).before(new Date())) {
            log.info("超过30分钟，默认取消成功");
            changeIdLog(changeId, orderId, ChangeStatus.CANCEL_SUCCESS, "超过30分钟，默认取消成功");
            notifyService.updateChangeOrder(changeId);
            return;
        }

        CancelReq req = CancelReq.builder().orderId(orderId).changeId(changeId).sequence(servicePo.getSequence())
        ...
        if (SupplierCode.HTHY.equals(supplierType)) {
            //航天华有
            String httpResult = hthyService.changeCancel(servicePo);
            ...
            if (success) {
                changeIdLog(changeId, orderId, ChangeStatus.CANCEL_SUCCESS, "取消成功");
                //修改通知
                notifyService.updateChangeOrder(changeId);
            } else {
                changeIdLog(changeId, orderId, ChangeStatus.CANCEL_FAIL, String.format("取消失败:%s", msg));
                notifyService.updateChangeOrder(changeId);
            }

//                log.info("请求航天华有改签取消，等待回调结果");
//                historyService.insertChangeLog(orderId, changeId, "请求航天华有改签取消，等待回调结果");
//            } else {
//                log.info("请求航天华有改签取消失败");
//                historyService.insertChangeLog(orderId, changeId, String.format("请求航天华有改签取消失败,Msg:%s", msg));
...
//            }
            ...
            //车票号
            List<String> subSequences = changeCpService.selectSeqByChangeId(changeId);
            req.setSubSequences(subSequences);

            //获取机器人
            workerPo = workerService.available(WorkerType.CANCEL);
            if (workerPo == null) {
                changeIdLog(changeId, orderId, ChangeStatus.CANCEL_FAIL, "无空闲机器人");
                notifyService.updateChangeOrder(changeId);
                return;
            }
            ...
            log.info("{}开始取消", url);
            historyService.insertChangeLog(orderId, changeId, String.format("%s开始取消", publicIp));
            ...
            if (cancelResp.isSuccess()) {
                ...
                changeIdLog(changeId, orderId, ChangeStatus.CANCEL_FAIL,
                        String.format("取消失败:%s", cancelResp.getMessage()));
                notifyService.updateChangeOrder(changeId);
            }
        }
