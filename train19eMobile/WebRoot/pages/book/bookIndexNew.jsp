		                ...
		                <dl class="oz">
		                    ...
		                    <dd><input class="text text2" type="text" id="travel_time" name="travel_time" 
		                    		value="${paramMap.travel_time}" readonly="readonly" 
							  		onfocus="WdatePicker({doubleCalendar: true ,dateFmt:'yyyy-MM-dd',minDate:'%y-%M-%d',maxDate:'%y-%M-{%d+19}'})" /></dd>
		                </dl>
		            </div>
                    <div class="notice oz">
            ...
            <div class="ser_term oz">
                ...
                <h2>
                	...
                	<c:choose>
                		...
                		<strong>${paramMap.from_city}</strong>到<strong>${paramMap.to_city}</strong>的列车
                	</span>
              ...
              <div class="ser_result">  
                <table cellpadding="0" cellspacing="0">
                    ...
                    <!-- 旧接口展示页面 -->
                    <c:if test="${osnd ==null && unBookList==null && otVo!=null}">
	                    ...
	                    <c:forEach items="${osnd.datajson}" var="list">
	                    	<c:if test="${!empty list.station_train_code}">
	                    		<tr class="trainBar" id="${list.train_no}_ticket_${fn:substring(list.station_train_code, 0, 1)}_${tn:fromTimeStr(list.start_time)}_${tn:toTimeStr(list.arrive_time)}" startTime="${list.start_time}">
			                        ...
			                        <td>${list.lishi}</td>
			                        <td class="seat">
				                        <c:if test="${!empty list.yz && list.yz ne '-'}">硬座<span>￥${list.yz} [${list.yz_num_show}]</span><br /></c:if>
				                        <c:if test="${!empty list.rz && list.rz ne '-'}">软座<span>￥${list.rz} [${list.rz_num_show}]</span><br /></c:if>
				                        <c:if test="${!empty list.zy && list.zy ne '-'}">一等座<span>￥${list.zy} [${list.zy_num_show}]</span><br /></c:if>
				                        <c:if test="${!empty list.ze && list.ze ne '-'}">二等座<span>￥${list.ze} [${list.ze_num_show}]</span><br /></c:if>
				                        <c:if test="${!empty list.yws && list.yws ne '-'}">硬卧<span>￥${list.yws}/${list.ywz}/${list.ywx} [${list.yw_num_show}]</span><br /></c:if>
				                        <c:if test="${!empty list.rws && list.rws ne '-'}">软卧<span>￥${list.rws}/${list.rwx} [${list.rw_num_show}]</span><br/></c:if>
				                        <c:if test="${list.gws ne '-'}">高级软卧<span>￥${list.gws}/${list.gwx} [${list.gr_num_show}]</span><br/></c:if>
				                        <c:if test="${list.tdz ne '-'}">特等座<span>￥${list.tdz} [${list.tz_num_show}]</span><br/></c:if>
				                        <c:if test="${list.swz ne '-'}">商务座<span>￥${list.swz} [${list.swz_num_show}]</span><br/></c:if>
			                        </td>
			                        <td>
				                        <c:choose>
				                        	<c:when test="${sessionScope.showJm_Gg eq '2'}">
