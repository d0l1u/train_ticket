            ...
            <div class="pub_order_mes ticket_mes oz mb10_all">
                ...
                <div class="pub_con">
                	<table class="pub_table">
                        ...
                        <tr>
                        	<td width="280">坐 席：<select name="seat_type" id="seat_type">
                                <option value="" selected="selected">请选择</option>
                            	<c:forEach items="${seatInfoList}" var="seat">
                            		<option value="${seat.seatType}">${seat.seatName}</option>
                            	</c:forEach>
                            </select><span id="wz_agree" style="display: none;"><span class="display_mid" style="padding-left: 20px;"><input
                            	 type="checkbox" id="wz_chk" /></span><label for="wz_chk" style="color:#0081cc;">同意硬座无票时购买无座</label></span>
                            </td>
                        </tr>
                        <tr>
                        	<td>票面价：
        	...
        	<div class="pub_delivery_mes oz mb10_all" id="fp_content">
        	 ...
        	 <p style="padding-left:153px;">
        	 	<span class="display_mid">
        	 		<input type="checkbox" id="chk_bxconfirm"/>
        	 	</span>
        	 	我已阅读并同意
        	 	<span style="color:#0081cc;cursor:pointer;line-height:24px;" onclick="window.open('<%=basePath%>pages/help/protocol.jsp');">《火车票线下代购服务协议》</span>
    ...
    $(document).ready(function(){
		...
		$("#btnSubmit").click(function(){
			if(checkForm()){
				...
				//提示是否不添加通行乘客
				if($(".ticket_type_select:visible").length==1){
					art.dialog.confirm('您是否确认不添加同行乘客？<br />如果不添加同行人，分开订购会导致座位号不连续！', function(){submitForm();},function(){});
				}else{
					submitForm();
				}
			}
			...
			$.ajax({
				...
				success: function(data){
					if(data=='SUCCESS'){
						...
						if(strAll!= ''){
							art.dialog.confirm(strAll,function(){},function(){});
						}
					}
				}
			});
		return checkIdsCards;
