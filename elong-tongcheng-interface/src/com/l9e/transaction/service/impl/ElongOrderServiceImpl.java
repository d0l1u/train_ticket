...
public class ElongOrderServiceImpl implements ElongOrderService{
	...
	public String send(ElongOrderInfo orderInfo, String type) {
		...
		try {
			...
			paramMap.put("channel", "elong");//合作商编号
			//购买了保险
	...
	@Override
	public void initNotice(String resultStr,String type,String order_id) {
		boolean sendStatus="Success".equalsIgnoreCase(resultStr);
		if("book".equalsIgnoreCase(type)){
			/**加入先预订后支付的通知表**/
			DBNoticeVo notice=new DBNoticeVo();
			notice.setOrder_id(order_id);
			notice.setCp_notify_status(sendStatus?Consts.NOTICE_OVER:Consts.NOTICE_START);
			notice.setChannel("elong");
			notice.setBook_notify_url("");//预订成功异步回调地址
			noticeDao.insertNotice(notice);
		}else{
			/**加入黄牛票模式的通知表*/
			ElongOrderNoticeVo notice=new ElongOrderNoticeVo();
			notice.setOrder_id(order_id);
			notice.setCp_notify_status(sendStatus?Consts.NOTICE_OVER:Consts.NOTICE_START);
			elongNoticeDao.addNotice(notice);
		}
		ElongOrderLogsVo log=new ElongOrderLogsVo();
		log.setOpt_person("elong_app");
		log.setContent("通知出票系统_"+resultStr+","+("outTicket".equalsIgnoreCase(type)?"黄牛":"先预定"));
	...
	public void addNotifyCpSys(Map<String, Object> orderInfo,
		...
		try {
			...
			paramMap.put("outtickettype", "11");//
			paramMap.put("channel", "elong");//合作商编号
			if("11".equals(orderInfo.get("order_type").toString())){
				//先预订后支付
				paramMap.put("ispay","11");
			}else{
				//先支付后预订
	...
	public void addOrder(ElongOrderInfo orderInfo) {
		...
		/*if("11".equals(orderInfo.getOrder_type())){
			*//**加入整合版通知表**//*
			DBNoticeVo notice=new DBNoticeVo();
			notice.setOrder_id(orderInfo.getOrderId());
	...
	public String queryChangeCpPayMoney(Map<String, String> paramMap) {
		return elongOrderDao.queryChangeCpPayMoney(paramMap);
	}
	@Override
	public String queryChangeFromTime(Map<String, String> paramMap) {
		return elongOrderDao.queryChangeFromTime(paramMap);
	}
	
	@Override
	public ElongOrderInfoCp queryElongCpInfoByCpId(String cp_id) {
		return elongOrderDao.queryElongCpInfoByCpId(cp_id);
	}
	@Override
	public Map<String, Object> queryAccountOrderInfo(Map<String, String> param) {
		return elongOrderDao.queryAccountOrderInfo(param);
	}
