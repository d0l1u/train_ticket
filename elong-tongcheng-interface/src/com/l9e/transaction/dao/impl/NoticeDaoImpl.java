...
public class NoticeDaoImpl extends BaseDao implements NoticeDao{
	...
	public void insertRemedyNotice(String order_id) {
		this.getSqlMapClientTemplate().insert("allNotice.insertRemedyNotice",order_id);
	}

	@Override
	public void updateRemedyNotice(DBRemedyNoticeVo noticeInfo) {
		this.getSqlMapClientTemplate().update("allNotice.updateRemedyNotice",noticeInfo);
	}

	@Override
	public List<DBRemedyNoticeVo> selectOrderRemedyList() {
		return this.getSqlMapClientTemplate().queryForList("allNotice.selectOrderRemedyList");
	}
