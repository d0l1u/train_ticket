	...
	public int orderbillByList(final int getNum) throws RepeatException, DatabaseException {
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				sql.append("select c.order_id from cp_orderinfo_refund c where c.order_status=? ");
...
//							map.put("cp_id", cpId);
							...
							logger.info("订单号："+orderId+"，存在06状态车票的数量是："+count);
						}
					}
				}
...
//					cs.setString(3, cp_id);
					...
					logger.info("query start refund order_id="+order_id+",cp_id="+cp_id+",refund_type="+refund_type);
					...
					if(num == 1){//update执行成功
						...
						sql.append(",c.user_name,c.ticket_type,c.ids_type,c.user_ids,c.channel, ");
						...
						while(rs.next()){
							...
							logger.info("refund add list success:"+order_id+"|"+cp_id);
//							//更新订单车票信息为“正在退票”状态
//							sql = new StringBuffer();
...
//							cs.executeUpdate();
						...
						logger.info("get the order_id info:"+order_id+",cp_id:"+cp_id);
					}
 				}
	 ...
	 * @throws DatabaseException
	 */
	public int updateCPOrderRefund(final Map<String,String> refund_map)throws RepeatException, DatabaseException{
		return dbbean.executeMethod(new ICallBack() {
			@Override
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				StringBuffer sql = new StringBuffer();
				...
				cs = cn.prepareStatement(sql.toString());
				int index = 1;
				cs.setString(index++, refund_map.get("refund_12306_money"));
				cs.setString(index++, refund_map.get("refund_12306_seq"));
				cs.setString(index++, refund_map.get("order_id"));
				cs.setString(index++, refund_map.get("cp_id"));
		...
		return dbbean.executeMethod(new ICallBack() {
			...
			public int execute(Connection cn, PreparedStatement cs) throws SQLException {
				...
				if("".equals(robot_id)){
					...
					sql_rand = new StringBuffer();
					sql_rand.append("select worker_id,worker_name, worker_type,worker_ext,robot_id ")
					...
					sql_rand = new StringBuffer();
					sql_rand.append("select worker_id,worker_name, worker_type,worker_ext,robot_id, worker_status ")
