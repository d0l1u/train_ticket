	...
	public final void run() {
		try{
			while(!Thread.interrupted()){
				...
				T t = queue.take();
				if(t ==null){
					logger.debug("消费者"+consumerid+"没有获取到待消费资源");
					continue;
				}
				if(consume(t)){
					//如果消费成功，则完成
					...
					if(isNeedReOffer){
						...
						reofferqueue.put(t);
						logger.info("消费者"+consumerid+"未完成"+getObjectKeyId(t)+"消费,将资源放入reoffer队列");
						
					}else{
						//不需要重新插入队列标识时，从存在的资源列表中删除
						logger.info("消费者"+consumerid+"未完成"+getObjectKeyId(t)+"消费,从存在的资源列表中删除");
					}
				}
			}
		}catch (Exception e) {
